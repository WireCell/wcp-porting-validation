#!/usr/bin/env python3
"""FORK of scanbot_capture.py (doc pr/116 bulk run) -- adds --vtx-span only.

Duplicated rather than edited (CLAUDE.md M10): the original is being driven by
four other agents concurrently, so it stays byte-for-byte untouched.

Why the fork exists.  The vertex sweep is framed from r95 of EVERY segment
point, so one long track sets the scale for the whole event: on evt283713 a
502 cm muon put the six vertex frames at +-400 cm and left the vertex region a
few pixels across, which makes the brief's "judge vertex-bad first" step
unperformable.  --vtx-span CM overrides the half-span for the vertex sweep and
for it alone; unset, the framing is identical to the original.
...
def main():
    ...
    ap.add_argument("--vtx-span", type=float, default=None,
                    help="half-span in cm for the vertex sweep only.  Unset = "
                         "the event's own r95 scale, i.e. identical to "
                         "scanbot_capture.py.  Use when event_half_span_cm "
                         "comes out huge because of a long track.")
    ...
    with sync_playwright() as pw:
        ...
        r_vtx = opts.vtx_span if opts.vtx_span else r_evt
        if opts.vtx_span:
            note("vertex sweep framed at half-span %.1f cm by --vtx-span "
                 "(event scale would have been %.1f cm)" % (r_vtx, r_evt))
        for az, el in SWEEP:
            cam_set(az, el, centre=(vx, vy, vz), R=r_vtx)
            shot("vtx-az%+04d-el%+03d" % (az, el))

        # A close-in second sweep whenever the event scale swamped the vertex.
        # Measured over waves 1-2 this fires on about a third of events -- one
        # long track is enough -- so it is not an exception worth a manual step.
        # Extra frames only: the vtx-* set above is untouched.
        r_close = 60.0
        if not opts.vtx_span and r_vtx > 150.0:
            note("vertex swamped at %.0f cm half-span -> extra close-in sweep at"
                 " %.0f cm, frames vtxz-*" % (r_vtx, r_close))
            for az, el in SWEEP[:3] + SWEEP[4:5]:
                cam_set(az, el, centre=(vx, vy, vz), R=r_close)
                shot("vtxz-az%+04d-el%+03d" % (az, el))
        ...
        # Pick the probe point FARTHEST from the projected centroid, do not take
        # xs[0][0].  On any event whose first segment starts at the nu vertex,
        # xs[0][0] IS the vertex; the sweep centres the camera there, so it sits
        # on the rotation axis and an azimuth change cannot move it.  The old
        # probe then reported segments_redrew=False on a perfectly good event
        # (evt350121: 9.88e-7 -> 1.03e-6), which reads as "stale frames, stop"
        # -- a false negative that condemns the event instead of the test.
        FAR = """(() => {
            const s = M('seg3_src').data;
            let sx = 0, sy = 0, c = 0;
            for (let i = 0; i < s.xs.length; i++)
                for (let j = 0; j < s.xs[i].length; j++) {
                    sx += s.xs[i][j]; sy += s.ys[i][j]; c++; }
            if (!c) return null;
            const mx = sx / c, my = sy / c;
            let bi = 0, bj = 0, best = -1;
            for (let i = 0; i < s.xs.length; i++)
                for (let j = 0; j < s.xs[i].length; j++) {
                    const d = (s.xs[i][j]-mx)**2 + (s.ys[i][j]-my)**2;
                    if (d > best) { best = d; bi = i; bj = j; }
                }
            return [bi, bj, Math.sqrt(best)];
        })()"""
        far = js(FAR)
        pi, pj, prad = (far if far else (0, 0, 0.0))
        az0 = js("M('cam_src').data.az[0]")
        u0 = js("M('seg3_src').data.xs[%d][%d]" % (pi, pj))
        c0 = js("M('cloud_src') && M('cloud_src').data.u.length"
                " ? M('cloud_src').data.u[0] : null")
        drag(100, 0)
        az1 = js("M('cam_src').data.az[0]")
        u1 = js("M('seg3_src').data.xs[%d][%d]" % (pi, pj))
        ...
        drag_check = dict(
            ...
            probe_point=[pi, pj], probe_radius_px=round(prad or 0.0, 3),
            probe_degenerate=bool((prad or 0.0) < 1.0),
        ...
        note("drag check: 100 px -> d_az %.4f rad (expect %.4f); segments moved %s"
             " (probe [%d][%d], %.1f px off centroid); cloud moved %s"
             % (drag_check["d_az_rad"], drag_check["expect_rad"],
                drag_check["segments_redrew"], pi, pj, prad or 0.0,
