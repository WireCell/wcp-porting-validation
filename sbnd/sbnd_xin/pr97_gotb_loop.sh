...
one() {
    ...
    ( cd "$R/.cwd" || exit 92
      # doc pr/97: Go's signal-based async preemption (Go 1.14+) is the classic
      # doc pr97: crash walls cluster at 126-129 s across five crashes from different
      # sweeps, i.e. a fixed ~120 s of process life once ROOT.s ~7 s stack-trace
      # gdb is subtracted -- Go sysmon.s forcegcperiod is exactly 2 min.  GOTRACEBACK=crash
      # GOTRACEBACK=crash makes the Go runtime print its OWN traceback (goroutines,
      # not C frames) before dying, which is the Go-side stack the gdb captures
      # cannot give.  For the upstream report.  It runs the
      # otherwise runs the UNMODIFIED production runner.
      GOTRACEBACK=crash \
