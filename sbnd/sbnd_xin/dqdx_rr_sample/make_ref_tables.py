...
def main():
    ...
    # ---- regression: the Box tables must BE the shipped ones, all five -------
    f = uproot.open(SBND_DQDX)
    print(f"Box tables (A=0.93, B=0.212, C={BOX_FUDGE}) vs {os.path.basename(SBND_DQDX)}:")
    box = {}
    worst = 0.0
    for part in PARTS:
        _, y = table(fr, graphs, part, "box_fixed", [], BOX_FUDGE)
        box[part] = y
        rel = float(np.max(np.abs(y / np.asarray(f[part].values("y"), float) - 1)))
        worst = max(worst, rel)
    ...
    out = {"_meta": {
        ...
        "Box_keys": {
            ...
            "note": "the tables the running config uses "
                    "(sbnd/particle_dataset.jsonnet, "
                    "energy_loss/pion_travel/stopping_ave_dQ_dx_sbnd.root, "
                    "generated by convert_field.C).  Reproduced here to "
                    f"{worst:.1e} relative.  MIP_DQDX and anything reporting on "
                    "a TaggerCheckSTM decision must read THESE keys.",
        },
