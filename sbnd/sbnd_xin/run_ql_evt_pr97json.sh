...
process_event() {
    ...
    # the config is compiled to JSON by a SEPARATE wcsonnet process first, so
    # the wire-cell process that does the clustering never runs gojsonnet.
    # Tests the doc pr/97 sec.5 hypothesis that the ~4 %-per-run SIGSEGV on
    # SBND 18255-178410 is a parked Go runtime thread, not the clustering:
    # gdb caught thread 34 (Go, PC=0x0) taking the signal while thread 1 was
    # healthy inside hough_transform.  Same TLAs, so the compiled config is
    # identical -- prove it with a normdiff against the in-process compile.
    local _cfg="$QLDIR/pr97-precompiled-cfg.json"
    wcsonnet \
        --tla-str  "input=$QLDIR" \
        "${ANODE_TLA[@]}" \
