...
- **THE BIG ONE, reported not fixed.** SBND runs one `FlashTensorToOpticalPCs` per APA and each does `lpcs["flash"] = ...` (ASSIGN), so the archive keeps only the LAST APA's flash list, while `QLMatching` stores the PER-APA row id on the cluster. Production `T_cluster`, 3067 events, 171 664 matched rows: **50.6% CORRECT, 48.6% resolve to a different REAL flash, 0.8% were out-of-range**. The matching itself is fine -- each APA matched against its own list while it was live; only the read-back after the fact is wrong. `flash.time() == cluster_t0` is the bit-exact discriminator. Three remedies costed in doc 99 sec 2; picking one is an owner call because option 1 changes archive bytes.
...
- **REPAIR OWED (owner):** `CONFIRM=yes ./scripts/d99_repair_nuecc48_merge.sh` restores `work-nuecc48-d97fvpr2`'s 49/547-row tables from the 48 intact per-event files. Dry run clean on all three interlocks, including "every row of the current truncated files is reproduced verbatim". Originals preserved at `work-nuecc48-d97fvpr2/merge-truncated-20260902/`. Not applied: CLAUDE.md sec 5.2, and the permission gate declines the write.
