...
## 14.6 Owner Bee verdicts on the 13 fires (2026-08-30, verbatim-condensed)

| bee_idx | event | owner verdict | classification |
|---|---|---|---|
| 0 | 103798 | "looks OK, overclustering of two gamma from a pi0 in both old vs. new" | fire tolerated; the gamma-gamma overclustering PRE-EXISTS on the OFF arm — consistent with sec 14.4's angle compression (the two gammas are spatially entangled) |
| 1 | 281485 | "looks OK" | fire tolerated |
| 2 | 409634 | "looks OK" | fire tolerated |
| 3 | 396222 | "very busy, hard to judge" | no verdict (ledger keeps the g2 4.20x OVER flag) |
| 4 | 172942 | "incorrect nu vertex, looks OK" | pairing OK; vertex defect (sample mcp1k) |
| 5 | 393212 | "the lower energy gamma from the pi0 does not look like part of pi0" | **ADVERSE fire** — fake member |
| 6 | 401450 | "the 200 MeV gamma does not look right in terms of start vertex" | start-vertex defect — joins the sec 13.1 deep-start ledger |
| 7 | 347824 | "looks OK, the clustering of the 130 MeV gamma should go with 82 MeV gamma for the single EM clustering" | fire tolerated; NEW EM under-clustering item (130+82 should be ONE cluster) |
| 8 | 54332 | "incorrect vertex" | vertex defect |
| 9 | 180801 | "incorrect nu vertex" (owner wrote 18081) | **NC pi0, wrong nu vertex — IN SCOPE** (ncpi0 sample; the round-5 scope rule exempts NC pi0) |
| 10 | 259542 | "incorrect vertex" | **NC pi0, wrong nu vertex — IN SCOPE** (ncpi0 sample) |
| 11 | 84229 | "the gamma with 914 MeV contains two gammas from a pi0 decay" | over-clustering: ONE shower holds BOTH gammas — the sec 10.6 substructure-split class, new specimen |
| 12 | 235435 | "a single EM shower was clustered as a hadronic shower" | EM-vs-hadronic typing miss (the pr/126 "PID is the top pi0 blocker" thread) |

Net owner read on K18 v2: 6 of 13 fires visually tolerable, 1 adverse
(393212), the rest blocked by upstream defects (vertex, start, typing).  The
gamma ledger cost (sec 14.3: 5 OK gammas lost, 0 healed, and the energy
record is what T_KINE's mass calibration rides on) stands, and no flip was
ordered — **K18 remains DEFAULT OFF**.  If the owner later weighs
pair-count over gamma-energy fidelity, r7am2 + this table is the evidence
base for that call.

## 14.7 Round-8 recommendation, updated on the scan

The 13 verdicts sort almost perfectly into the campaign's open fronts, and
they CHANGE the priority order.  The scan surfaced five wrong-vertex events,
and two of them are NC pi0 on the ncpi0 sample — the one class where the
owner's scope rule (sec 12) explicitly permits moving the nu vertex.

**Front B — NC pi0 vertex correction (recommended round-8 opener).**
Specimens now: 180801, 259542 (this scan, ncpi0), 76346, 116962 (round-3
verdicts: "NC pi0, the nu vertex is inside an EM shower"), plus 54332 and
172942 flagged wrong-vertex (scope to be checked per event).
`id_pi0_without_vertex` is the sanctioned mechanism (it already mutates
`main_vertex->fit().point` on accept) and it fired 0-1 times in 239 events
across every arm this campaign — the gates, not the physics, keep it shut.
Round 4 probed the FINDER acceptance (K13-K15, measured dead) but never
attributed which GATE (GATE1 >2-segment, GATE2 first/last-in-shower,
early-return-on-paired) kills each NC specimen.  Step 1 is that byte-neutral
attribution probe on the 5-6 specimens; step 2 is a gate knob chosen from
the evidence, with the round-4 lesson (281567's ADVERSE 21.9 cm shift)
guarded by K10 `pi0_nv_max_vtx_shift_cm` — which already exists, DEFAULT
OFF, unmeasured.
**Front A — the opening-angle probe** (sec 14.4) stays queued behind B:
byte-neutral opening-angle census vs label axes on the below-window
population (401450's bad start is a new specimen), then vertex->centroid
pair rays if the census says the angle deficit is systematic.
**Front C — the both-gammas-in-one-shower split**: 84229 (914 MeV) joins
37112; re-run WCT_PI0_SUBSTRUCT_DEBUG on it before re-litigating the sec
10.6 "recovery impossible" verdict — 99193's ax=1 split landed at m=133.4
in round 3, so the class is not uniformly hopeless.
...
# 16. Round 9 — K19: the back-projection NC vertex proposer, measured dead (2026-08-30)

Owner order: *"Please proceed to the round-9"* on the sec 15.3
recommendation.  New component `id_pi0_backproject_vertex`
(NeutrinoShowerClustering.cxx), knob **K19 `pi0_bp_vertex_miss_cm`**
(DEFAULT OFF), runs BEFORE path 1 (fixing the round-8 ordering lock), fires
only on an NC signature gate, proposes the decay point as the
closest-approach midpoint of two shower axes back-projected from their
starts, and applies the P2 acceptance verbatim (vertex hack + registration,
pio_type 2; path-1 pools skip proposer-paired showers, knob-gated).
Acceptance: miss < knob, crossings behind both starts (<= 120 cm, 1 cm
slop), shift 1-100 cm, pair mass in the (100,160) window.

## 16.1 The signature iterations (smoke, specimens)

* v1 `map_vertex_in_shower.count(main_vertex)`: **never fires** — the wrong
  vertex is seated as the shower's START vertex, which that map excludes by
  construction (`PI0_NCVTX in_shower_map=0` on 180801/259542).
* v2 "all main-vertex segments in showers": excludes 180801/259542 (each
  carries a non-shower stub prong).
* v3 = the P2 gate2 semantics at any prong count (>= 2 segs, >= 1 in a
  shower, none in a long muon).  Fires on 4 of 6 specimens.

Smoke: **116962 rescued** (pair 21072+55030, miss 1.5 cm, m=125.8, shift
12.0) and **76346 rescued at miss >= 7** (its true pair 14059+41031,
m=129.6, shift 4.3 — but truth-click distance 30.3 -> 33.7 cm: the crossing
recovers the PAIR, not the vertex).  **180801/259542 unreachable**: both
arms attach at the same wrong point, so their axes have no upstream
crossing (180801's only crossings pair a third shower at m=188/256).

## 16.2 Full arms at miss = 8 cm (r9off / r9bp)

Gate PASS 4/4 (478 archives, r9off vs r7off2); wcdoctest 2581/2581;
compiled-config proofs per event.  13 fires (shifts 1.3-79.7 cm):

| metric | r9off | r9bp |
|---|---|---|
| census exact/partial/none/no-group | 31/16/1/18 | **32**/15/1/18 (165157 partial->exact) |
| gamma ledger | 90.9% OK | 90.9% OK, zero row diffs |
| movers (vtx105) | — | **7 ADVERSE** (5 moved OFF a 0.00-cm click: 506746 owner-good, 486687, 348691, 406125, 347824; plus 166870, 76346), 1 toward (409888), 1 away |

## 16.3 The round-9 finding: the pi0 observables are FLAT across the vertex ambiguity

The offline v4 test (each fired pair's mass recomputed at the CURRENT
vertex): the fires that damage correct vertices are mostly in-window at the
current vertex too (166870 139.2, 348691 145.2, 486687 157.2) — a "move
only if necessary" guard suppresses them.  But it suppresses the NC targets
identically: on 76346 path 1 already accepts the SAME pair at the wrong
vertex (the census-E row), and on 116962 path 1 accepts an overlapping
pair — for the NC class the pair mass barely changes over the 4-30 cm
vertex ambiguity.  Mass + crossing geometry cannot identify which point is
the vertex; the discriminating information is NON-pi0 (track activity /
vertex dQ/dx / imaging), which no pi0-finder knob carries.  And the
boundary cases (347824 m_cur=100.0, 406125 m_cur=44.6) stay ADVERSE under
any window guard.  **K19 stays DEFAULT OFF** — +1 exact does not buy 7
ADVERSE vertices, five of them dead-on clicks.

## 16.4 Bee A/B (owner-requested, round 9)

* OFF (r9off, production point): `https://www.phy.bnl.gov/twister/bee/set/82e77f9d-fc18-49aa-a219-81dd9a6c05fb/event/list/`
* ON (r9bp, K19@8cm): `https://www.phy.bnl.gov/twister/bee/set/803e2f62-4b43-4d9f-b15b-62ab672d812c/event/list/`
* Annotated index: `bee/pr132r9/pr132r9.index.txt` (13 fires; the 2 NC
  rescues first, then the census win, then the 5 ADVERSE-at-zero).

## 16.5 Round-10 queue

1. The NC re-vertexer needs a non-pi0 discriminator (vertex-activity dQ/dx,
   track stubs, imaging-level point selection) — an owner-level design
   item, out of pi0-finder scope.
2. Front A (opening-angle census vs label axes, secs 14.4/14.7) — still
   the one unmeasured in-scope lever.
3. K16@120 owner call (round 5) still open.

Repro (round 9):
```
# smoke: WCT_PI0_PAIR_DEBUG=1 SBND_PI0_BP_VERTEX=6|12 run_pr_chain_batch.sh ... (specimens; r9dbg3-r9dbg5)
PR_JOBS=12 bash scripts/pr132_arms.sh 98 r9off 0   && ...141...   # gate vs r7off2: PASS 4/4
PR_JOBS=12 bash scripts/pr132_arms.sh 98 r9bp 1 SBND_PI0_BP_VERTEX=8  && ...141...
bash scripts/pr132_r2_manifests.sh r9bp
python3 scripts/pr132_pi0_census.py --manifest98 em117-132r9bp98-manifest.tsv --manifest141 em114c-132r9bp141-manifest.tsv \
    --fudge 0.84 --overlay-tag pi0scan-0829-agent --tsv docs/pr/pr132-census-r9bp.tsv
python3 scripts/pr132_gamma_ledger.py ... --tsv docs/pr/pr132-gamma-ledger-r9bp.tsv   # zero diffs
for s in mcp1k mcp2k ncpi0 nuecc48; do python3 scripts/pr90_movers.py work-pr132-r9off-$s work-pr132-r9bp-$s --tags vtx105; done  # 7 ADVERSE
```

# 17. Round 10 — Front A: the opening-angle census, the centroid ray measured dead (2026-08-30)

Owner order: *"...then proceed to round-10 improvements"*.  Executes sec
16.5 item 2 (the last unmeasured in-scope lever): does recomputing the
pair rays from vertex -> charge-centroid (instead of vertex -> start-point)
decompress the below-window population that round 7 diagnosed?

## 17.1 The tape and the census

`WCT_PI0_ANGLE_DEBUG` (byte-neutral, stderr): for every recorded path-1
pair, the legacy mass/angle next to the centroid-ray variant
(vertex -> associate-cloud centroid, both members).  Scored offline by
`scripts/pr132_angle_census.py` against the hand labels (label opening
angle = angle between the two per-gamma label axes).  Full manifests
(r10ang arms), 66 hand pairs:

| class | n | meaning |
|---|---|---|
| IN->IN | 29 | start ray already matches the label angle; centroid harmless |
| no-pair-on-tape | 14 | a gamma never associates (the no-group population) |
| other | 11 | m_start far above window (over-merge / fusion masses) |
| BELOW->BELOW | 5 | below-window on BOTH rays |
| **IN->OUT** | **5** | centroid ray pushes a good pair OUT (314838: 26.3 -> 3.8 deg!) |
| **BELOW->IN** | **2** | the gains (64591: 47.4 -> 77.5 deg vs label 90.0; 168432) |

## 17.2 The verdict — and what the below-window population actually is

The centroid-ray substitution is **measured dead**: 2 gains vs 5 in-window
losses on the hand set (overlapping showers put both centroids on one ray
from the vertex — 314838 collapses to 3.8 deg; lateral clouds blow 392901
up from 56 to 99 deg).  No knob is implemented — the tape census settles
the trade offline before an arm is spent.

The deeper result corrects round 7's diagnosis: the below-window
population is HETEROGENEOUS —

* genuinely angle-compressed AND centroid-fixable: ~2 (64591, 168432);
* angle-compressed but NOT centroid-fixable: 105946 (a_start 17.8 vs label
  72.7; a_cent 19.1 — the charge centroid carries the SAME downstream bias
  as the start, they are the same corrupted charge);
* angle CORRECT, below-window from CHARGE: 280972 (17.1 vs label 17.1),
  281485 (93.0 vs 81.7), 54341 (53.6 vs 64.1) — the rounds-5/6 UNDER
  classes, unreachable by any ray construction.

## 17.3 Campaign state after round 10

Every in-scope lever is now measured: K12/K16/K17 (geometry merges), K18
(acceptance-aware merge), the P2 gate family, K19 (back-projection
re-vertexer), and the centroid ray — all dead, each with its mechanism
identified.  Production stands at **31 exact / 2 fakes / 90.9% ledger-OK**
(fudge 0.84 + K7/K8 + K3=28 + sccc/K5 sentinels).  What remains is
owner-level: (1) the non-pi0 NC vertex discriminator (sec 16.3); (2) the
K16@120 call (round 5); (3) upstream imaging for the UNDER-nosibs /
deep-start charge (the ledger's standing items).

Repro (round 10):
```
PR_JOBS=12 bash scripts/pr132_arms.sh 98 r10ang 1 WCT_PI0_ANGLE_DEBUG=1  && ...141...
bash scripts/pr132_r2_manifests.sh r10ang
python3 scripts/pr132_angle_census.py --manifest98 em117-132r10ang98-manifest.tsv \
    --manifest141 em114c-132r10ang141-manifest.tsv --overlay-tag pi0scan-0829-agent \
    --tsv docs/pr/pr132-angle-census-r10ang.tsv
PR_JOBS=12 bash scripts/pr132_arms.sh 98 r10off 0  && ...141...   # gate vs r9off: PASS 4/4 expected
```

# 18. Owner r9 Bee verdicts, the typing census, and the next-steps recommendation (2026-08-30)

## 18.1 Owner verdicts on the 13 K19 fires (ON arm, verbatim-condensed)

| bee_idx | event | owner verdict | classification |
|---|---|---|---|
| 0 | 116962 | "incorrect vertex, NC pi0" | the K19-proposed point is ALSO wrong — confirms sec 16.3 by eye (the pair forms, the vertex is unidentifiable from pi0 observables) |
| 1 | 76346 | "incorrect vertex, NCpi0" | same; matches the measured truth distance 30.3 -> 33.7 cm |
| 2 | 506746 | "OK" | the 2.4 cm ADVERSE-at-zero move is visually tolerable |
| 3 | 486687 | "the 52 MeV gamma's vertex is not correct, the original is better" | ADVERSE confirmed |
| 4 | 348691 | "missing an EM shower @ (68.0, -182.1, 206.9)" | NOT missing: reco shower id 51080, **79.8 MeV, typed pdg=13 (muon)**, 5.4 cm from the point — mistyped, hence invisible to every pi0 pool |
| 5 | 406125 | "nu vertex strange for the proton and muon, original normal" | ADVERSE confirmed; also a signature-v3 false positive (a p+mu vertex passed the pure-shower gate — its muon is not in segments_in_long_muon) |
| 6 | 347824 | "looks better for the pi0" | a genuine K19 improvement (the round-6 over-extended-start event) |
| 7 | 166870 | "incorrect vertex" | ADVERSE confirmed |
| 8 | 409888 | "looks similar" | neutral |
| 9 | 281567 | "very busy" | no verdict |

Net: K19's OFF verdict stands (2 tolerable/better, 4 confirmed ADVERSE, the
2 NC rescues still have wrong vertices).  No flip.

## 18.2 The typing census (new, prompted by 348691)

For all 132 hand gammas on the current baseline arms: **127 matched-EM, 3
matched-but-MISTYPED, 2 id-changed, 0 truly absent-with-label-start.**  The
mistyped rows: 166870 g2 = 38.6 MeV typed **13**; 54341 g2 = 23.7 MeV typed
211; 71872 g2 = 23.0 MeV typed 211.  Plus the two owner specimens outside
the label set: 348691 (79.8 MeV typed 13) and 235435 (EM as hadronic,
round-8 scan).  The mu-typed class is the actionable one: pdg==13 shower
objects are HARD-excluded from every pi0 pool, and unlike the 211/A5 class
(which K7 readmits, production ON), no knob covers them.

## 18.3 Recommendation for the next steps

1. **K20 `pi0_admit_muon_showers` (recommended next round, in scope).**
   Admit shower-topology pdg-13 objects (not in a long muon) into the pi0
   pools, restamping EM on accept — the K7/K8 machinery and the owner's own
   round-2 direction ("the code can look at them, and update them to an EM
   shower in the pi0 reconstruction code") extended from 211 to 13.  Three
   concrete specimens (166870, 348691, 235435-adjacent); window-gated, so
   the expected cost is near zero.  DEFAULT OFF + census/ledger/movers as
   always.
2. **K16@120 owner call** (round 5, still open): +1 exact (54332) vs 3
   polluted gamma energies.
3. **NC pi0 vertex** — now confirmed by eye on both rescues: needs non-pi0
   information (vertex dQ/dx, track stubs, imaging); owner-level design.
   Any future NC gate must also veto track activity (the 406125 lesson).
4. **Upstream imaging** holds the rest: 105946's angle compression (not
   centroid-fixable), the UNDER-nosibs charge, the over-extended starts.
