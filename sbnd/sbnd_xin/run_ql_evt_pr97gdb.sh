...
process_event() {
    ...
    rm -f "$LOG"
    # doc pr/97 FORK (M10: run_ql_evt.sh stays byte-untouched).  Only change:
    # the Q/L wire-cell step runs under gdb -batch, so the ~4 %-per-run SIGSEGV
    # on SBND 18255-178410 (doc pr/97 sec.5) leaves a backtrace.  Kernel core
    # dumps are useless here -- /proc/sys/kernel/core_pattern is the bare
    # "core", so a 2.3 GB core lands in the dying process's CWD -- hence gdb.
    # Every signal the Go runtime inside gojsonnet uses is passed through
    # silently; only the fatal ones stop, and then we print the stack.
    gdb -q -batch \
        -ex "set confirm off" -ex "set pagination off" \
        -ex "set print pretty on" -ex "set print frame-arguments scalars" \
        -ex "handle all nostop noprint pass" \
        -ex "handle SIGSEGV SIGBUS SIGFPE SIGILL SIGABRT stop print nopass" \
        -ex run \
        -ex "echo \n==== PR97 GDB STOPPED ====\n" \
