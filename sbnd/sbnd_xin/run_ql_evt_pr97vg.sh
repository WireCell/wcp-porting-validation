...
process_event() {
    ...
    # the Q/L wire-cell step runs under valgrind memcheck, to enumerate every
    # invalid/uninitialised read in the clustering+matching chain on the event
    # that crashes with a 16-byte environment padding (SBND 18255-178410).
    valgrind --error-limit=no --track-origins=yes --num-callers=25 \
        --log-file="${VGLOG:-/home/xqian/tmp/pr97/vg_ql.log}" \
    wire-cell \
