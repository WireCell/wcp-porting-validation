...
# work-pr109e-{on,off}-nuecc48).  Forked rather than parameterised so the
...
for box in 3.0 2.25 3.75; do
  for ev in 6505 6528 6532 6650 6805 6806; do
    ...
    python3 $R --box-cm $box --max-junctions 3 --sigma-from OFF --anchors-from OFF --anchors-from ON \
      --arm ON=$S/pr109e_wct_on/${i}_${ev}/track_com_5384_${ev}.root:wct \
      --arm OFF=$S/pr109e_wct_off/${i}_${ev}/track_com_5384_${ev}.root:wct \
  ...
  for ev in 10550 46363 81597 360535 256587 433451; do
    python3 $R --box-cm $box --max-junctions 3 --sigma-from OFF --anchors-from OFF --anchors-from ON \
      --arm ON=$W/work-pr109e-on-nuecc48/pr_evt$ev/tracking-pr.root:wct \
      --arm OFF=$W/work-pr109e-off-nuecc48/pr_evt$ev/tracking-pr.root:wct \
