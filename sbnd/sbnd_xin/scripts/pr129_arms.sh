#!/bin/bash
# pr/129 round 1 arms (fork of pr128_arms.sh; prefix work-pr129r1-).  Usage: pr129_arms.sh <manifest 98|141> <armtag> <probes 0|1> [env assignments...]
...
for s in $SAMPLES; do
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr129r1-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr129_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
