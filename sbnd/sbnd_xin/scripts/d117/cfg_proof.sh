#!/bin/bash
# doc sbnd_xin/117: compiled-config proof of every study cell, two ways.
...
#
# Usage: bash scripts/d117/cfg_proof.sh [cells...] > docs/117_figs/117_cfg_proof.txt
set -u
...
CELLS=${*:-c1e c1x t1e t1x}
T=$(mktemp -d /home/xqian/tmp/d117_cfgproof.XXXX)
echo "# d117 cfg proof  toolkit=$(git -C $TK rev-parse --short HEAD)  wcp=$(git -C $SX rev-parse --short HEAD)  $(date +%F_%H:%M)"
...
cell_tla() {  # cell -> the --tla-code / --tla-str list, as stageB_cell.sh exports them
  local c=$1 f=$SX/docs/117_figs/tla/$1.tla j=$SX/docs/117_figs/tla/$1.tfjson
...
for s, base, pat in (('cv', 'work-r3cv-d115pr/f000', 'work-r3cv-d117%s/f*'),
                     ('nuecc', 'work-r3nue-d115pr/f000', 'work-r3nue-d117%s/f*'),
                     ('off', 'work-r3off-d115pr', 'work-r3off-d117%s')):
