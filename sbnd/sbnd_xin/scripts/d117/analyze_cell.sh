#!/bin/bash
# doc sbnd_xin/117: the doc-115 analysis stage, run on ONE study cell's stage-B arm.
...
#   - every output goes under a per-cell name: products/d117/<s>-<cell>/, docs/117_sel/<s>-<cell>/,
#     docs/117_sel_edep100/<s>-<cell>/, docs/117_{vtx,scan,time}/ with --label <s>-<cell>,
#     docs/117_off/<cell>/;
...
J=${JOBS:-24}
P=$SX/products/d117/$S-$CELL
P115=$SX/products/d115/$S
case "$S" in
    cv)    ARM=$SX/work-r3cv-d117$CELL;  MC=1 ;;
    nuecc) ARM=$SX/work-r3nue-d117$CELL; MC=1 ;;
    off)   ARM=$SX/work-r3off-d117$CELL; MC=0 ;;
    *) echo "unknown sample: $S" >&2; exit 2 ;;
esac
[ -d "$ARM" ] || { echo "ERROR: no stage-B arm $ARM" >&2; exit 1; }
[ "$(cat "$ARM/.d117_cell" 2>/dev/null)" = "$CELL" ] || { echo "ERROR: $ARM is not a '$CELL' arm" >&2; exit 1; }
# Completeness guard (added mid-round): the doc-115/116 drivers had none, and analysing an arm
# while stage B is still writing it silently produces a SHORT table -- it happened here on
# off-c1e (504 of 1000 events) and was caught only by reading the log.  FORCE=1 to override.
case "$S" in cv) A=$SX/work-r3cv-d115 ;; nuecc) A=$SX/work-r3nue-d115 ;; off) A=$SX/work-r3off-d115 ;; esac
NQL=$(ls -d "$A"/f*/ql_evt* "$A"/ql_evt* 2>/dev/null | wc -l)
NPR=$(ls -d "$ARM"/f*/pr_evt* "$ARM"/pr_evt* 2>/dev/null | wc -l)
NRC=$(grep -ls 'rc=0' "$ARM"/f*/pr_evt*/rc.txt "$ARM"/pr_evt*/rc.txt 2>/dev/null | wc -l)
if [ "${FORCE:-0}" != 1 ] && { [ "$NPR" -ne "$NQL" ] || [ "$NRC" -ne "$NQL" ]; }; then
    echo "REFUSING: $ARM is not complete -- stage A has $NQL events, this arm has $NPR pr_evt dirs and $NRC with rc=0." >&2
    echo "          Wait for stage B, or re-run with FORCE=1 if a short table is really what you want." >&2
    exit 3
fi
mkdir -p "$P"
rc_all=0
...
if [ "$MC" = 1 ]; then
    ...
    python3 d107_selection.py "$P" "$SX/docs/117_sel/$L" || rc_all=1
    step "doc 107 sec 5.6 selection tables (Edep > 100 MeV, nue cut also at 4)"
    python3 d107_selection.py "$P" "$SX/docs/117_sel_edep100/$L" \
        --edep-min 100 --cuts numu:0.9,nue:7.0,nue:4.0 || rc_all=1

    step "vertex assessment"
    python3 d115_vertex.py "$P" "$SX/docs/117_vtx" --label "$L" || rc_all=1
    python3 d115_vertex.py "$P" "$SX/docs/117_vtx" --label "${L}_edep100" --edep-min 100 || rc_all=1

    step "score scan"
    python3 d115_score_scan.py "$P" "$SX/docs/117_scan" --label "$L" || rc_all=1

    step "time-association check (d115 V11)"
    python3 d115_time_assoc.py "$P" "$SX/docs/117_time" --label "$L" || rc_all=1
else
    ...
    python3 d115_beamoff_rate.py "$P" "$SX/docs/117_off/$CELL" || rc_all=1
    step "score scan (rate vs cut)"
    python3 d115_score_scan.py "$P" "$SX/docs/117_scan" --label "$L" \
