#!/usr/bin/env python3
"""doc 94 fork of make_stmfb_bee.py -- adds the save_stm_fit trajectory layer.

Identical to the doc-93 packer except that 'stm_fit-global' is carried through
from the PR zip.  Requires the PR arm to have been run with save_stm_fit=true
(scripts/doc94_stmfit_instrument.sh); without it the layer is simply absent and
the packer reports it as missing, same as any other optional layer.

Original doc-93 docstring follows.
...
# doc 94: 'stm_fit-global' is the save_stm_fit trajectory layer -- the fitted
# STM path with its per-point dQ/dx, i.e. the exact evidence the STM verdict
# rests on.  It is deliberately NOT in PR_POINT_LAYERS: the id remap keys on
# per-point CHARGE against clustering-global, and an stm_fit point carries the
# fit's dQ, not a blob charge, so remapping it would mis-assign.  Its
# cluster_id is the PR-side ident, which is also what the tagger log lines and
# nusel-evt<ID>.tsv main_id use -- so it joins to the analysis, just not to the
# recoloured img ids.
STM_FIT_LAYER = 'stm_fit-global'
PR_LAYERS = PR_POINT_LAYERS + ('mc', STM_FIT_LAYER)
