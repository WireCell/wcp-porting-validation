#!/usr/bin/env python3
"""doc 94 round 3 -- census of the ENTRY-RISE feature (shoulder AND kink).

Round-3 fork of doc94r2_entry_census.py: the DEBUG probe line now also carries
the owner's KINK -- the largest direction change between two 5 cm chords
meeting at a fit point, over the muon segment with the last 15 cm before the
stop excluded.  Round 2's script cannot parse the new line, and it is the
record of round 2, so this is a fork rather than an edit.
...
ap.add_argument('--maxcut', type=float, default=60.0)
ap.add_argument('--kink', type=float, default=22.0, help='kink bar; the AND with the shoulder')
ap.add_argument('--out')
args = ap.parse_args()
        ...
        for line in open(log, errors='replace'):
            ...
            if m:
                seen[m.group(1)] = dict(
                    ...
                    kink=float(m.group(11)), kink_L=float(m.group(12)))
                continue
...
def hist(rs, title, field='shoulder'):
    ...
    n_win = sum(1 for r in rs if args.cut <= r[field] <= args.maxcut)
    n_fire = sum(1 for r in rs if args.cut <= r[field] <= args.maxcut and r['kink'] >= args.kink)
    print(f'    IN WINDOW [{args.cut},{args.maxcut}]        : {n_win} of {len(rs)}  ({100.0*n_win/len(rs):.2f}%)')
    print(f'    ... AND kink >= {args.kink} deg (FIRES) : {n_fire} of {len(rs)}  ({100.0*n_fire/len(rs):.2f}%)')
...
win = sorted([r for r in stm if args.cut <= r['shoulder'] <= args.maxcut],
             key=lambda r: -r['shoulder'])
print(f'\n=== STM=1 bundles inside the shoulder window: {len(win)} '
      f'(of which {sum(1 for r in win if r["kink"] >= args.kink)} also pass the kink and are RELEASED) ===')
for r in win:
    tag = 'RELEASE' if r['kink'] >= args.kink else 'held by kink'
    print(f"  {r['arm'][5:]:<14s} evt{r['evt']:>7s}:{r['cid']:<4s} run {r['run']}-{r['subrun']} "
          ...
          f"excess={r['excess']:.1f} kink={r['kink']:.1f}deg@{r['kink_L']:.0f}cm  {tag}")
...
if args.baseline:
    ...
    for cls in ('code-STM-correct', 'code-FALSE-STM', 'code-MISSED-STM', 'code-not-STM-correct'):
        ...
        if not vals:
            continue
        vals.sort()
        over = [v for v in vals if args.cut <= v[0] <= args.maxcut and v[3]['kink'] >= args.kink]
        print(f'    shoulder range [{vals[0][0]:.1f}, {vals[-1][0]:.1f}] cm   '
              f'WOULD FIRE: {len(over)} of {res}')
        for v, key, ln, p in vals:
            mark = '  <-- WOULD FIRE' if (args.cut <= v <= args.maxcut and p['kink'] >= args.kink) else ''
            print(f"      {key[0]}:{key[1]:>3s}  shoulder={v:5.1f}cm kink={p['kink']:5.1f}deg "
                  f"rise={p['rise']:.2f} body={p['body']:.2f} len={ln:.1f}cm{mark}")

if args.out:
    keys = ['arm', 'evt', 'cid', 'run', 'subrun', 'stm', 'tgm', 'fc', 'label', 'length',
            'L_stop', 'body', 'body_n', 'ent', 'ent_n', 'rise', 'shoulder', 'sh_nofirst',
            'excess', 'kink', 'kink_L', 'rejected']
