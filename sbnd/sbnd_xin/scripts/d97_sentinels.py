#!/usr/bin/env python3
# doc 97 -- FORK of scripts/pr127_sentinels.py (CLAUDE.md M10: the production
# script stays byte-untouched; it is live for every other round).
#
# One difference, at the PF-tree reader.  Under sep_fv_point, mcp2k 18255-x-105074
# -- itself a sentinel event -- loses its neutrino candidate entirely
# (nu-candidate -> STM), so its mabc-pr.zip carries no `mc` layer at all and the
# production script dies with
#     KeyError: "There is no item named 'data/0/0-mc.json' in the archive"
# before reporting ANY verdict for that arm.  A crash is the worst possible
# outcome for a tripwire: it hides the other 30 sentinels.  Here a missing PF
# tree is reported as an empty node list, which turns into an ordinary FAIL on
# any pf_* assertion and lets the rest of the suite run.
...
def pf_texts(arm, event):
    ...
    with zipfile.ZipFile(z) as zf:
        try:
            payload = zf.read("data/0/0-mc.json")
        except KeyError:
            # No PF tree: TaggerCheckNeutrino selected no candidate this event.
            # Report it as "no PF nodes" so pf_* assertions FAIL loudly instead
            # of the whole suite dying (doc 97; see the header).
            return out
        for root in json.loads(payload):
            walk(root)
    return out
