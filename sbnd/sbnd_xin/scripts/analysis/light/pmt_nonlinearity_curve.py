...
# --------------------------------------------------------------------------------------
# Simple functional form for QLMatching: log-quadratic capped power law
# --------------------------------------------------------------------------------------
# The realized per-PMT curve is described by a 2-parameter monotone form, flat below a
# knee (default 700 PE) so the correction is exactly 1 in the un-saturated regime:
#
#     observed(x) = x                                      for x <= knee
#                 = knee * exp( beta*L + gamma*L^2 ),  L = ln(x/knee),   for x > knee
#
# beta=1, gamma=0 reproduces the identity (no correction).  This is what QLMatching
# evaluates per PMT on its predicted PE.
KNEE_DEFAULT = 700.0


def fit_logquad(n_true, curve, knee=KNEE_DEFAULT):
    """Least-squares (beta, gamma) for the log-quadratic power law above `knee`."""
    a = n_true > knee
    if a.sum() < 2:
        return 1.0, 0.0
    L = np.log(n_true[a] / knee)
    ly = np.log(np.maximum(curve[a], 1e-9) / knee)
    A = np.vstack([L, L * L]).T
    (beta, gamma), *_ = np.linalg.lstsq(A, ly, rcond=None)
    return float(beta), float(gamma)


def eval_logquad(x, beta, gamma, knee=KNEE_DEFAULT):
    """Evaluate the log-quadratic capped power law (mirrors the QLMatching C++)."""
    x = np.asarray(x, dtype=float)
    out = x.copy()
    a = x > knee
    L = np.log(x[a] / knee)
    out[a] = knee * np.exp(beta * L + gamma * L * L)
    return out


def fit_all_pmts(params, knee=KNEE_DEFAULT, nmax=100000, npts=70, ntrials=30,
                 nopdets=312, range_lo=100, seed=7,
                 f_fast=0.25, tau_fast=6.0, tau_slow=1500.0):
    """Sweep the realized curve to `nmax` per PMT and fit (beta, gamma).

    Returns (n_true, curves[active], fits, beta_arr, gamma_arr, active_idx) where
    beta_arr/gamma_arr are length-`nopdets` arrays (identity 1.0/0.0 for non-PMT or
    nonlinearity-off channels), indexed by opch == QLMatching opdet index.
    """
    pesat, alpha = params[:, 1], params[:, 2]
    rhi = params[:, 3].astype(int) if params.shape[1] > 3 else np.full(len(params), 7000)
    opch = params[:, 0].astype(int)
    active = (pesat > 0) & (alpha > 0)

    n_true = np.unique(np.round(np.geomspace(1, nmax, npts)).astype(int))
    models = [PMTNonLinearity(p0, p1, range_lo, int(rh))
              for p0, p1, rh in zip(pesat[active], alpha[active], rhi[active])]
    curves = sweep_scintillation_multi(models, n_true, ntrials=ntrials, seed=seed,
                                       f_fast=f_fast, tau_fast=tau_fast, tau_slow=tau_slow)

    beta_arr = np.ones(nopdets, dtype=float)
    gamma_arr = np.zeros(nopdets, dtype=float)
    fits = []
    act_opch = opch[active]
    for k in range(len(models)):
        b, g = fit_logquad(n_true, curves[k], knee)
        fits.append((b, g))
        ch = int(act_opch[k])
        if 0 <= ch < nopdets:
            beta_arr[ch], gamma_arr[ch] = b, g
    return n_true, curves, fits, beta_arr, gamma_arr, np.where(active)[0]


def emit_qlmatching_jsonnet(beta_arr, gamma_arr, knee, outpath, profile):
    """Write the per-PMT (beta, gamma) arrays + knee as a jsonnet QLMatching can import."""
    def arr(a):
        return "[" + ", ".join(f"{v:.6g}" for v in a) + "]"
    body = (
        "// SBND PMT non-linearity parameterization for QLMatching predicted PE.\n"
        "// GENERATED by sbnd_xin/pmt_nonlinearity_curve.py --emit-qlmatching -- do not edit by hand.\n"
        "// Realized (scintillation) NPE_true -> NPE_reco per PMT, fitted to the monotone\n"
        "// log-quadratic capped power law:  observed(x) = x for x<=knee, else\n"
        "//   knee * exp(beta*L + gamma*L^2),  L = ln(x/knee).  beta=1,gamma=0 => identity.\n"
        f"// scintillation profile (assumed): f_fast={profile[0]}, tau_fast={profile[1]} ns, "
        f"tau_slow={profile[2]} ns.\n"
        "{\n"
        f"  pmt_nl_knee: {knee:g},\n"
        f"  pmt_nl_beta: {arr(beta_arr)},\n"
        f"  pmt_nl_gamma: {arr(gamma_arr)},\n"
        "}\n"
    )
    with open(outpath, "w") as f:
        f.write(body)
    nact = int(np.sum((beta_arr != 1.0) | (gamma_arr != 0.0)))
    print(f"[ok] wrote {outpath}  ({len(beta_arr)} opdets, {nact} with a correction)")


def fit_validation_plot(n_true, curves, fits, active_pesat, knee, outpath):
    """Demonstrate the parameterization: fit-vs-MC overlay + all-PMT residuals."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    from matplotlib import cm
    ...
    # left: overlay for the lowest / median / highest PESat channel
    order = np.argsort(active_pesat)
    picks = {"lowest PESat": order[0], "median PESat": order[len(order) // 2],
             "highest PESat": order[-1]}
    for (lab, k), c in zip(picks.items(), ["C0", "C1", "C2"]):
        b, g = fits[k]
        ax[0].plot(n_true, curves[k], c + "o", ms=3, alpha=0.5,
                   label=f"{lab}={active_pesat[k]:.0f} (MC)")
        ax[0].plot(n_true, eval_logquad(n_true, b, g, knee), c + "-", lw=1.5,
                   label=f"  fit β={b:.3f} γ={g:.4f}")
    ax[0].plot(n_true, n_true, "k:", lw=1, label="linear")
    ax[0].axvline(knee, color="0.6", ls="--", lw=1)
    ax[0].set_xscale("log"); ax[0].set_yscale("log")
    ax[0].set_xlabel("NPE_true"); ax[0].set_ylabel("NPE_reco")
    ax[0].set_title("fit vs MC realized curve"); ax[0].legend(fontsize=7.5); ax[0].grid(alpha=0.3)

    # right: relative residual (fit/MC - 1) for ALL channels, colored by PESat
    norm = Normalize(active_pesat.min(), active_pesat.max())
    for k in range(len(curves)):
        b, g = fits[k]
        fit = eval_logquad(n_true, b, g, knee)
        a = n_true > knee
        resid = fit[a] / np.maximum(curves[k][a], 1e-9) - 1.0
        ax[1].plot(n_true[a], 100 * resid, color=cm.viridis(norm(active_pesat[k])),
                   lw=0.6, alpha=0.6)
    ax[1].axhline(0, color="k", ls=":", lw=1)
    for yv in (-5, 5):
        ax[1].axhline(yv, color="0.7", ls="--", lw=0.8)
    ax[1].set_xscale("log")
    ax[1].set_xlabel("NPE_true"); ax[1].set_ylabel("fit / MC − 1  [%]")
    ax[1].set_title(f"parameterization residual, all {len(curves)} PMTs")
    ax[1].set_ylim(-15, 15); ax[1].grid(alpha=0.3)
    sm = cm.ScalarMappable(norm=norm, cmap="viridis"); sm.set_array([])
    fig.colorbar(sm, ax=ax[1], label="PESat (TF1 p0)")
    fig.suptitle("SBND PMT non-linearity: log-quadratic power-law parameterization")
...
def main():
    ...
    ap.add_argument("--emit-qlmatching", action="store_true",
                    help="fit the realized curve per PMT and emit the QLMatching jsonnet "
                         "param file (beta/gamma arrays + knee) + a validation plot")
    ap.add_argument("--knee", type=float, default=KNEE_DEFAULT,
                    help="PE below which the correction is exactly 1 (default 700)")
    ap.add_argument("--params-out", default="pmt_nonlinearity_params.jsonnet",
                    help="output path for the generated QLMatching jsonnet param file "
                         "(pass the toolkit cfg/.../sbnd path to install it)")
    args = ap.parse_args()

    import os

    # --- emit QLMatching parameterization ---------------------------------------------
    if args.emit_qlmatching:
        if not args.params_csv:
            raise SystemExit("--emit-qlmatching requires --params-csv (the real per-PMT CSV)")
        nmax = max(args.nmax, 100000)
        params = load_params_csv(args.params_csv)
        print(f"[..] fitting realized curve to NPE_true={nmax} for "
              f"{int(((params[:,1]>0)&(params[:,2]>0)).sum())} active PMTs (knee={args.knee:g}) ...")
        n_true, curves, fits, beta_arr, gamma_arr, act = fit_all_pmts(
            params, knee=args.knee, nmax=nmax, ntrials=args.ntrials,
            f_fast=args.f_fast, tau_fast=args.tau_fast, tau_slow=args.tau_slow)
        emit_qlmatching_jsonnet(beta_arr, gamma_arr, args.knee, args.params_out,
                                (args.f_fast, args.tau_fast, args.tau_slow))
        os.makedirs(args.pics_dir, exist_ok=True)
        fit_validation_plot(n_true, curves, fits, params[act, 1], args.knee,
                            os.path.join(args.pics_dir, "pmt_nonlinearity_fit.png"))
        return
