#!/bin/bash
# pr/134 arms (fork of pr133_arms.sh: work-pr134- prefix, same manifests/probe
# env -- pr133_arms.sh stays byte-untouched, M10).
...
for s in $SAMPLES; do
  [ -z "${EV[$s]}" ] && continue
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr134-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr134_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
