...
# sweep/dirweakon_ub.
set -eu
# doc pr/111: fork of qlport/scripts/run_one.sh, ONE line changed (adds
# -L clus.NeutrinoPattern:debug, without which every DL vertex decision is
# invisible -- -L clus:debug alone does NOT reach the sub-logger).  Lives under
# sbnd_xin/scripts/, so the roots are named explicitly instead of derived from
# $0's directory.  run_one.sh itself is NOT touched.
SCRIPTS=/nfs/data/1/xqian/toolkit-dev/toolkit/qlport/scripts
QLPORT=/nfs/data/1/xqian/toolkit-dev/toolkit/qlport
ABTEST=/nfs/data/1/xqian/toolkit-dev/wcp-porting-img/abtest
...
python3 "$ABTEST/timecmd.py" meta.txt \
    $NOASLR wire-cell -l stderr -l "wct_${RUN}_${EV}.log:debug" -L clus:debug -L clus.NeutrinoPattern:debug \
