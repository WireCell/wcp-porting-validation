#!/bin/bash
# pr/128 round 1 arms (fork of pr125_arms.sh: the hard-coded work-pr125r1- prefix is now work-pr128r1-, the trap that made a pr/127 gate compare zero archives while printing PASS).  Usage: pr128_arms.sh <manifest 98|141> <armtag> <probes 0|1> [env assignments...]
...
for s in $SAMPLES; do
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr128r1-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr128_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
