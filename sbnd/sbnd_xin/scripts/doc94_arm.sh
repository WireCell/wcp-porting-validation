#!/bin/bash
# doc 94 -- run one PR-tail arm with a descent_guard setting.
...
export LD_LIBRARY_PATH=$HOME/tmp/doc94-libsnap:${LD_LIBRARY_PATH:-}
...
{ echo "stm_descent_guard=true"; echo "stm_descent_cos_y=$COSY"; for x in "$@"; do echo "$x"; done; } > "$TLA"
