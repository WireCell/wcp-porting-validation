#!/bin/bash
# pr/130 arms (fork of pr128_arms.sh: prefix change only -- the membership-probe arm the q_miss refresh needs; pr128_arms.sh stays byte-untouched, M10).
...
for s in $SAMPLES; do
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr130r1-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr130_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
