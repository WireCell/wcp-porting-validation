#!/bin/bash
# pr/132 arms (fork of pr131_arms.sh: prefix + probe env + subset mode --
# pr131_arms.sh stays byte-untouched, M10).
...
for s in $SAMPLES; do
  [ -z "${EV[$s]}" ] && continue
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr132-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr132_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
