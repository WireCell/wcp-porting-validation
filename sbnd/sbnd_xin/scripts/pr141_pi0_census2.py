#!/usr/bin/env python3
"""doc pr/141 session 2, item 1 -- the pi0 census with a LABEL PRECEDENCE CHAIN.

A fork of scripts/pr132_pi0_census.py (M10: fork, do not refactor the production
scorer -- every census this campaign quotes came out of that file and it must
stay byte-identical).  The ONLY change is label resolution.

pr132 resolves BASE-WINS with a single `--overlay-tag` that may only EXTEND the
denominator.  That is correct for the pr/132 model pairing pass and wrong for the
owner's 2026-08-31 rescan, which CORRECTS earlier calls -- and naming the rescan
in the single overlay slot DISPLACES pi0scan-0829-agent, taking the denominator
from 66 to 52 (doc pr/141 sec 13.1).  So the two totals were never comparable.

This fork takes `--chain tag1,tag2,...`, highest precedence FIRST, and resolves
**per field**: a tag wins an event only where it actually carries a populated
`pio.gammas`.  That matters -- em_labels/pi0mass-0904-owner/labels-evt397630.json
exists but its pio block is EMPTY (doc pr/141 sec 2.3, the seeding defect), so a
file-level "newest wins" would silently delete a census row.

Owner decision on record (2026-08-31): *"newest scan wins"*.  Two readings are
priced, and --chain makes both explicit:

  A. owner-on-top   pi0mass-0904-owner, pi0scan-0829-agent, <base>
  B. uniform-newest pi0mass-0904-owner, pi0scan-0829-agent, <base>   [same order]

which are the same chain here, because pi0scan-0829-agent (2026-08-29) is already
newer than both base tags.  What A and B would differ on is only the base pair,
which carry no overlapping paired events (verified: 0 events paired in both).

DELIBERATELY EXCLUDED from the chain, though newer by mtime:
  em_labels/pi0mu-0905-owner   -- object-TYPE verdicts (gamma vs track)
  em_labels/pisplit-0905-owner -- SPLIT/KEEP verdicts
Their `pio` blocks are agent seeds carried along by the viewer, not pairing
verdicts, and mtime alone would have promoted them over the owner's own rescan.
...
def main():
    ...
    ap.add_argument("--chain", default="",
                    help="comma-separated label tags, HIGHEST precedence first; "
                         "resolved per-field on pio.gammas (see the module docstring)")
    ...
    chain = [t.strip() for t in a.chain.split(",") if t.strip()]
    chain_labels = [(t, SEL.load_labels(t)) for t in chain]
    if chain:
        print("label precedence chain (highest first): %s -> <base per set>"
              % " -> ".join(chain))
    ...
    for (setname, tag, m_scan, p_scan, m_cur, p_cur, buck) in SEL.SETS:
        ...
        for ev, mrow in sorted(man.items()):
            ...
            # ---- hand-pi0 events: per-field newest-wins over the chain -----
            rec, src = None, None
            for tname, tlab in chain_labels:
                rec = hand_pair(tlab.get(ev))
                if rec is not None:
                    src = tname
                    break
            if rec is None:
