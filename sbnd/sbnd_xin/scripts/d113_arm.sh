#!/bin/bash
# doc sbnd_xin/113 -- one PR (stage-B) arm on a doc-113 stage-A arm (work-<s>-<STAGEA>, default the
# production d102m).  Output work-<sample>-d113<TAG>.
#
# Fork by duplication (CLAUDE.md M10) of scripts/pr150_arm.sh (that script stays untouched); the one
# functional difference is the STAGEA env (stage-A arm suffix, e.g. STAGEA=d113on).
...
#   STAGEA     stage-A arm suffix to read (default d102m; the fix arm uses d113on)
#
...
if [ "${STM_FIT:-0}" = 1 ]; then
  ...
  TLA_TMP=$(mktemp /home/xqian/tmp/d113/tla_${TAG}.XXXX)
...
echo "=== d113 arm $TAG  samples=[$SAMPLES] jobs=$JOBS pin=$PIN stm_fit=${STM_FIT:-0}  $(date +%F_%H:%M:%S)"
...
for s in $SAMPLES; do
  ...
  PR_JOBS=$JOBS ./run_pr_chain_batch.sh "work-$s-${STAGEA:-d102m}" "$OUT" data "${EVTS[@]}"
  rc=$?
  [ $rc = 0 ] || rc_all=1
...
md5sum "$PIN/libWireCellClus.so"
echo "=== d113 arm $TAG DONE rc_all=$rc_all $(date +%F_%H:%M:%S)"
exit $rc_all
