#!/usr/bin/env python3
"""Retirement round 2026-08-31b -- the owner's second pass: release the
hand-scan SCAN-TIME dumps and the now-superseded pi0 reference points.
91 G -> ~73 G.

Fork of plan_20260831.py (same day, second pass).  The first pass KEPT
work-*-prod0825 because ASSERT 11 found the base hand-scan manifests
resolving into it.  The owner then asked the direct question -- "Do we need
0825prod?" -- and the answer was traced through the CODE rather than through
that assert: the three scripts that produce the metric (pr132_pi0_census.py,
pr132_gamma_ledger.py, pr135_pi0_peak_prod.py) unpack m_scan from SEL.SETS
but NEVER load it; so do pr126_pi0_census.py, pr126_pi0_mass.py and
pr132_angle_census.py.  Every one reads m_cur only.  The scan-time dumps are
therefore not an input to any current or next-round measurement; they are the
...
WHAT IS GIVEN UP, STATED PLAINLY: pr126-pi0-mass.tsv can no longer be
rebuilt, and "what did the scanner see for event N" is answerable only from
the label JSON.  The fit those dumps supported has been superseded TWICE
(0.80 -> 0.84 -> 0.86) and the 0.86 measurement used the fixed-pairing
estimator on the CURRENT arm, which never touches scan-time energies.

ALSO RELEASED: the two ends of the pi0 A/B (work-pr133-flipchk-*,
work-pr134-flip2-*), superseded as production by work-pr134-f086-* at 0.86;
the 50-pi0 manifest dump source (work-pr124r1-onA*); and the doc pr/126
sec 4f census arms (work-pr125r1-flipchk*).  The sentinel negative control
(work-pr125r1-flipK5*) is NOT released -- it is a live registry input.

THE SILENT-SKIP TRAP THIS ROUND CLOSES.  SEL.load_json() returns None for a
missing dump and every caller `continue`s on None, so a manifest pointing at
a released arm would have produced a zero-row census that looks like a clean
run.  scripts/pr126_pi0_select.py:load_manifest() now RAISES when a
manifest's dump arm is gone, naming the arm and the archive.
...
Three constraints, each shaping a different part of KEEP:
  ...
  1. "leave the latest production" -> work-*-prod0830 (all four samples,
     ...
     becomes the new metric baseline.  prod0825 is RELEASED in this second
     pass, on the consumer trace described above rather than on a
...
STATE = os.environ.get("RETIRE_STATE", os.path.join(SCR, "state-20260831b"))
...
KEEP_WHY = {
    ...
    # --- prod0825 is RELEASED this pass (see the header) ------------------
    # The first pass kept it on ASSERT 11's word.  The owner asked whether it
    # is really needed; tracing the consumers showed every metric script reads
    # m_cur and none loads the scan-time manifest, so it is released here and
    # the four manifests that resolved into it leave LIVE_MANIFESTS below --
    # they become historical records, and load_manifest() now RAISES rather
    # than letting a caller skip their missing dumps silently.
...
# (retire_20260831b.sh interlock 6) -- plan-time evidence expires.
a12_fail = 0
...
# ---------------------------------------------------------------- emit
with open(os.path.join(SCR, "tierA_20260831b.txt"), "w") as fh:
    fh.write("\n".join(sorted(TA)) + "\n")
...
print(f"\nremoval set: {len(R)} dirs -> scripts/retire/tierA_20260831b.txt")
