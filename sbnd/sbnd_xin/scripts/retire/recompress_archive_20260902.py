#!/usr/bin/env python3
"""doc 98 round 2 -- re-encode the NEW record layer from gzip to zstd-19.

Fork of recompress_archive_20260901.py by verbatim `cp` (cmp-verified before
editing); only the state dir moved to state-20260902 (M13: never write into an
earlier round's state).  Everything else -- the member-for-member verification,
the SKIP list, the deliberate avoidance of --long=31 -- is unchanged and still
correct.

WHY RE-RUN IT.  doc 98 wrote 62 fresh record tarballs as gzip
(archive_records_20260902.py inherits `tarfile.open(tgz, "w:gz")`), 2.08 GiB in
the 52 files at or above 1 MB.  Those members are per-event logs and compiled
configs -- ~3000 near-identical copies of the same text per arm -- which is
exactly the shape gzip's 32 KiB window cannot exploit.  MEASURED on this
round's own largest tarball, not projected:

    work-mcp2k-d97onpr.tar.gz
    141.5 MB .gz  ->  846.1 MB raw  ->  8.0 MB .zst   (17.6x smaller than gz)

Original doc 89 header follows.
...
ROOT = "/home/xqian/toolkit-dev/wcp-porting-img/sbnd/sbnd_xin"
