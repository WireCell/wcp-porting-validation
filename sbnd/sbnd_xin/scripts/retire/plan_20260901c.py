...
* The tier file name is tierA_20260901c.txt.  The 09-01 header warns that this
...
STATE = os.environ.get("RETIRE_STATE", os.path.join(SCR, "state-20260901c"))
...
KEEP_PREFIX = {
    ...
    # RELEASED in round C.  work-sent130neg* dated 2026-08-29 and was the
    # negative-control layer at THAT operating point -- before the 0.86 EM flip,
    # doc 77 r3/r4's 17 retired TLAs and the master merge.  doc 91 sec 12
    # replaced it with work-91neg-*, measured at the pinned prod0901b point
    # against the RE-BASELINED thresholds.  Keeping a control that brackets a
    # threshold nobody uses any more is how the registry went stale in the
    # first place.
    'work-91neg': 'doc 91 sec 12 -- the CURRENT-point negative controls; the proof the re-baselined suite can fail',
}
...
KEEP_WHY = {
    ...
    # --- RELEASED in round C: the pr125r1-flipK5* worked-failing case, 6 -----
    # These were kept because pr127_sentinels.py:31 names them as the example
    # that produces a FAIL, and because 137238's only passing arm lived here.
    # Both grounds are gone: doc 91 sec 12 re-baselined 137238 (the assertion
    # DIRECTION flipped, so "passing" now means the opposite of what it did),
    # and work-91neg-scccgap-nuecc48 is its measured negative control at the
    # current operating point.  ASSERT 16's falsifiability check re-derives that
    # by RUNNING the suite, so this release cannot silently leave the threshold
    # unfalsifiable.
...
ACK_BROKEN_REFS = {
    ...
    # --- added 2026-09-01c (doc 91 sec 12/13).  RECORD-SCRIPT RULE as always:
    # scripts/pr130_sentinel_neg34.sh is the script that PRODUCED these two
    # negative controls on 2026-08-29, and it keeps naming the arms it made.
    # Its successor for the current operating point is
    # scripts/doc91_rebaseline_negarms.sh, which produces work-91neg-* against
    # the RE-BASELINED thresholds; both scripts stay, each naming its own round.
    'work-sent130neg3-mcp2k': 'scripts/pr130_sentinel_neg34.sh:15 -- the script that BUILT it; superseded by work-91neg-gfimpact-mcp2k',
    'work-sent130neg4-mcp2k': 'scripts/pr130_sentinel_neg34.sh:18 -- same; superseded by work-91neg-prune2-mcp2k',
...
# Both checks live in scripts/retire/sentinel_guard_20260901c.py so that this
...
import sentinel_guard_20260901c as sg                       # noqa: E402
try:
    ok16 = sg.report_c(dirs, set(K))
...
# ---------------------------------------------------------------- emit
with open(os.path.join(SCR, "tierA_20260901c.txt"), "w") as fh:
    fh.write("\n".join(sorted(TA)) + "\n")
...
print(f"\nremoval set: {len(R)} dirs -> scripts/retire/tierA_20260901c.txt")
