...
#
# Fork of retire_20260831.sh.  Every interlock is carried verbatim, including
...
#
#   ./retire_20260831.sh A              # dry run (default action)
#   CONFIRM=yes ./retire_20260831.sh A  # actually delete
#
# Pre-flights (all must have passed before CONFIRM=yes):
#   scripts/retire/plan_20260831.py                (13 asserts, "OVERALL: PASS")
#   scripts/retire/archive_records_20260831.py     (integrity gate PASS n/n)
set -u
...
STATE=$BASE/scripts/retire/state-20260831
...
tierfiles=""
for t in ${TIERS//,/ }; do
    f=$BASE/scripts/retire/tier${t}_20260831.txt
...
# NOT `[ -s ]`.  archive_records_20260831.py DROPS 4.96 GiB of
...
if [ ! -f "$GD" ]; then
    echo "!! $GD MISSING -- run verify_group_dupes_20260831.py before deleting"; exit 2
fi
...
# The owner's stated next move reads these.  plan_20260831.py ASSERT 11 checked
...
if [ -n "$fresh" ]; then
    ...
    echo "refusing -- re-run plan_20260831.py (RETIRE_REPLAN=1) and re-read the list."
    exit 2
fi
...
if [ "$CONFIRM" = yes ]; then
    {
      echo "# retire_20260831.sh  tiers=$TIERS"
