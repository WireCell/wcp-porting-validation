#!/usr/bin/env python3
"""Retirement round 2026-08-29 -- release eleven CLOSED doc families and keep
the pi0 epoch.  152 G -> ~72 G, 442 arms, ~83 G gross.

Fork of plan_20260825b.py.  Asserts 1-10 carried; asserts 11 and 12 of that
round are RETIRED (they guarded a stage-A freeze this round does not do) and
THREE new asserts take their numbers and one more: 11 manifest resolution,
12 concurrent-writer safety, 13 the peer session's declared dependencies.
KEEP grows 26 -> 110, because for the first time the tree holds an OPEN round
owned by a *different session* and a named next move with on-disk inputs.

Owner scope, asked and given directly (2026-08-29):

    "the sbnd_xin directory now grow to 152 G, we should plan for a clean up
     round again before going to the next step.  We can keep the latest PR
     results. ... Note, our next major move is to improve the pi0
     reconstruction.  We need to use the hand scan results for pi0 to do this.
     ... we also do not want to delete the files related to the other running
     session."

Three constraints, and each one shapes a different part of KEEP:

  1. "keep the latest PR results" -> the doc 81 production pair
     (work-<s>-{grp0825,prod0825}, 8 arms, 33 G) and the vtx105 label epoch
     stay, as in every round since 08-25.  FLAGGED, not acted on: prod0825 is
     now SIX flips behind production (pr/123 pass4, pr/124 gap-band, pr/125
     pass3+samevtx, pr/127 K5+sccc, pr/128 PF-kine, pr/129 pointing all landed
     after it).  A fresh full-coverage campaign at today's HEAD would supersede
     it; that is a decision for the next round, not an inference for this one.
  2. "our next major move is pi0 ... using the hand scan results" -> the pi0
     epoch is carved OUT of two families that are otherwise the fattest sweep
     candidates.  See PROTECTED.txt block 1 and ASSERT 11.
  3. "do not delete the files related to the other running session" -> the
     whole work-pr130*/pr129*/sent130* prefix, by the peer's own verified
     dependency list.  See ASSERT 13.

WHAT MAKES THIS ROUND DIFFERENT FROM EVERY EARLIER ONE: A CONCURRENT WRITER.
Every prior round ran against a quiet tree, and retire_*.sh interlock 2 simply
REFUSED if any sbnd_xin wire-cell process was alive (M5).  Today a peer session
is mid-gate -- 239 events, two new default-OFF knobs -- and will still be
running when this round deletes.  A blanket refusal would either block the
round indefinitely or, worse, invite ALLOW_LIVE_JOBS=yes, which disarms the
interlock entirely.  So interlock 2 is REFINED rather than bypassed:

  * it refuses only if a live process names a dir in the REMOVAL set
    (the shape interlock 1 already used for Bokeh viewers), and
  * ASSERT 12 / interlock 6 re-derive that at DELETION time, not plan time,
    plus an mtime check -- a name computed 20 minutes ago is not evidence
    about the tree 20 minutes later.

That is a real hazard, not a hypothetical: work-pr130r1-gs1on-* and
work-pr130r1-g1off141-* did not exist when this round's first census ran and
did exist when the plan ran.  They are in KEEP because KEEP names the pr130*
PREFIX, and the deletion driver iterates the tier list rather than the
directory, so an arm created after the plan cannot be swept by construction.

THE pi0 CARVE-OUT IS THE LOAD-BEARING PART.  work-pr124r1 (8.0 G, 39 arms) and
work-pr125r1 (15.8 G, 84 arms) are the two fattest closed families and the pi0
inputs sit INSIDE them:

  em_display/pr126-pi0-manifest.tsv      -> work-pr124r1-onA{98,141v2}-*  (50)
  em_display/em117-125flipchk98-*.tsv    -> work-pr125r1-flipchk98-*      (98)
  em_display/em114c-125flipchk141-*.tsv  -> work-pr125r1-flipchk141-*    (141)

Ten arms out of 123.  The tempting alternative -- "sweep both families, re-run
a fresh arm at current HEAD" -- was considered and REJECTED: today's HEAD is a
different operating point, so doc pr/126's 0.84 fudge-factor fit would stop
being re-checkable the moment those dumps go.  Same failure the 08-25b round's
ASSERT 11 was built to prevent, one layer up.  A fresh arm is ADDITIVE work for
the pi0 round, never a substitute for the dumps the fit was made on.

THE HAND-SCAN LABELS THEMSELVES WERE UNPROTECTED, and this round found it
rather than assumed it.  sbnd_xin/em_labels/ holds 249 label JSONs (2.0 MB) --
the 141-set, the 98-set and the pi0 pairings, i.e. the literal input to the
owner's next move.  `git ls-files` returns ZERO for it: the wcp-porting-img
.gitignore has `*.json` at line 2, and overclustering_labels' 230 files are
tracked only because an earlier round ran `git add -f` (M9).  Nobody had done
that for em_labels, and there was no archive/records/labels/ copy either.  So
the labels were one `rm -rf` from unrecoverable while every arm around them was
being carefully preserved.  ASSERT 6b checks both halves now.  This is not
retirement work -- no byte of em_labels is retired -- it is the record layer
this round declines to leave broken.

COSTS, stated before the round rather than discovered after it:
  * 31 em_display/*manifest*.tsv stop resolving (ASSERT 11 lists them by name
    and row count).  All 31 are closed-round A/B legs -- pr/117 K-arms, pr/118
    dbg, pr/119 dbgA, pr/120 on2, pr/123 on1, pr/124 dbgv2/onC, pr/125
    dbg/onM.  Their verdicts are tables in their docs.  The 11 that survive are
    exactly the ones the pi0 round and the peer's scorers read.
  * doc 84's whole long-muon/MCS product layer goes: work-d84r{1,2,3,4}-* plus
    work-{mcp1k,mcp2k}-mcs80* (23 G + 3.8 G).  Owner released all three fat
    arms explicitly when asked.  Doc 84 deferred item 1 (MCS absolute scale vs
    truth) is unaffected in practice -- it needs a truth-level numu sample that
    does not exist here, so it could not be advanced from these arms anyway.
  * doc 114's em114/em114b/em114c display arms go except -prodnow-{mcp1k,mcp2k}.
  * pr/121's family goes entirely (6.0 G, 34 arms): zero script literals, zero
    surviving manifests, 305 doc mentions -- all narrative.

This round is HYGIENE with a deadline attached: the pi0 round starts next, and
starting it on a 152 G tree with 546 arms is how the last four rounds' worth of
arms became indistinguishable from each other.  /nfs/data/1 is at 62 % with
1.4 T free; nothing is at risk of filling.

Writes scripts/retire/tierA_20260829.txt and state-20260829/plan.json.
...
STATE = os.environ.get("RETIRE_STATE", os.path.join(SCR, "state-20260829"))
...
KEEP_PREFIX = {
    'work-pr129':  'doc pr/129 SHIPPED today; on98/on141 are the open round\'s production baseline (peer)',
    'work-pr130':  'doc pr/130 OPEN -- live 239-event gate, peer session asked for the whole prefix',
    'work-sent130': 'the sentinel suite guarding today\'s pr/127-129 flips, incl. negative controls',
}

KEEP_WHY = {
    # --- the doc 81 production pair: "the latest PR results", 8 -----------
    # FLAGGED (see the header): prod0825 predates six flips.  Kept because it
    # is the named production baseline and grp0825 is the campaign INPUT for
    # any re-run; superseding it is next round's decision.
    ...
    'work-nuecc48-prod0825': 'doc 81 stage B (PR), 48 evts; PROTECTED.txt',
    'work-ncpi0-prod0825':   'doc 81 stage B, 19 evts; PROTECTED.txt',
    'work-mcp1k-prod0825':   'doc 81 stage B, 1000 evts; PROTECTED.txt',
    'work-mcp2k-prod0825':   'doc 81 stage B, 2000 evts; PROTECTED.txt',
    # --- campaign INPUT for the two SIM samples, 4 ------------------------
    ...
    # --- THE pi0 EPOCH, 12 (owner's stated next move) ---------------------
    # Carved out of pr124r1/pr125r1, the two fattest sweep families.  See the
    # header and PROTECTED.txt block 1; verified live by ASSERT 11.
    ...
    # --- hand-scan DISPLAY defaults, 6 ------------------------------------
    # Hardcoded default arm names, not citations.  ASSERT 10 would let an ACK
    # silence these; that is the wrong disposition for a live default.
    ...
    'work-pr117r1-onK1-ncpi0':   'completes the 98-set display baseline (558 MB for the quartet)',
    ...
    # --- the hand-scan PROBE layer, 7 -------------------------------------
    # Added after ASSERT 10 flagged them on the first run.  These are NOT
    # closed-round records: em_display/prep_em_scan.py:275 resolves a probe
    # root through the chain work-<tag>-<arm> -> work-em114-<arm> ->
    # prod0825, and prep_em_scan.py is the tool that BUILDS the hand-scan
    # manifests the pi0 round reads.  The chain degrades gracefully if the
    # middle link is gone -- the code tests for the event, not just the dir --
    # but it degrades by silently dropping the probe text for the original 94
    # events, which is precisely the "works until someone runs it without
    # explicit args" trap ASSERT 10 exists to catch.
    # work-em114c-{mcp1k,mcp2k} are the --parse-probes source for
    # em114c-manifest.tsv (141 rows, in LIVE_MANIFESTS), named by
    # em_display/run_em114c_probe.sh:72 as the documented rebuild command.
    # 1.2 GB for the whole probe layer of both hand-scan sets.
    ...
    # --- the gate label the OPEN round cites, 6 ---------------------------
    'work-pr128r1-on98-mcp1k':   'doc pr/130 item 1 cites "gated SAME against work-pr128r1-on*"',
    'work-pr128r1-on98-mcp2k':   'the gate reference the open round cites by label',
    'work-pr128r1-on98-ncpi0':   'the gate reference the open round cites by label',
    'work-pr128r1-on98-nuecc48': 'the gate reference the open round cites by label',
    'work-pr128r1-on141-mcp1k':  '141-set half of the cited gate reference',
    'work-pr128r1-on141-mcp2k':  '141-set half of the cited gate reference',
}
for d in dirs:
...
LIVE_MANIFESTS = {
    ...
    'em_display/em117-124onA98-manifest.tsv':     98,   # pi0/EM 98-set at the pr/124 point
    'em_display/em114c-124onA141-manifest.tsv':  141,   # ... and its 141-set half
    'em_display/em117-125flipchk98-manifest.tsv': 98,   # doc pr/126 sec 4f census
    'em_display/em114c-125flipchk141-manifest.tsv': 141,
    'em_display/em117-pr130q98-manifest.tsv':     98,   # the newest scoring point (peer)
...
PEER_DEPS = [
    ...
    'work-pr129r1-on98-mcp1k', 'work-pr129r1-on98-mcp2k',
    'work-pr129r1-on98-ncpi0', 'work-pr129r1-on98-nuecc48',
    'work-pr129r1-on141-mcp1k', 'work-pr129r1-on141-mcp2k',
    'work-pr130-qx1-mcp1k', 'work-pr130-qx1-mcp2k',
    'work-pr130-47212-guardoff', 'work-pr130-292643-guardoff',
    'work-em114c-prodnow-mcp1k', 'work-em114c-prodnow-mcp2k',
    'work-pr117r1-onK1-mcp1k',
    'work-pr125r1-flipK598-mcp1k', 'work-pr125r1-flipK598-mcp2k',
    'work-pr125r1-flipK598-ncpi0', 'work-pr125r1-flipK598-nuecc48',
    'work-pr125r1-flipK5141-mcp1k', 'work-pr125r1-flipK5141-mcp2k',
    'work-pr128r1-on98-mcp1k', 'work-pr128r1-on98-mcp2k',
    'work-pr128r1-on98-ncpi0', 'work-pr128r1-on98-nuecc48',
    'work-pr128r1-on141-mcp1k', 'work-pr128r1-on141-mcp2k',
]
...
# ---------------------------------------------------------------- report
print("=== RETIREMENT ROUND 2026-08-29 (eleven closed families; the pi0 epoch stays) ===")
...
# (retire_20260829.sh interlock 6) -- plan-time evidence expires.
a12_fail = 0
...
# ---------------------------------------------------------------- emit
with open(os.path.join(SCR, "tierA_20260829.txt"), "w") as fh:
    fh.write("\n".join(sorted(TA)) + "\n")
...
print(f"\nremoval set: {len(R)} dirs -> scripts/retire/tierA_20260829.txt")
