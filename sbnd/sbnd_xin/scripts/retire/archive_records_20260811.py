#!/usr/bin/env python3
"""Archive of the record layer of the 2026-08-11 retiring arms (453 dirs).

Fork of archive_records_20260805.py. One substantive difference:

  HEAVY GAINS TWO CLASSES this round -- tracking-pr.root and
  oc56scan-evt*.jsonl -- both reclassified from `record` to `drop`:
    - tracking-pr.root's only consumer, pr33_cmp.py's comparison families
      (work-pr33-*), is entirely retired this round (superseded by the cb0805
      campaign). Keeping it would have cost ~4.8 GiB raw across 453 arms for a
      comparison nothing runs anymore.
    - oc56scan-evt*.jsonl are dumps, not results: doc pr/57 sec.12.4 re-joined
      all 899 labels to freshly regenerated dumps with 0 orphans, and the two
      arms scripts/analysis/pr57/oc56_truth.py actually cites
      (work-pr64r4-scan48/19) are KEPT whole this round, not archived-then-
      dropped -- so nothing downstream needs the retired arms' copies.

Output tree archive/records/pr38-65-era-20260811/<group>/. Never writes into
...
SCR = os.environ.get("RETIRE_STATE",
                     os.path.join(ROOT, "scripts", "retire", "state-20260811"))
OUT = os.environ.get("RETIRE_OUT",
                     os.path.join(ROOT, "archive", "records", "pr38-65-era-20260811"))
