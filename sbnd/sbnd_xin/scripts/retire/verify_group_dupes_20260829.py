#!/usr/bin/env python3
"""Prove every .groups/g<N>.tar.gz in the 2026-08-29 removal set is a pure
...
Writes state-20260829/group-dupes.tsv (one row per group tar) and exits
...
if __name__ == "__main__":
    ...
    for arm in sorted(R):
        for tgz in sorted(glob.glob(os.path.join(arm, '.groups', 'g*.tar.gz'))):
            jobs.append((arm, tgz))
    if not jobs:
        print("no .groups/g*.tar.gz in the removal set -- nothing to prove")
        sys.exit(0)
