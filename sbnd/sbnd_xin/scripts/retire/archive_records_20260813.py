#!/usr/bin/env python3
"""Archive of the record layer of the 2026-08-13 retiring arms (388 dirs).

Fork of archive_records_20260811.py.  HEAVY is UNCHANGED -- the eight patterns
inherited from that round were re-verified against this round's universe and are
still sufficient: every file over 5 MB in the removal set that they do not match
is a log (wct_pr_evt*.log, stdout.log; largest 32 MB), which is record by design,
and all 751 *scan-evt*.jsonl are oc56scan (no oc66/oc74 variant exists).

One consequence to record rather than repeat: the 08-11 round justified dropping
oc56scan-evt*.jsonl on the grounds that the two arms oc56_truth.py cites were
KEPT whole.  Those arms (work-pr64r4-scan48/scan19) retire THIS round, so the
oc56 truth table becomes non-recomputable without a fresh PR_OC56_SCAN_DUMP=1
arm.  Owner-confirmed; see plan_20260813.py's docstring and docs/work-tags.md.

Output tree archive/records/pr66-75-era-20260813/<group>/.  Never writes into an
...
SCR = os.environ.get("RETIRE_STATE",
                     os.path.join(ROOT, "scripts", "retire", "state-20260813"))
OUT = os.environ.get("RETIRE_OUT",
                     os.path.join(ROOT, "archive", "records", "pr66-75-era-20260813"))
