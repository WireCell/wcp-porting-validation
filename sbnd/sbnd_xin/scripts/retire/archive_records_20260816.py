#!/usr/bin/env python3
"""Archive of the record layer of the 2026-08-16 retiring arms (pr/79-86 sweep).

Fork of archive_records_20260813.py.  HEAVY is carried unchanged from that
round -- see plan_20260816.py's own note: not re-verified exhaustively against
this round's larger, more varied universe (pr/79-86 introduced no new
per-event product beyond what 08-13 already classified), but the archiver's
own integrity gate below still catches a tar/manifest mismatch if a HEAVY
class was missed.

Output tree archive/records/pr79-86-era-20260816/<group>/.  Never writes into
...
OUT = os.environ.get("RETIRE_OUT",
                     os.path.join(ROOT, "archive", "records", "pr79-86-era-20260816"))
