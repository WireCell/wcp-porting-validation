...
FORK NOTE (doc 81 sec 10, the 08-25b round).  Byte-identical to
``hash_manifest_20260825.py`` except for the output directory: this writes into
``state-20260825b/hashes/`` so it cannot overwrite the 08-25 round's frozen
snapD2 manifests (M13 -- those are the record of a past gate).  Used here on
``work-<s>-prod0823``, whose ``pr_evt<N>`` layout this tool already handles.
Stage-A arms (``evt<N>``/``ql_evt<N>``) are NOT covered -- see
``hash_manifest_stagea_20260825b.py`` for why reusing this one on them would
fail silently.

Read this before treating the prod0823 manifests as gate preservation, because
they are not: ``work-<s>-prod0825`` is at a DIFFERENT operating point (post
dl_vtx_dual_chain/snapD2 + fast_xgb_forest), so no byte-identity claim exists
between the two to keep re-checkable.  What the freeze buys is a future
revert-reproduction check -- re-run at toolkit b5c9f43a and diff against this
file.  Real, but speculative; recorded as insurance, not as a gate.
...
def main():
    arms = sys.argv[1:]
    if not arms:
        sys.exit("usage: hash_manifest_pr_20260825b.py <arm> [<arm> ...]")
