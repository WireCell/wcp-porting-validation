#!/usr/bin/env python3
"""doc 91 round C -- the sentinel-coverage guard, shared by plan ASSERT 16 and
driver interlock 8 so the two cannot drift apart.

ROUND C ADDS THE FALSIFIABILITY CHECK.  After the sec-12 re-baseline the suite
is GREEN against production, so round B's regression check (`which failing
sentinel would lose its last passing arm`) has nothing left to protect and would
pass vacuously -- the dead-guard shape this round exists to avoid.  The property
that matters now is the mirror image: each re-baselined sentinel must keep a
surviving arm in which it FAILS.  Without one, the threshold is green and
nothing on disk can show it is still capable of failing.

Scope is the FOUR sentinels this round re-baselined and built controls for.
The other 27 have never had a negative control -- a pre-existing gap, stated
here rather than silently folded into a passing check.
...
# ---------------------------------------------------------------------------
REBASELINED = {
    47212:  'work-91neg-backguard-mcp2k',
    137238: 'work-91neg-scccgap-nuecc48',
    292643: 'work-91neg-backdvtx-mcp1k',
    393505: 'work-91neg-gfimpact-mcp2k',
}


def falsifiable(keep, prod_suffix='-prod0901b'):
    """Each re-baselined sentinel must keep ITS OWN negative control, still failing.

    FIRST FORMULATION WAS TOO WEAK AND ITS OWN CONTROL CAUGHT IT.  It asked
    "does SOME surviving non-production arm make this sentinel FAIL", and that
    stayed clean even with every new control deleted -- because after the sec-12
    re-baseline the thresholds track production, so any arm at an OLDER
    operating point fails them too.  That is drift detection, not knob
    detection: an arm failing because it predates the 0.86 EM flip is no
    evidence at all that the threshold can still see a dead fix.

    The causal property -- "this arm fails BECAUSE the knob was off" -- is not
    recoverable from the output after the fact; it is a fact about how the arm
    was produced.  So the control is named, one per re-baselined sentinel, and
    the check re-derives the FAIL by RUNNING the suite.  The name is the claim;
    running it is what stops the claim going stale when a control silently
    starts passing.
    """
    bad = []
    for ev, ctl in sorted(REBASELINED.items()):
        if ctl not in keep:
            bad.append((ev, ctl, 'control not in KEEP'))
        elif not os.path.isdir(os.path.join(ctl, 'pr_evt%d' % ev)):
            bad.append((ev, ctl, 'control does not hold the event'))
        elif _verdicts(ctl).get(ev) != 'FAIL':
            bad.append((ev, ctl, 'control no longer FAILs -- it proves nothing'))
    return bad


def report_c(all_dirs, keep, out=print):
    ok = report(all_dirs, keep, out=out)
    bad = falsifiable(keep)
    for ev, ctl, why in bad:
        out(f"  !! evt {ev}: negative control {ctl} -- {why}.  The re-baselined "
            f"threshold would be green with nothing able to show it can fail.")
    if not bad:
        out(f"  OK  all {len(REBASELINED)} re-baselined sentinels keep a surviving "
            f"arm in which they FAIL")
...
if __name__ == '__main__':
    ...
    os.chdir(sys.argv[2])
    ok = report_c(sorted(d for d in os.listdir('.')
