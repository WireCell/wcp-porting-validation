...
#
# Fork of retire_20260816.sh. All of that round's fixes are carried forward
# (the `-type d` survivor census, the real-tier-file Bokeh interlock, the
# widened interlock-2 grep, the `cd -P` symlink fix, interlocks 0/1/2 exiting
# in dry run too).
#
# ONE defect fixed in this fork's interlock 0: 08-16's interlock 0 refuses
# outright if ANY broken symlink exists anywhere in the tree before the
# round starts. This round the pre-flight found 360 broken symlinks, all
# self-contained inside four dirs already in the removal set
# (work-{r1qlmc,r2mc,nuecc48,ncpi0}-vfcbr3off) -- the doc-73 sec.12.5
# "harness fix that rode along" bug (run_valfast.sh's nusel_root() returned
# RELATIVE roots for the small samples' first OFF attempt, so
# ql_evt<N>/*.npz symlinks resolve relative to the wrong directory). The
# real data those broken links point at lives in work-img-<sample>/evt<N>/
# (a KEEP imaging hub) via a SEPARATE, working symlink one level up
# (evt<N> -> work-img-<sample>/evt<N>) -- nothing is lost, only a
# convenience symlink inside a dir being deleted anyway. A straight
# unconditional refuse here would block every future round until someone
# manually cleans four already-condemned dirs. Fix: interlock 0 now checks
# whether EVERY broken link's containing top-level dir is already in this
# round's own removal set; if so it is a WARNING, not a refusal (the
# dangling links vanish along with their dir), and refuses exactly as
# before for any broken link outside the removal set.
#
#   ./retire_20260817.sh A              # dry run (default action)
#   CONFIRM=yes ./retire_20260817.sh A  # actually delete
#
# Tier dispositions: tier A archives (already done via
# archive_records_20260817.py) then deletes -- all removal-set dirs this round.
#
# Pre-flights (all must have passed before CONFIRM=yes):
#   scripts/retire/plan_20260817.py             (8 asserts, "OVERALL: PASS")
#   scripts/retire/archive_records_20260817.py  (integrity gate PASS n/n)
#
...
STATE=$BASE/scripts/retire/state-20260817
REC=$BASE/archive/records/pr88-90-era-20260817
TIERS=${1:-A}
CONFIRM=${CONFIRM:-no}
...
tierfiles=""
for t in ${TIERS//,/ }; do
    f=$BASE/scripts/retire/tier${t}_20260817.txt
...
if [ "$CONFIRM" = yes ]; then
    {
      echo "# retire_20260817.sh  tiers=$TIERS"
