...
#
#   ./scripts/retire/retire_20260904c.sh              # dry run (default)
#   CONFIRM=yes ./scripts/retire/retire_20260904c.sh  # actually delete
#
# Order (non-negotiable, carried from the 08-31 / 09-01 / 09-02 rounds):
#   scripts/retire/plan_20260904c.py                 (8 interlocks, "OVERALL: PASS")
#   scripts/retire/archive_records_20260904c.py      (integrity gate PASS n/n)
#   ./scripts/retire/retire_20260904c.sh             (DRY RUN, check dirs/bytes)
#   CONFIRM=yes ./scripts/retire/retire_20260904c.sh
#
...
STATE=$ROOT/scripts/retire/state-20260904c
REC=$ROOT/archive/records/campaign-close-20260904c
CONFIRM=${CONFIRM:-no}
cd "$ROOT" || exit 2

[ -f "$STATE/plan.json" ] || { echo "REFUSE: no plan.json -- run plan_20260904c.py"; exit 2; }

# --- interlock A: the plan must be PASSing right now, not when it was written
echo "== interlock A: re-running plan_20260904c.py =="
python3 "$ROOT/scripts/retire/plan_20260904c.py" > "$STATE/plan-recheck.log" 2>&1
rc=$?
...
STATE=os.path.join(SX,"scripts/retire/state-20260904c")
REC=os.path.join(SX,"archive/records/campaign-close-20260904c")
...
if [ "$CONFIRM" != "yes" ]; then
  ...
  echo
  echo "To execute:  CONFIRM=yes ./scripts/retire/retire_20260904c.sh"
  exit 0
fi
...
for d in $DIRS; do
  case "$d" in
    # work-em114* is deliberately ABSENT from this list: doc 100 sec 10
    # releases it (no em_display manifest resolves into it).
    # work-vtx105-base-* IS still here and STAYS -- 878 hand-scan label
    # files under vertex_labels/vtxscan-vtx105-* reference it 1724 times (M13).
    ...
    work-vtx105-base-*|work-pr134-f086-*|work-pr130r1-*|\
