#!/usr/bin/env python3
"""Archive of the record layer of the 2026-08-25b retiring arms -- the stage-A
reference side (work-img-<s> + work-<s>-ql0819) and the pre-flip PR baseline
(work-<s>-prod0823).

Fork of archive_records_20260825.py; only the state dir and the output tree
differ.  Writes archive/records/stagea-refside-20260825b/ -- a NEW tree, never
into the 08-25 round's (M13).

HEAVY is carried unchanged and it is what makes this round's archive small:
the bulk here is icluster npz, mabc zips, pctree tarballs, tracking-pr.root and
calib dumps, all classified HEAVY and therefore excluded.  What is archived is
the record layer -- the per-event compiled config (.wct-cfg-evt<N>.json, i.e.
the operating point each arm actually ran), logs, rc files and nusel TSVs.
The member-content proof of the heavy layer is NOT in this tar; it is in
state-20260825b/hashes/ (see hash_manifest_stagea_20260825b.py), which is
git-tracked and is what keeps doc 81 sec 7's 24536/24536 re-checkable.
...
SCR = os.environ.get("RETIRE_STATE",
                     os.path.join(ROOT, "scripts", "retire", "state-20260825b"))
OUT = os.environ.get("RETIRE_OUT",
                     os.path.join(ROOT, "archive", "records", "stagea-refside-20260825b"))
