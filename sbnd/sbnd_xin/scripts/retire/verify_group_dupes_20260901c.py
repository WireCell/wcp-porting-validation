...
Writes state-20260901c/group-dupes.tsv (one row per group tar) and exits
...
STATE = os.environ.get("RETIRE_STATE",
                       os.path.join(ROOT, "scripts", "retire", "state-20260901c"))
...
def sample_of(arm):
    # TWO naming conventions live in this tree and only the first was handled:
    #   work-pr134-f086-mcp1k   sample as SUFFIX   (every pr-round arm)
    #   work-mcp1k-prod0830     sample in the MIDDLE (every production arm)
    # The 08-29..08-31b verifier only matched the suffix form, and returned
    # None for the second -- which made QL_ROOT.get(None) fail the "root in
    # KEEP" test and refuse the round.  It never showed up before because no
    # earlier removal set contained a group-mode PRODUCTION arm; doc 89's does
    # (work-*-prod0830 carries 193 .groups/g<N>.tar.gz).  Suffix is tried
    # first, so no previously-classified arm changes meaning.
    for s in QL_ROOT:
        if arm.endswith('-' + s):
            return s
    parts = arm.split('-')
    if len(parts) >= 3 and parts[0] == 'work' and parts[1] in QL_ROOT:
        return parts[1]
    return None
...
if __name__ == "__main__":
    ...
    if not jobs:
        ...
        with open(os.path.join(STATE, "group-dupes.tsv"), "w") as fh:
            fh.write("# verify_group_dupes_20260901c.py: 0 .groups/g*.tar.gz in the\n"
