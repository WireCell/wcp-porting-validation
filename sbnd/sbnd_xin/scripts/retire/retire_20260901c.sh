...
#
#   ./retire_20260901c.sh A              # dry run (default action)
#   CONFIRM=yes ./retire_20260901c.sh A  # actually delete
#
# Order (non-negotiable, from the 08-31 round):
#   scripts/retire/verify_group_dupes_20260901c.py  (writes group-dupes.tsv)
#   scripts/retire/plan_20260901c.py                (16 asserts, "OVERALL: PASS")
#   scripts/retire/archive_records_20260901c.py     (integrity gate PASS n/n)
#   ./retire_20260901c.sh A                         (DRY RUN, check dirs=/bytes=)
#   RETIRE_REPLAN=1 python3 .../plan_20260901c.py   (re-stamp planned_at)
#   CONFIRM=yes ./retire_20260901c.sh A
#
...
# editing.  Every date-bearing name was repointed: STATE=state-20260901c,
# REC=archive/records/campaign-close-20260901c, and the INTERPOLATED tier file
# tier${t}_20260901c.txt -- the 09-01 header warns that a missed rename here
...
STATE=$BASE/scripts/retire/state-20260901c
REC=$BASE/archive/records/campaign-close-20260901c
TIERS=${1:-A}
CONFIRM=${CONFIRM:-no}
...
tierfiles=""
for t in ${TIERS//,/ }; do
    f=$BASE/scripts/retire/tier${t}_20260901c.txt
...
# NOT `[ -s ]`.  archive_records_20260901c.py DROPS 4.96 GiB of
# .groups/g<N>.tar.gz on the strength of verify_group_dupes_20260901c.py's
...
if [ ! -f "$GD" ]; then
    echo "!! $GD MISSING -- run verify_group_dupes_20260901c.py (RETIRE_STATE=...state-20260901c) before deleting"; exit 2
fi
...
# The owner's stated next move reads these.  plan_20260901c.py ASSERT 11 checked
...
if [ -n "$fresh" ]; then
    ...
    echo "refusing -- re-run plan_20260901c.py (RETIRE_REPLAN=1) and re-read the list."
    exit 2
fi
...
#
# Same implementation as ASSERT 16 -- scripts/retire/sentinel_guard_20260901c.py
...
if ! python3 "$BASE/scripts/retire/sentinel_guard_20260901c.py" "$STATE/plan.json" "$BASE"; then
...
if [ "$CONFIRM" = yes ]; then
    {
      echo "# retire_20260901c.sh  tiers=$TIERS"
