...
# See ../../docs/work-tags.md section "RETIREMENT ROUND 2026-08-05" and
# ../../docs/pr/37_cross-audit-residual-round.md sec.13.
#
#   ./retire_20260805.sh A,D              # dry run (default action)
#   CONFIRM=yes ./retire_20260805.sh A,D  # actually delete
#
# Tier dispositions:
#   tier A  archive record layer first, then delete   (182 dirs)
#   tier D  DROP whole, no archive                    (19 dirs)
#             - the 12 vf37 floor arms, released by the owner 2026-08-05 under
#               the valfast disposal rule (the compare summary lives in doc
#               pr/37 sec.2.5, and sec.13.5 records that they are going);
#             - 7 arms their own docs call void, or named smoke/probe.
#
...
#   c. NEW: a post-hoc removal manifest, state-20260805/removed.tsv.  Three
...
BASE=$PWD
STATE=$BASE/scripts/retire/state-20260805
REC=$BASE/archive/records/pr28-37-era-20260805
TIERS=${1:-A,D}
CONFIRM=${CONFIRM:-no}
...
if [ -n "$viewers" ]; then
    ...
    vhit=""
    for d in $(cat "$BASE"/scripts/retire/tier{A,D}_20260805.txt); do
...
list=""
for t in ${TIERS//,/ }; do
    f=$BASE/scripts/retire/tier${t}_20260805.txt
...
if [ "$CONFIRM" = yes ]; then
    {
      echo "# retire_20260805.sh  tiers=$TIERS"
