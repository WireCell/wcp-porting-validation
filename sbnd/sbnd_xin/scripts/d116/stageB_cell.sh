#!/bin/bash
# doc sbnd_xin/116: stage B (the 15-stage PR + tagger + BDT chain) for ONE study cell on a doc-115
# stage-A arm.  Fork by duplication (CLAUDE.md M10) of scripts/d115/stageB.sh, which stays untouched
# and keeps writing the baseline arms work-r3<s>-d115pr.
#
# A cell is a set of PR-job knobs, given as ONE file of raw jsonnet `key=value` lines
# (docs/116_figs/tla/<cell>.tla -> PR_EXTRA_TLA, appended last by run_pr_chain_batch.sh:1881 so it
# wins over every SBND_* env) plus, when docs/116_figs/tla/<cell>.tfjson exists, the TrackFitting
...
# `s0rep` has no .tla and no .tfjson: it is the production configuration re-run, the DL-vertex
# noise floor (CLAUDE.md M4).
#
...
# existing out_root is refused unless its .d116_cell marker names this cell (resume of a partial
...
#   cell   s0rep | cs | p3bw | csp3bw | tfull | any later rung with docs/116_figs/tla/<cell>.tla
...
case "$S" in
    cv)    A=$SX/work-r3cv-d115;  B=$SX/work-r3cv-d116$CELL;  REALITY=sim ;;
    nuecc) A=$SX/work-r3nue-d115; B=$SX/work-r3nue-d116$CELL; REALITY=sim ;;
    off)   A=$SX/work-r3off-d115; B=$SX/work-r3off-d116$CELL; REALITY=data ;;
    *) echo "unknown sample: $S" >&2; exit 2 ;;
esac
[ -d "$A" ] || { echo "ERROR: no stage-A arm $A" >&2; exit 1; }

TLA=$SX/docs/116_figs/tla/$CELL.tla
TFJ=$SX/docs/116_figs/tla/$CELL.tfjson
if [ "$CELL" != s0rep ]; then
    [ -s "$TLA" ] || { echo "ERROR: cell '$CELL' has no $TLA (s0rep is the only cell without one)" >&2; exit 2; }
...
if [ -e "$B" ]; then
    [ "$(cat "$B/.d116_cell" 2>/dev/null)" = "$CELL" ] \
        || { echo "REFUSING: $B exists and is not a '$CELL' arm of this script (M13)" >&2; exit 2; }
    echo "=== resuming existing $B"
else
    mkdir -p "$B" && echo "$CELL" > "$B/.d116_cell"
fi
LOGD=$HOME/tmp/d116-stageB-$CELL-$S; mkdir -p "$LOGD"

echo "=== d116 stage B: cell=$CELL sample=$S reality=$REALITY PR_JOBS=$J  $(date +%F_%H:%M:%S)"
...
# sub-root is claimed with an atomic mkdir of $qb/.d116_running and released on completion; a
...
[ "${CLEAR_LOCKS:-0}" = 1 ] && rm -rf "$B"/f*/.d116_running "$B"/.d116_running
...
for sub in "${SUBS[@]}"; do
    ...
    mkdir -p "$qb"
    if ! mkdir "$qb/.d116_running" 2>/dev/null; then
