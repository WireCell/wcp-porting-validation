#!/bin/bash
# pr/123 round 1 arms (fork of pr121_arms.sh, prefix change only).  Usage: pr123_arms.sh <manifest 98|141> <armtag> <probes 0|1> [env assignments...]
...
export PR_EXTRA_STAGES=pr_display PR_JOBS=24
declare -A EV
if [ "$MAN" = "98" ]; then
...
for s in $SAMPLES; do
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr123r1-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr123_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
