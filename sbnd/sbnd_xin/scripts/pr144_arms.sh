...
#
# Usage: [JOBS=16] [PIN=/home/xqian/tmp/d144_libpin2] [TAG=d144] ./scripts/pr144_arms.sh off|on|frameonly
...
set -u
ARM=${1:?usage: pr144_arms.sh off|on|frameonly}
JOBS=${JOBS:-16}
...
case "$ARM" in
  ...
  *)   echo "arm must be off|on|frameonly" >&2; exit 2 ;;
esac
