...
def marks(labs):
    ...
    for ev, rec in labs.items():
        for shw, det in ((rec.get("em") or {}).get("marks_detail") or {}).items():
            ...
            outs[(ev, int(shw))] = {int(s) for s, v in m.items() if v.get("kind") == "out"}
    return ins, outs
    ...
    for tag, manf, labtag, tsv, prepdir in SETS:
        ...
        try:
            ...
            for r in rows:
                ...
                ins, outs = ins_by.get((ev, shw), set()), outs_by.get((ev, shw), set())
        ...
        if tot:
            top = sorted(aff_ev.items(), key=lambda kv: -kv[1])[:10]
            print("\n  AFFIRMATIVE q_extra: %d events, top-10 holds %.1f%%"
                  % (len(aff_ev), 100 * sum(v for _, v in top) / tot))
            same = cross = 0
            ...
            for ev, shw, shw_cl, q, segs in sorted(detail, key=lambda d: -d[3]):
                txt = []
                for s, i in segs:
                    txt.append("%d(cl%s,%.0fcm,pdg%s)" % (s, i.get("cluster"), i.get("length", 0.0), i.get("pdg")))
                    if i.get("cluster") == shw_cl:
                        same += 1
                    else:
                        cross += 1
                print("  %8d %8d %7s %10.3e %5d  %s" % (ev, shw, shw_cl, q, len(segs), "  ".join(txt)))
            print("\n  condemned segment sits in the SHOWER'S OWN cluster: %d      "
                  "in a DIFFERENT cluster: %d" % (same, cross))
        print()
    return 0
