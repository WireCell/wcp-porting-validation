#!/bin/bash
# pr/131 arms (fork of pr130_arms.sh: prefix + probe env only -- pr130_arms.sh stays byte-untouched, M10).
# probes=1 => WCT_PFDENOM_DEBUG=1, the doc pr/131 denominator census (stderr only, no knob,
#   no production gate relaxed).  See MultiAlgBlobClustering.cxx "THE DENOMINATOR CENSUS".
...
for s in $SAMPLES; do
  ./run_pr_chain_batch.sh work-$s-grp0825 work-pr131-$TAG-$s data ${EV[$s]} > /home/xqian/tmp/pr131_${TAG}_$s.log 2>&1
  echo "arm $TAG-$s rc=$?"
done
