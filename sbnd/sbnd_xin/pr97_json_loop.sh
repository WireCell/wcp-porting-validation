#!/usr/bin/env bash
# doc pr/97 sec.5 -- the gojsonnet arm: is the SBND 18255-178410 SIGSEGV a Go
# runtime thread?  gdb caught the signal on thread 34 (a parked libgojsonnet Go
# thread, PC=0x0) while thread 1 was healthy inside hough_transform, and all 32
# other threads were Go threads in runtime.futex.  This driver runs the SAME
# event with the config PRECOMPILED by a separate wcsonnet process
# (run_ql_evt_pr97json.sh), so the wire-cell process never runs jsonnet.  If the
# ~4 %-per-run rate goes to zero over ~100 runs, the Go runtime is the cause and
# precompiling is the workaround.
#
...
# whichever run dies: run_ql_evt_pr97json.sh is run_ql_evt.sh with the wire-cell
...
one() {
    ...
    ( cd "$R/.cwd" || exit 92
      SBND_INPUT_DIR=$STAGE SBND_WORK_ROOT=$R \
          ...
          "$SBND_DIR/run_ql_evt_pr97json.sh" data 1
    ) > "$R/.log.log" 2>&1
    local rc=$?
...
while [ "$i" -le "$N" ]; do
    ...
    wait
    # no early break: this arm measures a RATE, so every run counts.
done
