...
local cfsinks_live = [ClusterFileSink("sbnd_live_clus_apa%d.npz" % n) for n in std.range(0, std.length(tools.anodes) - 1)];
local cfsinks_dead = [ClusterFileSink("sbnd_dead_clus_apa%d.npz" % n) for n in std.range(0, std.length(tools.anodes) - 1)];

local img_dump_pipe = [g.intern(
    innodes = [img_pipes[n]],
    centernodes = [],
    outnodes = [cfsinks_live[n], cfsinks_dead[n]],
    edges = [
        g.edge(img_pipes[n], cfsinks_live[n], 0, 0),
        g.edge(img_pipes[n], cfsinks_dead[n], 1, 0)
    ]
)
