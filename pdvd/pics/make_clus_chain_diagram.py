#!/usr/bin/env python3
"""ProtoDUNE-VD Wire-Cell 3-D clustering + charge-light (Q/L) matching diagram.
...
Traced from cfg/pgrapher/experiment/protodunevd/{clus,qlmatching,flash}.jsonnet
and pdvd/{wct-clustering,run_clus_evt,run_light_evt}.  Output:
pdvd/pics/pdvd_clus_ql_chain.{png,pdf}.
"""
import os
...
OUT = os.path.join(HERE, "pdvd_clus_ql_chain")
...
def main():
    c = Canvas()
    ov = c.ov
    ov.text(W / 2, 8.66, "ProtoDUNE Vertical Drift — Wire-Cell 3-D "
    ...
    ov.text(W / 2, 8.22, "per-face → per-APA → per-drift-group → all-TPC "
            ...
            "drift T0", ha="center", va="center", fontsize=13.5, color="#444")
    ...
    c.algobox(*n_in, 2.55, 1.50, "imaging clusters",
              ["ClusterFileSource ×8 anodes",
               "live (active) + dead (masked)",
               "→ PointTreeBuilding",
               "   (BlobSampler: live 3d, dead)"],
              "white", C_IN, tc=C_IN, title_fs=11.5, bullet_fs=8.6, dy=0.265)
    c.algobox(*n_s1, 2.45, 1.50, "① per-face  ×16",
              ["pointed · live/dead",
               "extend · regular ×2",
               "parallel_prolong · close",
               "extend_loop · connect1"],
              "white", C_CLUS, tc=C_CLUS, title_fs=11.5, bullet_fs=8.8,
              dy=0.265)
    c.algobox(*n_s2, 2.05, 1.50, "② per-APA  ×8",
              ["merge 2 faces",
               "deghost",
    ...
    c.algobox(*n_s3, 3.28, 1.50, "③ per-drift-group  ×2",
              ["extend·regular×2·prolong·close·extend_loop",
               "separate — 3-D track split + recover",
               "connect1 · deghost · examine_x_boundary",
               "neutrino · isolated"],
              "white", C_CLUS, tc=C_CLUS, title_fs=11.5, bullet_fs=8.5,
              dy=0.265)
    c.algobox(*n_s4, 2.55, 1.50, "④ all-TPC  ×1",
              ["switch_scope → x_t0cor",
               "cathode_connect",
               "   (stitch cathode crossers)",
               "→ mabc-all-apa.zip"],
              "#eaf5ee", C_OUT, tc=C_OUT, title_fs=11.5, bullet_fs=8.6,
              dy=0.265)
    ...
    c.algobox(*n_lr, 3.95, 1.52, "LIGHT RECO — separate job",
              ["OpDecon — deconv, detect_saturation",
               "OpHitFinder — flag_sat, coverage",
               "OpFlashFinder — minPE 25",
               "→ 40 photon-detector flashes"],
              "white", C_LIGHT, tc=C_LIGHT, title_fs=11.5, bullet_fs=8.7,
              dy=0.275)
    c.algobox(*n_ql, 5.35, 1.66, "QLMatching — joint LASSO Q/L fit  "
              ...
              ["bundles = cluster × flash  ·  containment / fiducial prefilter",
               ...
               "xTPC crosser pin · consistency ladder · rescues · "
               "sat/cov keep-and-mark",
               "photon model: library 128 nm Ar · QtoL 0.094 · per-family eff"],
              "white", C_QL, tc=C_QL, title_fs=11.2, bullet_fs=8.7, dy=0.295)
    ...
    ov.text(bx[0], bx[1], "opflash\n*.tar.gz", ha="center", va="center",
            fontsize=8.0, color=C_IN, zorder=6)
    ...
    ov.text(7.9, 5.82, "clusters, both sides", ha="center", fontsize=8.4,
    ...
    ov.annotate("", xy=(n_s4[0] - 0.35, cy - 0.75),
                ...
                arrowprops=dict(arrowstyle="-|>", color=C_T0, lw=2.6,
                                shrinkA=2, shrinkB=2,
                                connectionstyle="arc3,rad=0.12"))
    ov.text(12.95, 5.85, "cluster T0 → x_t0cor", ha="center", fontsize=8.6,
            color=C_T0, fontweight="bold")

    # mode note (empty mid-gap, under ①/②)
    ov.text(4.55, 5.86, "default do_qlmatch = on:  ③ → QLMatching → ④    "
            "(no-QL: ③ → ④ direct merge)", ha="center", va="center",
            fontsize=8.0, color="#6a6f78", style="italic")

    # ================= insets =================
    c.place_image(os.path.join(SRC, "qlmatch_pattern.png"), 5.15, 4.30, 0.55,
                  "Q/L light pattern — flash 57 (measured vs library-predicted)",
                  (7.35, 3.60), C_QL)
    c.place_image(os.path.join(SRC, "clus_event_3d.png"), 12.75, 3.30, 0.55,
                  "clustering result — clusters coloured  (evt 298567)",
                  (n_s4[0], 6.30), C_OUT)
    ...
    for i, (col, lab) in enumerate([
        ...
        ov.text(lx + 0.58, ly - i * dy, lab, va="center", fontsize=8.8,
                color=INK)

    ov.text(W / 2, 0.13,
            "Wire-Cell Toolkit  ·  cfg/pgrapher/experiment/protodunevd/"
            "{clus,qlmatching,flash}.jsonnet  ·  pdvd/{wct-clustering,"
            "run_clus_evt,run_light_evt}  ·  operating point ON via runner "
            "(jsonnet defaults byte-identical OFF)  ·  insets: run 039252 "
            "evt 298567 (keep round)", ha="center", va="center", fontsize=8.6,
            color="#8a8a8a")
