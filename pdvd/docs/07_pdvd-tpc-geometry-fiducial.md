...
> see the dedicated `pdvd/docs/qlmatch/04_pdvd-crp-anode-plane-geometry.md` for the
...
- **Drift speed:** sim uses **1.473 mm/µs** (`simparams.jsonnet`, the true MC
  ...
  **1.568 mm/µs** (rescaled from 1.57 by `v ∝ D`; `params.jsonnet`; see
  `pdvd/docs/nf_sp_img_clus/02_clus-workflow.md`). A cluster's apparent drift-x therefore differs
...
Flash-side inputs (from the light-chain multiplicity study, `06_pdvd-light-chain.md`
...
- No PDVD-specific `cathode_fiducial` (CPA structure-exclusion) exists; SBND-only
  today.
- **Flash count & dead-PD self-id are QL-time work** (`06_pdvd-light-chain.md`): the
...
- Related PDVD docs: `nf_sp_img_clus/16_pdvd-wire-geometry-v3-v4.md`,
  `nf_sp_img_clus/24_single-face-anode-feasibility.md`,
  `nf_sp_img_clus/02_clus-workflow.md` (drift-velocity calibration), `01_photon-detector-chain.md`.
