...
1. **Next: item 7b, the stop snap within the entry's component.** *Done in doc 88:* 36/63's stopper returns, and the flip is held for the owner's smx6 look at 15/36. When the stop-local keep has fired on a candidate, the stop's `anchor_vertex` should consider only vertices the entry can reach. Kept residuals are disconnected by construction, so this removes exactly the capture of §6.2. It needs a new default-OFF knob, evaluated only on keep-fire candidates (7 on `p87v5f`), so every other candidate stays bit-identical by construction. Re-run `p87v5f` with it.
