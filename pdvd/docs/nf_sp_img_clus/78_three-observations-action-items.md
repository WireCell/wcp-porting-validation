...
3. **T2c's kink margin.** `039252_2/79`: owner-confirmed STM_MICHEL, split fired, 8.9 MeV / 9.3 cm Michel at 59.58°, demoted by `moved_stop_michel_guard` with `moved_stop_michel_kink_min` 60. Re-grade T2c on the current record (doc 61 graded it on smx1a, where this item read THRU): if it is now 1 owner TP lost for 4 THRU FPs removed, state so; a value under 59.5° recovers the item — a one-item margin, to be declared as such. No scan.
   ...
   *Result (doc 84): FLIPPED in PDVD production, as a reach exemption, not the kink value* — `moved_stop_michel_reach_min_cm: 6.5`. `039252_2/79` recovers its Michel; on the owner's merged record `michel_found` goes 140 / 12 / 20 → 141 / 12 / 19, and every other candidate, branch, point row, zip and calib is identical. The 59.1° kink value gives the identical result on this record and is reported, not flipped.

   *Correction (doc 84, 2026-09-11), on three points.* **(a)** On today's production T2c removes **2** THRU FPs (`039252_4/55`, `039349_61/62`) for the 1 owner TP, not 4: two of doc 61's five "through-going" items are STM_MICHEL on the merged record, and `039349_20/41` no longer fires. **(b)** The kink value is a 0.9° window, (58.68°, 59.58°], beside `039349_61/62`, whose kink reads 48.21° under the survey bag and 58.68° bare — 10.5° of movement from a configuration that never touches T2c. KE does not separate the 12 T2c instances on record (61/62 reads 8.0 MeV on the survey arms, 2/79 7.2 MeV on the stop-mover arms). Reach (arm length + far subtree) does: THRU 4.9–5.8 cm, owner Michels 7.5–11.9 cm. The owner chose it. **(c)** `census_score.py` grades on the frozen smx1a record by default, where this item is still THRU; there the recovered Michel reads as one more FP. The grade that answers this item is on the merged record.
...
5. **P4 rejection census.** For the 76 same-bundle gamma tags within 35 cm and ≤ 10 cm on items *with* a Michel object, which gate excluded them (cone, per-gamma cap, body); then decide whether the collect should anchor on the stop when no Michel is found (the other 54 items). Item 1's per-piece `rej` column is the natural carrier. Doc 71's purity (0.933) is the bar.
   ...
   *Result (doc 85): the census needs no knob, the stop anchor is dead, and a capture gamma is now withheld from a rejected candidate — FLIPPED in PDVD production* as `stop_gamma_require_stm: true`. On items with a Michel, every P4 gate rejects at least as much delta/other as gamma (the cone 12 gamma / 29 delta-other, the body test 6 / 7), so none is loosened. The stop-anchored collect already exists: it is doc 51's capture stage (role 5). Widened to 50 cm on the arm (`p85vsg50`), it adds 15 good / 38 delta-other where P4 never runs. The census found that the capture stage fires on candidates the verdict rejects: 36 gamma / 0 delta-other on `is_stm` 1, and 7 / 33 on `is_stm` 0 (27 of the latter on through-going muons). With the knob, role 5 on judged items goes 43 / 33 / 1 → 36 / 0 / 0, with verdicts identical and every `is_stm`-1 candidate bit-identical.

   *Correction (doc 85, 2026-09-11), on three points.* **(a)** §4.3 joined the record's tags to the chain by segment id, and the scan arms' segment ids sit one graph index off production's on some clusters. 23 gamma tags whose cluster *is* collected (role 4 on 14, role 3 on 6, role 5 on 3) were counted as "no chain role". At cluster level the "76" is 55 tags in 53 clusters on 38 items. **(b)** Only 13 of those clusters, on 9 items, have a Michel object: 7 are excluded by the cone, 4 by the body test, 1 by the blob energy cap, and 1 has no gate line. Not 76. **(c)** The "54 items without a Michel" are 29 items, and 32 of their 40 clusters are on STM_ONLY items. Those are capture gammas, not a missed Michel's, and the chain already anchors a collect on the stop for them.
...
7. **The unfitted Michel blob at the stop (doc 86 §8.1).** PR fits a residual at the stop and drops it as isolated (`pr54`). Doc 62 T3a's `stop_local_residual_cm` keeps such a residual, but it has no size floor, and at 20 cm it touches 31 items, 14 of them through-going. Add a size floor to the anchor keep, default OFF (≥ 5 points, ≥ 5 cm), and run it at 5 cm. On today's production that touches **5 items**:
   ...
   *Result (doc 87): built, measured, NOT flipped.* `stop_local_residual_min_points` / `_min_len_cm` (C++ default 0 / 0); the OFF path is byte-identical on both detectors. At 5 cm with 5 / 5 (`p87v5f`):
   - the Michel census goes 141 / 12 / 19 → 143 / 13 / 17: both owner Michels are gained, `039253_15/36` is the one FP, and 0 THRU are touched;
   - but `039349_36/63` loses its stopper call.

   The kept residual's endpoint (0.64 cm from the tagger's stop) is nearer than the chain's own end, and the stop snap takes the nearest cluster vertex without a reachability test. The chain is then unmatched; this is doc 62's `039349_61/21` loss again. The floor itself works: against the bare 5 cm radius it removes all 4 THRU fires and the 61/21 loss. Next: **item 7b**, the stop snap restricted to vertices the entry can reach (doc 87 §8).

   *7b result (doc 88): built; flip HELD for the owner's look at 15/36.* The new knob `stop_snap_reachable` (C++ default false) acts only where the keep fired. With it, the stop snaps onto nothing the entry cannot reach. On the floored 5 cm arm (`p88v5fr`) the skip fires on `039349_36/63` alone, and 36/63 is a stopper with its Michel again. The totals:
   - Michel census 141 / 12 / 19 → 143 / 13 / 17;
   - `is_stm` 233 / 7 / 45, identical to production;
   - 0 THRU touched, and nothing moves without a keep fire.

   The only new FP is `039253_15/36`. By the owner's standing answer, it is served blind as `smx6` on :5017, and their verdict decides the flip.

   *Flipped (doc 88 §9):* the owner judged `039253_15/36` STM + MICHEL (`unfitted`). On the smx1a+smx3+smx4+smx5+smx6 record the flipped keep plus snap is Michel 141 / 12 / 20 → 144 / 12 / 17 with `is_stm` unchanged. `stop_local_residual_cm` 5, `stop_local_residual_min_points` 5, `stop_local_residual_min_len_cm` 5 and `stop_snap_reachable` true are PDVD production. **Item 7 is closed.**
...
9. **The charge-based Michel energy still depends on the Michel's segmentation (doc 81; raised by the owner 2026-09-11, after doc 88).**
   - **The owner's intent.** The 2-D charge estimator exists so that the Michel energy does not depend on the Michel's trajectory fit. The fit is used only to predict, and subtract, the muon's charge.
   - **What doc 81 does.** It reads *measured* cell charge minus the muon's prediction. That removes the fit's dQ-sharing bias from today's `michel_ke_best`.
   - **Where it misses the intent: the cell selection.** A cell is the Michel's when it lies within 0.6 cm (in that plane) of the associated-point or fit cloud of a segment in the Michel object, or is covered by an admitted unfitted cluster's blobs. A segment's associated points come from a Voronoi partition of the cluster's points among its segments, then ghost removal (`clustering_points_segments`). So charge PR never gave a Michel segment is not counted:
     - a Michel with no segment (`michel_found` 0) gets nothing;
     - blob points partitioned to the muon's last segment are at most role-1 footprint;
     - points dropped by ghost removal are absent.
     On PDHD's cross-shared cells the estimator also falls back to the fit's own prediction (doc 81 §8a).
   - **How much this misses is not measured.** Doc 86 §10's "36 of 120 found Michels carry unfitted charge near the stop" is a 3-D point count, not an energy shortfall.
   - **Proposed definition (the owner decides).** The Michel charge per plane is Σ (measured − muon prediction) over *every* cell of the main cluster and the admitted companions inside a region around the stop: the 2-D projection of a sphere of radius R, with R set by a Michel electron's reach. The trajectory then enters only through the muon prediction and the stop position. Doc 81's three definition questions stay open: cross-shared cells, the charge-only class, and no truth anchor.
   - **First step, a measurement.** Add the unclaimed cells inside R to doc 81's cell table (role 0, a new default-OFF knob). Then measure per Michel how much measured-minus-muon charge sits in role-0 cells, split by class: found and missed Michels, doc 86 §8.1's unfitted-excess items, and item 7's kept residuals. That tells whether the association-based definition loses a meaningful amount before any definition is changed.
   - **The nominal association-based energy is not in use for the Michel, and it reads 0** (the owner's question, 2026-09-11).
     - *The nominal path.* The PR chain's energy from the point association is `cal_kine_charge` → `kine_charge_from_maps` (`NeutrinoEnergyReco.cxx`). It collects a 2-D cell's charge when the nearest point of the object's `associate_points` cloud (fallback: the fit cloud) is within 0.6 cm, then converts it.
     - *For the Michel.* `CheckSTM_Michel` stores it as `michel_ke_charge` (the Michel shower's `get_kine_charge()`), "for comparison, never best". The headline `michel_ke_best` is the fitted dQ/dx inversion plus the unfitted-piece charge.
     - *Measured on today's production* (`p88vprod`, `p88hoff`): nonzero on **10 of 171** found PDVD Michels and **5 of 124** PDHD ones. Where nonzero it is 0.08 / 0.39 / 3.4 (PDVD) and 0.08 / 0.20 / 4.1 (PDHD) of `michel_ke_best` (p10 / p50 / p90).
     - *Cause.* Doc pdhd/16 and the code comment attribute PDVD's zeros to the 2-D cell frame versus the t0-corrected cloud frame. PDHD reads 0 too, so the cause is not re-diagnosed here and may be broader, possibly the value never being computed for this shower.
     - *Relation to doc 81.* Doc 81's estimator is this same association rule with the frame handled (a per-face drift shift measured from the cloud's own points) and the muon subtracted. So "use the associated 3-D points" is what doc 81 implements, and item 9's segmentation question applies to it equally.
...
Doc 79: the cap. Doc 80: item 1. Doc 82: item 2 (with the corrections above; the peak-relative rule is dead, `split_kink_min_deg` 10 flipped). Items 3, 4 and 5 remain — and doc 82 §9 recommends **4 before 3**, since `039252_2/79` turns out to want a smaller stop correction than item 2 applied, and doc 80's census now makes the attached-arm population (rej 13) gradeable. Doc 83: item 4 (flipped: `michel_near_stop_arm_cm` 5). Doc 84: item 3 (flipped as a reach exemption: `moved_stop_michel_reach_min_cm` 6.5). Items 5 and 6 remain; 5 is next. Doc 85: item 5 (no P4 gate to loosen; the stop anchor is dead at 50 cm; `stop_gamma_require_stm` flipped: no capture gamma on a rejected candidate). Doc 86: item 6 (the owner's hand check: all four are Michels the chain misses, two collinear with the fit's tail and two unfitted blobs PR drops; none is unimaged). It opens items 7 and 8. **Item 7 is next**, the floored stop-local residual keep; item 8 waits for a blind re-judge of its 10 fires. Doc 87: item 7 (the floor built and OFF; the flip candidate gains both owner Michels but loses one stopper to a stop snap captured by the kept residual). **Item 7b is next**, the stop snap within the entry's component. Then the owner's look at `039253_15/36`, then item 8's re-judge. Doc 88: item 7b (`stop_snap_reachable` restores 36/63; the flip candidate is +2 Michels, 1 FP, 0 stoppers lost). The owner judged 15/36 a Michel, and the floored keep plus the reachable snap is FLIPPED in PDVD (doc 88 §9); **item 7 is closed**. The owner then asked whether the charge-based Michel energy meets its intent; it does not fully (item 9, above). Doc 88 §9.5 ranks what follows: **(1) a census of the 45 missed stoppers by the gate that rejects them; (2) the PDVD flips of docs 83–88 graded on PDHD's smx18 record; (3) item 9's role-0 measurement; (4) item 8's blind re-judge.** Doc 89: (1) done, read-only.
- **The count.** 55 missed on all 576 judged items: 45 by the chain and 10 the tagger never hands on.
- **The shape tests are not borderline.** `shape_flat` misses sit a median 0.038 past the margin, and `no_bragg` misses reach 0.71 of the required contrast.
- **The partition.**
  - 5 are cleared at 0 FP by two existing settings together: P1 floors of 3 MeV / 3 cm and `plateau_mip_hi` 2.0.
  - 6 carry a Michel under the 3 cm floor.
  - 22 are medium- or low-confidence shape-only verdicts to re-judge first.
  - 4 are high-confidence with no lever.
