...
For a higher-level overview of all scripts and input/output paths see [01_pdvd.md](01_pdvd.md).
...
- Algorithm pipeline: `switch_scope → extend → regular×2 → parallel_prolong → close → extend_loop(×3) → separate → neutrino → isolated` (`examine_bundles` disabled 2026-06-14 — it only rewrote the unused `perblob` array; see `23_clustering-scope.md` §Stage 3).
...
> `qlmatch/04_pdvd-crp-anode-plane-geometry.md`.
...
> `pdvd/docs/07_pdvd-tpc-geometry-fiducial.md`.
