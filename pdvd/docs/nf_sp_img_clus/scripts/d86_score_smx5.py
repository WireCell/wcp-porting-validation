#!/usr/bin/env python3
"""doc pdvd/86 sec 7 -- score the owner's smx5 answers against the key, and fold them
into a NEW merged record (the old one is never rewritten, M13).
...
Usage: STM_SCAN_RECORD=<smx1a+smx3+smx4 record> d86_score_smx5.py --labels L.json --key K.tsv
                                                    [--write-merged OUT]
"""
...
if not os.environ.get("STM_SCAN_RECORD", "").endswith("smx1a_smx3_smx4_verdicts.json"):
    sys.exit("STM_SCAN_RECORD must name the smx1a+smx3+smx4 record (doc 70)")
...
# the owner's words -> MECH (2026-09-11: "note the first two cases, the Michel seems to be
# aligned with the STM, there is a clear Bragg peak, and then a short one after it that
# could be the Michel electron. For the latter two events, there are some clear blob
# topology near the end of the stopping point of the muon. Those should be Michel
# electrons, but not identified by the PR chain at all.")
MECH = {"039349_43/66": "fit-through", "039349_72/11": "fit-through",
        "039253_0/44": "unfitted", "039349_30/45": "unfitted"}
...
def owner_row(k, v):
    ...
    row = dict(key=k, verdict=v["choice"], michel_kind=v.get("michel_kind"), confidence="owner",
               ...
               tags=tags, source="smx5 (owner, 2026-09-11)", pin=pin, mech=MECH[k])
...
for k, v in L.items():
    ...
    print("  %-13s %s%s" % (k, "; ".join(diff) or "no field change",
                           "; pin: the placed %s pin (rr %.1f) kept over an unplaced smx5 pin" % (
...
if a.write_merged:
    ...
    print("\nwrote %d merged records -> %s (%d from smx5)" % (len(rec1), a.write_merged, len(L)))
