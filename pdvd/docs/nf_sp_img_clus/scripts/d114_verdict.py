#!/usr/bin/env python3
"""doc pdvd/114 -- fork of d114_verdict.py: read every clause of the frozen rule figs/114_pred.txt off the committed output files and print the
verdict per level.  No number is computed here that a clause script did not print; this only parses and compares.

Inputs (figs/):
  114_steiner_<arm>.json            d114_steiner_census.py       S1, Gb, P5 (seed), terminal classes (reported)
  114_eval_<det>_<lvl>.txt          d111_eval.py --base <base>   P1, P2, P3, P4, P6'
  114_evalR_<det>_<lvl>.txt         d111_eval.py --base <base2>  R
  114_compare_<det>_<lvl>.txt       d114_compare.py --logd-base  Ga, Gc, Gd
  114_grade_<det>.txt               d114_grade.py                T
  114_spot.tsv                      d114_spot_figs.py            P5f
  /home/xqian/tmp/d113/arm_<arm>.out + run logs + work dirs     C (amendment 1: rc 0 and the three output files;
                                    the runner's summary and its INCOMPLETE events are written to 114_arms_complete.txt)

Usage: d114_verdict.py [--figs DIR] > figs/114_verdict.txt
"""
import argparse, glob, json, os, re, sys

IMG = "/home/xqian/toolkit-dev/wcp-porting-img"
ARMD = "/home/xqian/tmp/d114"


def completeness(det, arm):
    """figs/114_pred_amend1.txt: complete = no 'wire-cell PR failed' line in the arm's run logs, and every event dir
...
def main():
    ...
    comp_lines = []
    spot = {}
    for line in open(f"{F}/114_spot.tsv").read().splitlines()[1:]:
        f = line.split("\t")
        spot[(f[0], f[1])] = float(f[4])
    out = []
    verdict = {lvl: {} for lvl, _ in LEVELS}
    for det, x in DETS.items():
        base = json.load(open(f"{F}/114_steiner_d114{x}base.json"))["summary"]
        grade = open(f"{F}/114_grade_{det}.txt").read()
        out.append(f"## {det}: base d114{x}base -- S1 off {100*base['off']:.2f} %, far {100*base['far']:.2f} %, "
                   f"{SPOT[det]} seed max {max(base['spots'].get(SPOT[det], [float('nan')])):.2f} cm")
        for lvl, suf in LEVELS:
            arm = f"d114{x}{suf}"
            c = {}
            s = json.load(open(f"{F}/114_steiner_{arm}.json"))["summary"]
            rel = (s["off"] - base["off"]) / base["off"]
            c["S1"] = (rel <= -0.25, f"seed > 1 cm off {100*base['off']:.2f} -> {100*s['off']:.2f} % ({100*rel:+.1f} %, bar -25 %)")
            ev = eval_rows(f"{F}/114_eval_{det}_{suf}.txt")
            ...
            cmp_ = open(f"{F}/114_compare_{det}_{suf}.txt").read()
            ...
            c["W"] = (s["wig"] <= 1.10 * base["wig"], f"seed wiggle {100*base['wig']:.2f} -> {100*s['wig']:.2f} % (bar x1.10)")
            g2 = open(f"{F}/114_gate_off_{det}.txt").read()
            c["G2"] = ("RESULT PASS" in g2, re.search(r"RESULT \w+:[^\n]*", g2).group(0) if re.search(r"RESULT \w+:", g2) else "no result line")
            ...
            er = eval_rows(f"{F}/114_evalR_{det}_{suf}.txt")
            rr = num(find(er, "R")[1])
            c["R"] = (rr <= 1.20, f"median wall ratio vs the concurrent OFF arm {rr:.3f}")
            out.append(f"### {det} {lvl} ({arm})")
            for k in ("S1", "P1", "P5", "T", "W", "P2", "P3", "P4", "P6'", "P5f", "Ga", "Gb", "Gc", "Gd", "C", "R", "G2"):
                ok, txt = c[k]
    ...
    open(f"{F}/114_arms_complete.txt", "w").write("\n".join(comp_lines) + "\n")
    print("\n".join(out))
