#!/usr/bin/env bash
# doc pdvd/84 -- doc 78 action item 3, the moved-stop veto's reach exemption.
#   Fork by duplication (CLAUDE.md M10) of d83_gates.sh: same zip / tree / branch
#   comparisons; section 3 names every candidate on which ANY production branch or
#   point-row set moved, with the T2c fields (kink, reach, KE, veto counters).
#   Every arm is BARE PRODUCTION plus this round's key (no d53 survey bag).
#   OFF: p84voff vs p83vprod (PDVD production after the doc 83 flip),
#        p84hoff vs p83hoff (PDHD) -- every Bee zip member, calib json, every
#        tracking-pr.root tree, every T_stm_michel branch and point row.
#   ON : p84vr65 / p84vr8 / p84vr5 / p84vk59 vs p83vprod; the is_stm and michel_found
#        census against the record.
# Usage: bash d84_gates.sh > /home/xqian/tmp/p84/gates.log 2>&1
set -u
IMG=/nfs/data/1/xqian/toolkit-dev/wcp-porting-img
W=/home/xqian/tmp/p84
X=$IMG/pdvd/docs/nf_sp_img_clus/scripts
REC=$IMG/pdvd/docs/scan/pdvd_stm_michel_smx1a_smx3_smx4_verdicts.json
PROD=${PROD:-p83vprod}          # the production baseline arm (post doc-83 flip)
VOFF=${VOFF:-p84voff}; HOFF=${HOFF:-p84hoff}; HBASE=${HBASE:-p83hoff}
ARMS=${ARMS:-"p84vr65 p84vr8 p84vr5 p84vk59"}
cd $IMG

echo "=== 0. completeness, pins, markers"
for spec in "pdvd $PROD /home/xqian/tmp/p83" "pdvd $VOFF $W" "pdhd $HBASE /home/xqian/tmp/p83" "pdhd $HOFF $W"; do
...
NEW = {"n_michel_veto_reach_exempt"}
def read(arm):
    out = {}; pts = {}
    for d in sorted(glob.glob("pdvd/work/*_" + arm)):
        ...
        for i in range(len(t["cluster_id"])):
            ...
            pts[k] = collections.Counter((int(r), int(s)) for s, r in zip(q["seg_id"][sel], q["role"][sel]))
    return out, pts
def same(a, b):
    try:
...
for arm in ARMS:
    A, AP = read(arm)
    rx = sum(int(g(a, "n_michel_veto_reach_exempt")) for a in A.values())
    kx = sum(int(g(a, "n_michel_veto_exempt")) for a in A.values())
    vv = sum(int(g(a, "n_michel_veto")) for a in A.values())
    print("  --- %s vs %s: %d candidates; T2c vetoes %d, kink-exempt %d, reach-exempt %d (production: %d / %d / 0)" % (
        arm, PROD, len(A), vv, kx, rx, sum(int(g(p, "n_michel_veto")) for p in P.values()),
        sum(int(g(p, "n_michel_veto_exempt")) for p in P.values())))
    only = sorted({b for a in A.values() for b in a} - {b for p in P.values() for b in p})
    print("    branches only in %s: %s (expected: %s)" % (arm, only, sorted(NEW) if "r" in arm[3:] else []))
    movers = []
    for k, a in sorted(A.items()):
        ...
        pa, pp = AP[k], PP.get(k, collections.Counter())
        roles = sorted({r for (r, s) in set(pa) | set(pp) if pa.get((r, s)) != pp.get((r, s))})
        if bs or roles: movers.append((k, bs, roles))
    for k in sorted(set(P) - set(A)): print("    DROPPED candidate %s" % k)
    for k, bs, roles in movers:
        a, p = A[k], P[k]
        print("    %-14s %-11s kink %.2f reach %.1f cm KE %.2f MeV | conn %d->%d michel_found %d->%d is_stm %d->%d bits %d->%d | veto %d->%d kink-ex %d->%d reach-ex %d" % (
            k, tag(k), float(a["michel_kink_deg"]), float(a["michel_len"]) + float(a["michel_far_len"]), float(a["michel_ke_best"]),
            int(p["michel_conn_type"]), int(a["michel_conn_type"]), int(p["michel_found"]), int(a["michel_found"]),
            int(p["is_stm"]), int(a["is_stm"]), int(p["reject_bits"]), int(a["reject_bits"]),
            int(g(p, "n_michel_veto")), int(g(a, "n_michel_veto")), int(g(p, "n_michel_veto_exempt")), int(g(a, "n_michel_veto_exempt")),
            int(g(a, "n_michel_veto_reach_exempt"))))
        print("        branches moved: %s | point-row roles moved: %s" % (bs, roles))
        for b in bs:
            if b not in ("michel_conn_type", "michel_found", "n_michel_veto", "n_michel_veto_exempt"):
                print("          %s: %s -> %s" % (b, P[k].get(b), a.get(b)))
    print("    candidates moved: %d" % len(movers))
