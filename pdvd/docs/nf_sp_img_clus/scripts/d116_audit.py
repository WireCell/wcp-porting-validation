#!/usr/bin/env python3
"""doc pdvd/116 sec 5 (fork by duplication of d116_audit.py, untouched; round root /home/xqian/tmp/d116) -- audit one blind scanner's transcript for any read that would break the blind.

    python3 d116_audit.py TRANSCRIPT.jsonl DET OWN_SLOT      (OWN_SLOT e.g. w1_a3)
    python3 d116_audit.py --selftest                          (negative control: every bad line must be flagged)

Fork by duplication of pdhd/docs/scan/h25/d25_audit.py (untouched).  Changed: the round lives in
/home/xqian/tmp/d116/round_<det>/; a scanner may read only RUBRIC.md, AGENT_TASK.md, shots/, its own waves/<slot>.txt,
...
def bad_patterns(det, own, rname=None):
    ...
    B = [R + r'/items', R + r'/round/', R + r'/round"', R + r'/round$', R + r'/pred', R + r'/doc_draft', R + r'/v1_',
         ...
         R + r'/%s/logs' % rn, R + r'/%s/reshoot' % rn, r'/home/xqian/tmp/p100',
...
def audit(lines, det, own, rname=None):
    ...
    for line in lines:
        ...
        for c in content:
            ...
            probe = re.sub(r'cd /home/xqian/tmp/d116/%s/?\s*&&' % re.escape(rn), '', inp)
...
if sys.argv[1:] == ["--selftest"]:
    R = "/home/xqian/tmp/d116/round_pdhd"
    ...
    bad = [tu({"file_path": "/home/xqian/tmp/d116/items/pdhd_items.tsv"}),
           ...
           tu({"command": "cat /home/xqian/tmp/d116/round/prep_d102hcs/smprep-029107_1-c85.json"}),
           ...
           tu({"command": "cat /home/xqian/tmp/d116/pred.txt"}), tu({"command": f"ls {R}/scratch/w1_a1"}),
           tu({"command": "cat /home/xqian/tmp/d116/round_pdvd/RUBRIC.md"})]
    ...
    R2 = "/home/xqian/tmp/d116/round_pdvd2"          # sec 10: a named round dir (4th argument)
    ...
    bad2 = [tu({"command": "cat /home/xqian/tmp/d116/round_pdvd/RUBRIC.md"}), tu({"command": f"ls {R2}"}),
            tu({"command": "cat /home/xqian/tmp/p100/carry_r2/latest_on_p99rwon_corrected.json"}),
            tu({"command": "cat /home/xqian/tmp/d116/own/set_pdhd_smx27/manifest.txt"}),
            tu({"command": f"cat {R2}/set/sheet_d103v1.txt"}), tu({"command": "ls /home/xqian/tmp/d116/pdvd_prod"}),
