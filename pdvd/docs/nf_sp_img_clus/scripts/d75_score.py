#!/usr/bin/env python3
"""doc pdvd/75 (P1b) -- grade the P1b arms on the owner's record, tags and pins.

Read-only over the arms' preps and the grading record.  Fork of d74_score.py
(sections B-F are its code).

  A  the verdict-stage reading against the OFF arm: every candidate where
     bragg_anchor_fallback fired or where is_stm / reject_bits / contrast /
     ks_mu / ks_flat / plateau_med changed -- off -> on, with the record's
     verdict -- and a count of candidates where anything ELSE changed (the stop,
     the chain, the Michel object: must be 0, the rule is verdict-only)
  ...
  G  prediction vs arm: d75_sizing.py's fire lists against the C++'s bragg_anchor_fallback

Usage:
  STM_SCAN_RECORD=.../pdvd_stm_michel_smx1a_smx3_smx4_verdicts.json \\
  python3 d75_score.py --off p75voff=/home/xqian/tmp/p75/prep_p75voff \\
      --on p75vfb=/home/xqian/tmp/p75/prep_p75vfb:fb ... [--pred pred.json]
"""
...
ap.add_argument("--on", action="append", default=[], help="NAME=PREP[:variant], variant in fb / fb0; repeatable")
ap.add_argument("--pred", default=None, help="d75_sizing.py --json output")
...
# ---------------------------------------------------------------- A. the verdict-stage reading
print("=== A. the verdict-stage reading against %s, off -> on ===" % off)
FIELDS = ("is_stm", "reject_names", "bragg_anchor_shift_cm", "contrast", "contrast_expected", "plateau_med", "ks_mu", "ks_flat", "michel_found", "bragg_anchor_fallback")
OTHER = ("stop_dis", "n_chain_segs", "n_retreat", "n_split", "michel_conn_type", "michel_len", "michel_ke_best", "michel_seg_id", "n_michel_veto", "stop_x", "stop_y", "stop_z")
...
for nm, _, var in ARMS[1:]:
    ...
    for k in keys:
        ...
        diff = [f for f in FIELDS if f != "bragg_anchor_fallback" and not same(x.get(f, 0), y.get(f, 0))]
        oth = [f for f in OTHER if not same(x.get(f, 0), y.get(f, 0))]
        if oth:
            other_changed.append((k, oth))
        if diff or y.get("bragg_anchor_fallback", 0):
            ch.append((k, diff, x, y))
    print("\n%s (%s): %d candidates where the reading changed or the fallback fired; only-%s %s, only-%s %s"
          % (nm, var, len(ch), off, sorted(set(PALL[off]) - set(PALL[nm])) or "-", nm, sorted(set(PALL[nm]) - set(PALL[off])) or "-"))
    print("   %-14s %-22s %s" % ("item", "record", " | ".join(f.replace("bragg_anchor_", "").replace("contrast_expected", "expected") for f in FIELDS)))
    for k, diff, x, y in ch:
        cells = []
        for f in FIELDS:
            xs, ys = fmt(x, f), fmt(y, f)
            cells.append(ys if xs == ys else "%s->%s" % (xs, ys))
        print("   %-14s %-22s %s" % (k, rv(k), " | ".join(cells)))
    print("   candidates where a NON-verdict field changed (stop, chain, Michel object; must be 0): %d %s"
          % (len(other_changed), other_changed[:10]))
    summary.setdefault("reading", {})[nm] = dict(changed=[(k, diff) for k, diff, _, _ in ch], other_changed=other_changed)
...
if a.pred:
    ...
    print("\n=== G. prediction (d75_sizing.py, before launch) vs the arm's bragg_anchor_fallback ===")
    for nm, _, var in ARMS[1:]:
        if var not in PR:
            continue
        fired = sorted(k for k, p in PALL[nm].items() if p["verdict"].get("bragg_anchor_fallback", 0))
        pred = set(PR[var]["fires"]); got = set(fired)
        gain_pred = set(PR[var]["is_stm_gain"])
        gain_got = set(k for k in fired if k in PALL[off] and not PALL[off][k]["verdict"]["is_stm"] and PALL[nm][k]["verdict"]["is_stm"])
        print("%s (%s, rise %.2f): predicted fires %d, fired %d | both %s | predicted only %s | fired only %s"
              % (nm, var, PR[var]["rise"], len(pred), len(got), sorted(pred & got) or "none", sorted(pred - got) or "none", sorted(got - pred) or "none"))
        print("   is_stm 0->1 through the fallback: predicted %s, arm %s" % (sorted(gain_pred) or "none", sorted(gain_got) or "none"))
        summary.setdefault("prediction", {})[nm] = dict(both=sorted(pred & got), pred_only=sorted(pred - got), fired_only=sorted(got - pred), gain=sorted(gain_got))
