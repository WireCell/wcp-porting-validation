#!/usr/bin/env python3
"""doc pdvd/116 sec 5 -- fork by duplication of d103_scan_record.py (untouched): the calibration check reads the doc-113
truth precedence (d113_grade.truth) instead of one record, and the source_round names this round.  Original docstring:
...
Usage: d116_scan_record.py --det pdhd --round /home/xqian/tmp/d103/round_pdhd --items /home/xqian/tmp/d103/items/pdhd_items.tsv
...
def old_truth(det):
    """the doc-113 precedence (own103h2 > own103h > smx27 > smx28; own103v2 > own103v > carried > smx11): (verdict, source)"""
    import sys as _s
    _s.dont_write_bytecode = True
    _s.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    import d113_grade as G
    T, _ = G.truth(det)
    return {k: (v, src) for k, (v, mk, src) in T.items()}
...
def main():
    ...
    for f in sorted(glob.glob(os.path.join(a.round, "v_parts", "*", "*.json"))):
        ...
        if r["key"] in recs:
            ...
            continue
        recs[r["key"]] = r
    print(f"# doc pdvd/116 blind scan record ({a.det}, tag {a.tag}); round {a.round}")
    ...
    if a.record_out:
        ...
        for k in sorted(recs):
            ...
            v.update(display_arm=items[k]["display_arm"], calibration=items[k]["role"] == "calibration", tag=a.tag,
                     source_round="doc pdvd/116 verdict-blind Opus agent scan")
            out.append(v)
