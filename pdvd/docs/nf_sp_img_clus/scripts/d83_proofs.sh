#!/usr/bin/env bash
# doc pdvd/83 -- the compiled-config proofs for the michel_near_stop_arm_cm flip in
# pdvd/wct-pr-perevt.jsonnet.  PRE_EVT = a copy of the file BEFORE the flip; VAL = the
# flipped value (the arm it came from ran `-S stm_michel_extra={michel_near_stop_arm_cm:VAL}`).
#   A  PRE + `-S stm_michel_extra={michel_near_stop_arm_cm:VAL}` (the arm's config) vs POST unset -> 0 lines
#   B  POST + `-S stm_michel_extra={michel_near_stop_arm_cm:0.0}` (the C++ default forced back) vs PRE
#      -> exactly the one key, present at 0 vs absent (a value knob has no key-suppression form)
#   C  PRE vs POST -> exactly the one key
#   D  PDHD untouched: the key is not in the compiled PDHD config
# Fork by duplication (CLAUDE.md M10) of d82_proofs.sh; only the key and D changed.
set -u
: "${PRE_EVT:?}"; : "${VAL:?}"
D=/nfs/data/1/xqian/toolkit-dev/wcp-porting-img/pdvd
H=/nfs/data/1/xqian/toolkit-dev/wcp-porting-img/pdhd
W=/home/xqian/tmp/p83/proofs; mkdir -p $W
...
comp pre      "$PRE_EVT"
comp pre_on   "$PRE_EVT" -S "stm_michel_extra={michel_near_stop_arm_cm:$VAL}"
comp post     wct-pr-perevt.jsonnet
comp post_off wct-pr-perevt.jsonnet -S 'stm_michel_extra={michel_near_stop_arm_cm:0.0}'
...
echo "   michel_near_stop_arm_cm in the compiled POST: $(grep -c '"michel_near_stop_arm_cm"' $W/post.json) key(s): $(grep -o '"michel_near_stop_arm_cm" *: *[0-9.]*' $W/post.json | sort -u)"
echo "D  PDHD: michel_near_stop_arm_cm in pdhd/wct-pr-perevt.jsonnet: $(grep -c michel_near_stop_arm_cm $H/wct-pr-perevt.jsonnet) line(s)"
echo PROOFS_DONE
