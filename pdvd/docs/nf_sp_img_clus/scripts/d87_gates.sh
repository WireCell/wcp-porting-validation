#!/usr/bin/env bash
# doc pdvd/87 -- doc 78 action item 7: a size floor on doc 62 T3a's stop-local
# residual keep (stop_local_residual_min_points / _min_len_cm).
#   Fork by duplication (CLAUDE.md M10) of d85_gates.sh: same zip / tree / branch
#   comparisons; section 3 names every candidate on which ANY production branch or
#   point-row set moved, with the keep counters, and names every mover WITHOUT a keep
#   fire (the flip bar's "bit-identical elsewhere"); section 5 runs d87_keep_census.py.
#   Every arm is BARE PRODUCTION plus this round's keys (no d53 survey bag).
#   OFF: p87voff vs p85vprod (PDVD production after the doc 85 flip),
#        p87hoff vs p85hoff (PDHD).
#   ON : p87v5f (flip candidate), p87v5, p87v20, p87v20f vs p85vprod.
#   The record is the MERGED smx1a+smx3+smx4+smx5 one (doc 86): d85_gates.sh's REC
#   predates smx5, and on it 039253_0/44 grades as an FP.
# Usage: bash d87_gates.sh > /home/xqian/tmp/p87/gates.log 2>&1
set -u
IMG=/nfs/data/1/xqian/toolkit-dev/wcp-porting-img
W=/home/xqian/tmp/p87
...
VOFF=${VOFF:-p87voff}; HOFF=${HOFF:-p87hoff}; HBASE=${HBASE:-p85hoff}
ARMS=${ARMS:-"p87v5f p87v5 p87v20 p87v20f"}
cd $IMG
echo "record: $REC"
