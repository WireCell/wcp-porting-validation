...
def trace(k):
    v = oa[k]["_v"]
    return "ext=%s stub=%s kept=%s local=%s rveto=%s veto=%s conn=%s dis=%.2f ke=%.2f rej=%s" % (v.get("n_ext"), v.get("n_stub_absorb"),
        v.get("n_kept_near_stop_main"), v.get("n_local_pieces"),
        v.get("n_michel_range_veto"), v.get("n_michel_veto"), v.get("michel_conn_type"),
        v.get("michel_dis_cm") or 0, v.get("michel_ke_best") or 0, v.get("reject_names"))
...
for f in ("n_kept_near_stop_main", "n_kept_near_stop_comp", "n_local_pieces", "n_michel_range_veto", "n_michel_veto", "n_ext", "n_stub_absorb"):
