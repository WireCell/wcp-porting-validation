#!/bin/bash
# doc pdvd/102 P4 -- FORK BY DUPLICATION of d101_sim_chain.sh (untouched): the same sim -> NF -> SP ->
# imaging -> clustering chain for the near-isochronous muon set (d101_make_muons.py --theta 65,75,85,89,
# truth in /home/xqian/tmp/d102/sim).  Differences from d101, all bookkeeping: run numbers 900103 (PDVD) /
# 900104 (PDHD), TAG default d102, SIMROOT and PIN defaults under /home/xqian/tmp/d102.  The sidecar file
# names d101-sim-provenance.txt / d101-time-offset.txt are KEPT so the forked PR arm reads them unchanged;
# the derived time offsets are the same numbers (same compiled simulation config, asserted below).
#
...
SIMROOT=${SIMROOT:-/home/xqian/tmp/d102/sim}
PIN=${PIN:-/home/xqian/tmp/d102/libpin_d102}
...
case $DET in
    pdvd) RUN=900103; V=""; SPNAME=protodune-sp-frames; DETDIR=$WCP/pdvd
          DERIVED_TOFF_NS=-226598; EXP="frame_t0_ns=-249933.673 sp_ctoffset_ns=4000 drift_speed_mmus=1.568 response_x_mm=-3191.6375" ;;
    pdhd) RUN=900104; V="-V elecGain=14"; SPNAME=protodunehd-sp-frames; DETDIR=$WCP/pdhd
...
if [[ " $STAGES " == *" img "* ]]; then
    ...
    { echo "doc=pdvd/102 near-isochronous simulated muon (d102_sim_chain.sh)"; echo "det=$DET k=$K anode=$ANODE sim_seed=$SEED";
