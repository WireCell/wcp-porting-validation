...
- **15 of the offline twin's geometric contrasts** differ from the payload's by up to 0.68 for no identified reason (§2), five of them near the cut. They did not matter here (the prediction matched the arm on every replayable chain), but the twin should not be trusted on those five. *(Doc 75 §2 found the cause: the payload's `L` is rounded to 0.01 cm, so a row at exactly rr = 3.0 / 20 / 40 cm can fall on the other side of a window edge than in the C++.)*
