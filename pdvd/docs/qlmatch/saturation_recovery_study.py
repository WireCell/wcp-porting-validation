...
Repro (order matters -- 1. makes the validation input for 2.):
  ...
  2. cd pdvd/docs/qlmatch && python3 saturation_recovery_study.py --all
     (or --validate / --study / --realrail / --prevalence)

Methods compared on clipped pulses (see pdvd-saturation-recovery.md):
...
def _find_templates():
    ...
    cands.append(os.path.join(HERE, "../../../../toolkit/cfg", rel))
    for c in cands:
        if os.path.exists(c):
            return c
...
def study(args):
    ...
    ax.legend()
    fig.tight_layout()
    out = os.path.join(HERE, "saturation_recovery_bias.png")
