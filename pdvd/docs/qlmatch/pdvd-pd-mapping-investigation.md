...
python3 docs/qlmatch/pd_mapping_audit.py            # parts 1-3 (documentation + dump-level tests)
python3 docs/qlmatch/pd_mapping_audit.py --raw      # + parts 4-5 (raw-waveform adjudication, prevalence)
```
...
- **B — official `PDVD_PDS_Mapping_v04152025.json`**: geometry-channel
  ...
  (wrong) layout (`pdvd-photon-model.md` §2), so the literal reading is
...
Full trace (previous audit + this round, `pdvd-photon-models.md` §5,
...
- The official-map v4-literal mirror reading is an artifact of the mirrored
  ...
  from raw data, the `pdvd-photon-model.md` §2 open item is **closed**
