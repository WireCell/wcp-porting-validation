#!/usr/bin/env python3
"""doc qlmatch/32 -- blindness audit of one scanner agent's transcript (JSONL).  Lists every tool call the agent made
...
RUBRIC, or the recorder script.  Prints only the flagged calls and a summary (never the transcript itself).
    ...
    python3 d32_audit.py <transcript.jsonl> <wave> [--scan-root /home/xqian/tmp/p32/scan/r1q]
"""
import argparse
import json
import re

REC = "docs/qlmatch/scripts/d32_record.py"
...
def main():
    ...
    ap.add_argument("--scan-root", default="/home/xqian/tmp/p32/scan/r1q")
    a = ap.parse_args()
