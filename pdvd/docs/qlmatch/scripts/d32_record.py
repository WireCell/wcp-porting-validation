#!/usr/bin/env python3
"""doc qlmatch/32 -- the only writer of blind-scan verdicts.  Appends one JSON line to
...
sheet's candidate letters, and refusing a second verdict for the same id.
    ...
    python3 d32_record.py --wave 0 --id s12345 --pick B --conf high --reason "..."
"""
import argparse
import datetime
import json
...
SCAN_ROOT = os.environ.get("D32_SCAN_ROOT", "/home/xqian/tmp/p32/scan/r1q")
...
def main():
    ...
    pick = a.pick.strip()
    if pick not in ("none", "unsure") and (len(pick) != 1 or pick not in idx[a.id]):
        sys.exit(f"REFUSED: pick '{pick}' not one of {list(idx[a.id])} / none / unsure")
