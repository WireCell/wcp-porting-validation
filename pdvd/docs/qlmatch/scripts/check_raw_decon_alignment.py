...
02_pdvd-anode-time-consistency.md section 8.13.
...
Repro:
    ...
    python3 scripts/check_raw_decon_alignment.py   # writes raw_decon_align_*.png + prints table
"""
...
_PICS = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "pics")  # scripts/ -> qlmatch/pics
...
for run6, idx, anode, side in CASES:
    ...
    for r, tpk in pick_isolated(fr, wset, N_PER_CASE):
        ...
        fig.tight_layout()
        out = os.path.join(_PICS, "raw_decon_align_%s_a%d_ch%d.png" % (evt, anode, ch))
