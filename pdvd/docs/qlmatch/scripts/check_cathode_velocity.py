...
Follows the anode/T0 calibration of 02_pdvd-anode-time-consistency.md sec 8.9:
...
Usage (from this directory):
  python3 scripts/check_cathode_velocity.py                       # _ctoff (sec 8.12)
  python3 scripts/check_cathode_velocity.py --tag anodefix        # historical sec 8.10
...
_SDIR = os.path.dirname(os.path.abspath(__file__))          # scripts/ (data outputs)
_PICS = os.path.join(os.path.dirname(_SDIR), "pics")        # qlmatch/pics (figures)
...
def main():
    ...
    with open(os.path.join(_SDIR, "cathode_velocity_tracks_%s.json" % TAG), "w") as f:
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(_PICS, "cathode_velocity_stops_%s.png" % TAG), dpi=110)
    print("\nwrote", os.path.join(_PICS, "cathode_velocity_stops_%s.png" % TAG))
    return summary
