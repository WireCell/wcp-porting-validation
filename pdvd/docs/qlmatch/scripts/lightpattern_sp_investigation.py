...
Repro (all read-only; run from pdvd/docs/qlmatch):
    python3 scripts/lightpattern_sp_investigation.py            # tables + figures
    python3 scripts/lightpattern_sp_investigation.py --tables   # tables only
    python3 scripts/lightpattern_sp_investigation.py --figs     # figures only
...
Findings documented in 14_pdvd-lightpattern-sp-investigation.md:
...
membrane 1.0, PMT 3.5 -- pdvd/docs/04_pdvd-light-filter.md).
"""
...
sys.path.insert(0, os.path.join(HERE, "..", "..", "..", "ql_light_calib"))  # scripts/ -> pdvd/ql_light_calib
...
def fig1_raw_decon(recs, tmpl, d, f):
    ...
    fig.tight_layout()
    out = os.path.join(os.path.dirname(HERE), "pics", "lightpattern_fig1_raw_decon_cathode.png")
...
def fig2_od7_templates(recs, tmpl, d, f):
    ...
    fig.tight_layout()
    out = os.path.join(os.path.dirname(HERE), "pics", "lightpattern_fig2_od7_template_bug.png")
    fig.savefig(out, dpi=130)
    print("wrote", out)
...
def fig3_ch2020(recs, tmpl):
    ...
    fig.suptitle("OpDet 2 = ch2020 + ch2021: ch2020's negative-area template "
                 ...
                 fontsize=10)
    fig.tight_layout()
    out = os.path.join(os.path.dirname(HERE), "pics", "lightpattern_fig3_ch2020_inversion.png")
    fig.savefig(out, dpi=130)
    print("wrote", out)
...
def fig4_coverage(recs, ch2od, d, f):
    ...
    axr.set_title("green = snippet covers flash\ngrey = NO DATA (scored 0!)",
                  fontsize=8)
    fig.tight_layout()
    out = os.path.join(os.path.dirname(HERE), "pics", "lightpattern_fig4_coverage_timeline.png")
    fig.savefig(out, dpi=130)
    print("wrote", out)
...
def fig5_pattern(d, f, pred):
    ...
    fig.tight_layout()
    out = os.path.join(os.path.dirname(HERE), "pics", "lightpattern_fig5_pattern.png")
    fig.savefig(out, dpi=130)
    print("wrote", out)
