...
This doc **synthesizes** material already spread across `10_pdvd-ql-pending.md §1`,
`02_pdvd-anode-time-consistency.md §3/§6/§8.13`, `05_pdvd-flash-dt.md`, and
`03_pdhd-anode-time-consistency.md`; it does not duplicate their derivations. Its new content
...
python3 docs/qlmatch/scripts/trace_light_charge_time.py
# other event / flash:
python3 docs/qlmatch/scripts/trace_light_charge_time.py work/039252_6/calib-evt298651.json 88
```
...
(`02_pdvd-anode-time-consistency.md §8.14`, `check_anode_stop_ensemble.py`): take
...
- `10_pdvd-ql-pending.md §1` — raw-DAQ derivation, per-run offset summary, beam-flash closure
  (99/120 events, residual −0.9 µs).
- `02_pdvd-anode-time-consistency.md` — **§3** full clock chain formula-by-formula; **§3.4** the
...
- `05_pdvd-flash-dt.md` — per-PD-group time coincidence (≤64 ns, no per-type correction needed).
- `03_pdhd-anode-time-consistency.md` — the PDHD precedent (constant 249.808 µs offset, measured
  not fitted).
