...
wcsonnet -A input_file=$PWD/input_data_light_rawwf/np02vd_raw_run039252_1176_df-s03-d3_dw_0_20250830T054542_rawwf.root \
         ...
         -o /home/xqian/tmp/satstudy/decon-dump.json docs/qlmatch/wct-decon-dump.jsonnet
wire-cell -l stderr -c /home/xqian/tmp/satstudy/decon-dump.json
cd docs/qlmatch && python3 saturation_recovery_study.py --all
```

Context: `pdvd-pd-mapping-investigation.md` §6-7 established that the DAPHNE
...
3. **The physics-critical fix is flag propagation** (option iv of
   `pdvd-pd-mapping-investigation.md` §7): keep the (clipped or repaired) hit
...
4. Recommended operating point (all default-OFF knobs):
   ...
   recalibration (companion doc `pdvd-qtol-recalibration.md`) must likewise
