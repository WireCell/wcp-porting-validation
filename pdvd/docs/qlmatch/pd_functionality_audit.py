...
pdvd-questions-dune.md).

Repro:
    cd pdvd/docs/qlmatch
    python3 pd_functionality_audit.py            # runs 039252/253/349, _satrep dumps
    # writes pdfunc_perchannel.tsv and pdfunc_*.png next to this script,
    # prints all tables quoted in pdvd-pd-functionality-run39252.md.
"""
...
PDVD = os.path.dirname(os.path.dirname(HERE))        # .../pdvd
...
def make_figs(res, pr0):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "pdfunc_liveness.png"), dpi=110)
    plt.close(fig)
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "pdfunc_wallxa_topbottom.png"), dpi=110)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "pdfunc_crossrun.png"), dpi=110)
    plt.close(fig)
