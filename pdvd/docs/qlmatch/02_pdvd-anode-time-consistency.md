...
> `04_pdvd-crp-anode-plane-geometry.md` for the current record. The examination logic
...
> (`03_pdhd-anode-time-consistency.md`), the FV anode convention was corrected
...
python3 scripts/check_anode_time_consistency.py            # all checks, per-side-corrected times
python3 scripts/check_anode_time_consistency.py --raw-top  # reproduce the dump-time (BDE-folded) view
```
...
| **−335.835 cm** | "grid" plane, `apa_plane = 0.5·apa_g2g = 57.15 mm` | the FV anode edge everywhere: QLMatching `run.anode_x` (via `inner_bounds` → `IAnodeFace::sensitive()`, whose x span is exactly the xregions `anode`↔`cathode` interval, `gen/src/AnodePlane.cxx:282-317`, `aux/src/DetectorVolumes.cxx:187-192`); clus per-drift `dvm` `FV_xmin = −3358.35 mm` (`clus.jsonnet:96`); the calib-dump `geometry.anode_x`; the u=0 face of `compute_two_boundary_flag` | `params.jsonnet:38-40` — `apa_g2g = 114.3 mm` is copied from the ProtoDUNE-SP DocDB 203 wire stack (see `07_pdvd-tpc-geometry-fiducial.md` §3); **it does not describe the PDVD CRP**, whose planes are 0.2 mm apart (above) |
...
**PDHD comparison** (`03_pdhd-anode-time-consistency.md` §2.3: `speed = 1.565`
...
binning) — see `pdvd/docs/10_pdvd-ql-pending.md` §1. There is no equivalent
...
cd docs/qlmatch && python3 scripts/check_anode_time_consistency.py --tag anodefix
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_flagged_boundary.py --tag anodefix   # current U-plane result
...
Repro: `python3 scripts/check_anode_time_consistency.py --tag anodefix`, checks
B/C/D.
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_anode_gap_imglevel.py
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_chain_consistency.py     # 5 cases; writes chain_<evt>_<uid>.png
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_anode_t0_ensemble.py    # writes prof_<evt>_<uid>.png profiles
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_cathode_velocity.py     # writes cathode_velocity_stops.png
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_anode_stop_ensemble.py --tag anodefix --sample decisions   # pre-shift (§8.9 method, all validated boundary+crosser)
python3 scripts/check_anode_stop_ensemble.py --tag ctoff --sample bundles --margin 4.5   # post-shift (re-clustered => bundle scan)
python3 scripts/check_anode_stop_verify.py --gate 3   # endpoint A/B: same tracks anodefix->ctoff, same flash (historical, needs _anodefix)
python3 scripts/check_anode_time_consistency.py --tag anodefix   # A' unbiased 3-D imaging edge (all-plane, contrast)
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_cathode_velocity.py --tag ctoff   # (now the default tag)
...
cd docs/qlmatch
python3 scripts/check_cathode_velocity.py  --tag vcal            # cathode closure
python3 scripts/check_anode_stop_ensemble.py --tag vcal --sample bundles --margin 3
```
...
```
cd pdvd/docs/qlmatch
python3 scripts/check_raw_decon_alignment.py   # writes raw_decon_align_*.png + the table below
```
...
+2.6 µs TDE, the +4 signature of §8.13's `05_raw_decon_alignment_plots.md`), and
...
```bash
cd pdvd/docs/qlmatch
python3 scripts/check_anode_stop_ensemble.py --tag v153 --sample bundles --margin 3    # tight toucher gate
python3 scripts/check_anode_stop_ensemble.py --tag v153 --sample bundles --margin 4.5
python3 scripts/check_anode_stop_ensemble.py --tag v153 --sample bundles --margin 12   # unbiased; take histogram PEAK
python3 scripts/check_anode_time_consistency.py --tag v153                             # A' unbiased 3-D imaging edge
...
cd docs/qlmatch
python3 scripts/check_anode_stop_ensemble.py --run 039253 --tag v153 --sample bundles --margin 12
python3 scripts/check_anode_stop_ensemble.py --run 039349 --tag v153 --sample bundles --margin 12
```
...
- `pdvd/docs/07_pdvd-tpc-geometry-fiducial.md` — three-source geometry
  reconciliation, cathode-thickness provenance (§3), FV-definition survey.
- `pdvd/docs/10_pdvd-ql-pending.md` §1 — trigger-offset resolution record and
  beam-flash closure.
