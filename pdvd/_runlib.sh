#!/bin/bash
# Shared helper library for pdvd/run_*.sh scripts.
# Source after setting PDVD_DIR:
#   . "$PDVD_DIR/_runlib.sh"
...
discover_events() {
    ...
    [ -z "$run_stripped" ] && run_stripped=0
    {
        ...
        ls -d "$PDVD_DIR/work/${run_padded}_"* 2>/dev/null \
...
#   batch_summary; exit $?
#
# Concurrency cap: ${PDVD_MAX_JOBS:-$(nproc)}.

batch_init() {
    BATCH_OK=0
    BATCH_FAIL=0
    BATCH_FAIL_LIST=()
    BATCH_MAX=${PDVD_MAX_JOBS:-$(nproc)}
