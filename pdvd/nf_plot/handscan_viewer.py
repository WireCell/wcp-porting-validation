"""Bokeh hand-scan viewer for Stage-A round-1 candidates (PDVD).

PDVD-adapted analog of ``pdhd/nf_plot/handscan_viewer.py``: same UI,
same data flow, but with PDVD-correct ``PLANE_NAMES`` (U / V only — no
W) and edge-of-readout-window thresholds tuned to PDVD's longer
~11848-tick readout. See that file for the upstream commentary on the
Bokeh 3.9 quirks worked around (Tabs / DataTable autosize_mode /
DataRange1d).
...
serves a clickable review tool. Layout is a single scrollable column:
  ...
  - "edge group" dropdown to scan boundary ROIs
    (``start_tick <= 10`` or ``end_tick >= 11838``) separately from
    interior ROIs.
...
Run via the wrapper::
    ...
    ./pdvd/nf_plot/serve_handscan_viewer.sh \\
        experiments/stage_a_pu_round1_pdvd/handscan
...
PLANE_NAMES = ["U", "V"]  # PDVD has only induction U and V planes

# Frame-edge thresholds. PDVD readout is ~11848 ticks (verified against
# consolidated_pdvd/data.h5 — max end_tick = 11847). An ROI is "edge"
# if it touches either the leading or trailing readout boundary;
# matches the 10-tick band the PDHD viewer uses at the 6000-tick edge.
EDGE_START_LE = 10
EDGE_END_GE = 11838
...
def main(argv: list[str]) -> None:
    ...
    # ------------- widgets -------------------------------------------------
    title_div = Div(
        text=(f"<h2>L1SP hand-scan — round 1 (PDVD)</h2>"
    ...
    curdoc().title = "L1SP hand-scan round 1 (PDVD)"
