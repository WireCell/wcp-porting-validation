"""Bokeh viewer for tuning HF (Wiener / Gaus_wide) and LF (ROI_*_lf)
software filters on top of the deconvolved waveform.

Background — the production SP chain produces, per plane, a magnify
TH2 named ``h{u,v,w}_wiener<ident>`` that holds the deconvolved
waveform AFTER FR/ER division and the geometric Wire_ind/Wire_col
filter, but BEFORE Wiener_tight, Wiener_wide, Gaus_wide or any LF
filter (see ``OmnibusSigProc::decon_2D_init`` lines 1027-1153, save
at lines 1818-1824).  This tool reads that frame and lets you apply
arbitrary HF and LF filters on top, live, with sliders and presets.

Filter forms (from ``util/src/Response.cxx:435-444``):
    HF: H(f) = exp(-0.5 * (f/sigma)**power),  H[0]=0 if flag=true
...
class MagnifyCache:
    """Lazy per-file loader: reads ``h{u,v,w}_wiener<ident>`` on demand."""
    HIST_PFX = {0: "hu_wiener", 1: "hv_wiener", 2: "hw_wiener"}

    def __init__(self, specs: list[FileSpec]):
        self._specs = {s.label: s for s in specs}
        self._frames: dict[tuple[str, int], tuple[np.ndarray, int]] = {}
    ...
    def get_plane(self, label: str, plane: int) -> tuple[np.ndarray, int]:
        """Return (data[n_chan, n_tick], channel_offset)."""
        key = (label, plane)
        ...
        fname = spec.path
        hname = f"{self.HIST_PFX[plane]}{spec.ident}"
        with uproot.open(fname) as f:
            try:
                h = f[hname]
            except KeyError:
                # Empty fallback: keeps app alive even if the frame is missing.
                self._frames[key] = (np.zeros((1, 6000), dtype=np.float32), 0)
                return self._frames[key]
            data = np.asarray(h.values(), dtype=np.float32)
            ch_offset = int(round(h.to_numpy()[1][0]))
        self._frames[key] = (data, ch_offset)
        return self._frames[key]


def best_channel(data: np.ndarray) -> int:
    """Index (within the loaded array) of the channel with largest peak-to-peak."""
    if data.size == 0:
        return 0
...
def main(argv):
    ...
    # ---- state ------------------------------------------------------------
    state = dict(
        wave_t=None,    # current channel waveform (n_tick,)
    ...
    f_time = figure(title="Decon waveform — raw (dashed grey) vs filtered (blue)",
                    **fig_kw_t)
    f_time.xaxis.axis_label = "tick (0.5 µs each)"
    f_time.yaxis.axis_label = "amplitude"

    f_filter = figure(title="Filter shapes |H_HF(f)|, |L_LF(f)|, and product",
                      height=240, sizing_mode="stretch_width",
                      active_scroll="wheel_zoom",
                      tools="pan,wheel_zoom,box_zoom,reset,save")
    f_filter.xaxis.axis_label = "frequency (MHz)"
    ...
    def load_channel():
        ...
        data, ch_offset = cache.get_plane(file_select.value, plane)
        n_chan, n_tick = data.shape
        # Resolve global channel → local index, clamped.
        try:
        ...
        wave = data[local].astype(np.float64)
    ...
    def on_file_change(_a, _o, _n):
        ...
        data, ch_offset = cache.get_plane(file_select.value, plane_radio.active)
        local = best_channel(data)
        chan_input.value = str(ch_offset + local)
        reload_and_render()
    ...
    refresh_preset_lists()
    data, ch_offset = cache.get_plane(file_select.value, plane_radio.active)
    chan_input.value = str(ch_offset + best_channel(data))
    reload_and_render()
    on_reset_range()
    ...
    layout = column(
        sel_row,
        info_div,
        row(hf_box, lf_box, sizing_mode="stretch_width"),
        range_row,
        f_time,
        f_filter,
        f_spec,
