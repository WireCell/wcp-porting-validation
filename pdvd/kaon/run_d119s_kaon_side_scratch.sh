#!/bin/bash
# doc pdvd/119 knob-ON arm on the top-only run 39305 (the kaon candidates); sec 8 reuses it with TAG.
...
#   clus : work/039305_<evt>_$TAG (default d119sidescratch; sec 5.4 used d119beamscratch) (imaging symlinked from the
...
for e in "${EVTS[@]}"; do
    ...
    ( one $e > $KDIR/logs/${TAG}_$e.log 2>&1; rc=$?; echo "rc=$rc" >> $KDIR/logs/${TAG}_$e.log; exit $rc ) &
done
while [ $(jobs -rp | wc -l) -gt 0 ]; do wait -n || nfail=$((nfail+1)); done
echo "run_d119_kaon_beam_scratch: ${#EVTS[@]} events, $nfail failed"
grep -H '^rc=' $(for e in "${EVTS[@]}"; do echo $KDIR/logs/${TAG}_$e.log; done)
exit $nfail
