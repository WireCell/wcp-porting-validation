// wct-sim-xtrack-sp.jsonnet  (ProtoDUNE-VD)
//
...
// pgrapher/experiment/protodunevd/sp.jsonnet) -> FrameFileSink, ONE anode per run.  Every
...
//
// Forked BY DUPLICATION from pdhd_sim/wct-sim-xtrack-sp.jsonnet (the PDHD twin) and this
// directory's wct-sim-check-track.jsonnet (both untouched).  SP is built from
...
//                      assumes: pdvd_track_fitting.json physical DL 4.1307 DT 7.9135 cm2/s,
//                      params.jsonnet 1.568 mm/us; lifetime 1e4 ms so charge/tick is flat)
...
//   l1sp_mode          'dump' = PDVD production (L1SPFilterPD passes the frame through);
//                      '' = bypass the node entirely
//   seed               Random seeds[0]
...
function(
    ...
    anode_index = 0,             // 0..3 bottom CRPs, 4..7 top
    output_prefix = '/home/xqian/tmp/xtrack/pdvd/S1',
    noise = true,
    fluctuate = true,
    diffusion = true,
    lar_DL = 4.1307,             // cm2/s
    lar_DT = 7.9135,             // cm2/s (PHYSICAL; the shipped fit JSON carries the empirical 4.872)
    lar_drift = 1.568,           // mm/us (params.jsonnet; the fit assumes the calibrated 1.48073 -- doc pdvd/29)
    lar_lifetime = 1.0e4,        // ms
    splat = false,
    nsigma = 3,
    wire_filter = 'production',  // | 'passthrough'
    dump_rawdecon = false,
    l1sp_mode = 'dump',          // PDVD production = 'dump' (pass-through); '' bypasses the node
    seed = 0,
    save_raw = true,
...
local splat_node = g.pnode({
    ...
    data: {
        anode: wc.tn(anode),
        field_response: wc.tn(tools.fields[if ident < 4 then 0 else 1]),
...
}, nin=1, nout=1, uses=[anode, tools.fields[if ident < 4 then 0 else 1]]);
...
local all_chans = util.anode_channels(ident);
local chndb = {
...
local nf_node = nf_maker(params, anode, chndb, ident, name='nf%d' % ident, intraces='orig%d' % anode_index);
...
local cmdline = {
    ...
    data: {
        plugins: ['WireCellGen', 'WireCellPgraph', 'WireCellSio', 'WireCellSigProc',
                  'WireCellAux', 'WireCellRoot', 'WireCellTbb'],
        apps: ['Pgrapher'],
    },
};
