#!/bin/bash
# Serve the PDHD imaging event display over HTTP for remote browser viewing.
#
# Usage: ./serve_img_viewer.sh [port] [evt.npz] [magnify-template]
#   port              (optional, default 5013)
...
# then open http://localhost:5013/img_viewer in the laptop browser.
...
if [ -n "${3:-}" ]; then
    ...
    MAGNIFY='/nfs/data/1/xqian/toolkit-dev/toolkit/pdhd/work/027409_0/magnify-run027409-evt0-apa{anode}-dnnroi.root'
fi
