...
* A slice's points all sit at exactly `x = time2drift(slice start)` (one x per
  slice, 0.315 cm apart — 4 ticks × 500 ns × 1.576 mm/µs); the
  viewer matches points to the displayed slice by
  `|pts_x − blob_x_start| < 0.1576 cm`, then keeps points in-polygon **or within
...
* `time_offset = 0` (no preset T0; was −250 µs before the chain zeroed it),
  `drift_speed = 1.576 mm/µs` (`cfg/pgrapher/experiment/pdhd/clus.jsonnet`);
