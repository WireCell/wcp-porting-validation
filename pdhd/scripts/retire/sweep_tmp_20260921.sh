...
# plan_20260921.py's sbnd open_prefix.  This releases ~15 GiB of ~/tmp that a plain sed
...
# tier; and the registered git worktrees the census lists (tmp_worktrees_20260921.txt)
...
while read -r p; do
  ...
  case "$p/" in "$T"/d119*|"$T"/d120*|*"/$THIS/"*) echo "REFUSING: live unit $p"; exit 6;; esac
  for k in $PERM; do
...
PY
  ...
  while read -r w; do
    [ -n "$w" ] || continue
    case "$w/" in "$T"/d119*|"$T"/d120*) echo "   WT held, skipped: $w"; continue;; esac
