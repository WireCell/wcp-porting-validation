...
TREES = {
 "sbnd": dict(
   ...
   substrate=[],
   # ROUND H (2026-09-19).  Production re-derived from primary sources, NOT inherited:
   #  * stage A  `work-<s>-d102m`  -- doc 109 line 512 "stay at eacacafe"; unchanged since 09-08.
   #  * stage B  `work-<s>-pr150s0` -- the OFF arm of doc pr/150 IS today's production
   #    (ref/prod-2026-09-17b); 269 vertex_labels resolve into it (cit 445).
   #  * `d102mpr` is the SUPERSEDED stage B (cit 175, 56 in scripts).  KEPT this round as a
   #    menu item: feedback_gate_source_arm_retired -- a superseded OUTPUT arm can still be the
   ...
   production=["d102m","pr150s0","d102mpr","d109prod"],
   scan_arms=[],
   flip_evidence=[],
   # d111/d112/d113 hold DROPPED for sbnd: doc sbnd_xin/113 is committed and pushed (f2a0d600)
   # and its session is this one.  d113adopt stays live via live_tokens() until its 4 metrics
   # TSVs under docs/pr/150_figs/metrics/ are committed -- that is the round's own record gate.
 ...
 "pdvd": dict(
   ...
   production=["d116vflip","d116vr2","q29flip","q29stm","d103vprod1"],
   scan_arms=scan_arms("pdvd"),
   flip_evidence=[],
   # the doc-116 round closed 2026-09-18; hold its prefix one round.
   open_prefix=("d116",),
   keep_arms=["magnify","ql_scores","ql_labels","stm_michel_labels",
              ...
              # ROUND H: the 2026-09-18 master-merge validation round (toolkit b93673ef/d2646110).
              # Undocumented, so INTERLOCK 14 cannot map it to a doc by round number.  The mg10
              # precedent (doc 105 sec 13) is that the OWNER releases a merge-validation round by
              # name; held here pending that call.  ~2.5 GiB.
              "mg18vbase","mg18vfix"],
   uncited_ok={},
   tier2={}, tier3={}, tier4={},
 ),
 ...
 "pdhd": dict(
   ...
   keep_arms=["stmw","allpd","ql_labels","stm_scan_labels","stm_michel_labels",
              ...
              "d101hnew",
              # ROUND H: see the pdvd note -- the 09-18 master-merge validation arms.
              "mg18hbase","mg18hfix"],
   uncited_ok={},
   tier2={}, tier3={}, tier4={},
 ),
