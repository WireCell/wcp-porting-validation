...
"""Round G (doc sbnd_xin/111 sec 17) worker: compress each listed file to <file>.zst, verified.
  ...
  compress_worker_20260921b.py LIST MANIFEST      # DRY RUN unless CONFIRM=yes in the environment
...
R = "/home/xqian/toolkit-dev/wcp-porting-img"   # round J: load() resolves relative manifest paths

JOBS = int(os.environ.get("COMPRESS_JOBS", "24"))

def load(man):
    """Accept BOTH manifest generations.  Round G wrote absolute paths and no header row;
    round J writes paths relative to R and a leading `# relpath size mtime sha256`.  Keying on
    the manifest spelling without normalising would make every lookup miss -- and the caller
    reads the target list, which is always absolute, so a miss reads as "not in the manifest"
    and the record gate refuses the whole run.  Normalise here instead of in three callers."""
    rows = {}
