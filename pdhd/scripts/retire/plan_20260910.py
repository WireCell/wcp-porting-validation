#!/usr/bin/env python3
"""
Cleanup round 2026-09-10 -- planner for sbnd_xin + pdvd + pdhd.  Doc 104.

OWNER INSTRUCTION, verbatim:
  "I would like to do a round of clean up for pdhd, pdvd, sbnd_xin, ~/tmp
   directory.  Note, we want to keep the latest production, but for work*
   directory that are intermediate results, we can retire them to save disk"
and, the same day:
  "We want to save the hand scan results for STM, which we still need them
   for improvements.  PDVD and PDHD"

Fork of plan_20260908.py.  All twelve interlocks are carried unchanged; read
that file's header for why each exists.  What follows is what THIS round
changes, and the first item is not a tuning -- it is the keep test itself.

  1. CITATION NO LONGER KEEPS AN ARM.  Every round from 09-02 to 09-08 auto-
     kept any dir a doc cited (`keep_dirs |= set(CITED)`), and 09-08 concluded
     in doc 103 sec 4 that pdvd and pdhd could therefore release NOTHING: 454
     pdvd arm families and 271 pdhd ones, every single one cited by the doc
     that made it.  That is the correct answer to "is this arm documented" and
     the wrong answer to the question the owner is actually asking.  A
     committed doc IS the record of a closed A/B; the per-event bytes beneath
     it are the intermediate result.  So CIT is now REPORTED, not obeyed, and
     an arm is kept only if it is:
        substrate | latest production | a hand-scan source | live | OPEN.
     INTERLOCK 15 is what keeps this honest: it refuses the round if the docs
     trees carrying the record are not fully committed.

  2. NEW KEEP CATEGORY: scan_arms, DERIVED NOT TYPED (INTERLOCK 13).
     scan_arms_20260910.py reads each scan prep set's own provenance key and
     each viewer's ARM binding.  This round would otherwise have released
     d53v, d53h, d67v, d68d4, d68a3, d08pv30on/off -- the entire evidentiary
     basis of smx1/smx1a/smx3/smx4, d08pvflip0 and the pdhd scans -- because
     a label JSON never names an arm (so INTERLOCK 9's manifest test sees
     nothing) and cit_20260908.py's ROOTS never included pdhd/stm_michel_scan
     (so the citation census saw nothing either).  Both gaps are now closed.

  3. NEW KEEP CATEGORY: OPEN decisions.  A closed doc is not a closed
     question.  Doc pdvd/81 shipped its two knobs default-OFF and handed the
     flip to the owner; its gate arms and ON arms are the evidence that
     decision will be made on, so p81*off3 / p81*q2d3 / p81*leg2 stay.

  4. THE SUBSTRATE MOVED, AND THE OLD CENSUS COULD NOT SEE IT.
     os.path.realpath() follows the WHOLE chain, so a link into
     <evt>_keep/<file> whose target is itself a link into
     /home/xqian/pdvd-frame-store resolved past `keep` and scored it 0
     inbound -- the intermediate hop is exactly what is being priced.  The
     corrected census (lexical target resolution, alias-canonical root)
     reproduces PROTECTED.txt's documented 968 for keep, and finds the new
     hub: d51vclus at 23728 inbound, d51hclus at 5132.  Both are now
     substrate, though the rest of their doc-50 families are released.

  5. PREFIX ENTRIES IN PROTECTED.txt ARE NOW HONOURED AS PREFIXES.  The old
     matcher did `a == p.strip("*")`, so a line reading `d51v*` protected
     nothing at all and the 09-08 round was relying on its inline open_prefix
     to do that job.  Fixed, in the safe direction.  The 09-08 blocks that
     were explicitly temporary ("protect by prefix while the round moves")
     have been MOVED to each file's RETIRED section with today's date, which
     is what makes this round's release possible at all.

  6. mg10 IS LIVE AND HAS NO DOC.  Arms in all three trees, ~/tmp/mg10
     written 09:11 today, a peer `claude --resume d9d688aa` alive.  An
     uncited live family is precisely what a citation-driven planner scores
     zero and releases (the 09-04 lesson, seven families between plan and
     confirm).  Protected by PREFIX in all three trees before the first plan
     run, not after an interlock caught it.
...
TREES = {
 ...
 "pdvd": dict(
   ...
   flip_evidence=[],
   open_prefix=("mg10","p79","p80","p81","p82"),
...
def live_tokens():
    ...
    ARMRE = re.compile(r"\b(?:p|d)[0-9]{1,3}[a-z][a-z0-9]{1,14}\b")
    for line in g:
...
def plan_tree(tree, cfg):
    ...
    open(os.path.join(HERE, f"prebroken_{tree}_{STAMP}.txt"), "w").write("\n".join(pre))
