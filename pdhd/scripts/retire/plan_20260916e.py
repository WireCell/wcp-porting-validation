#!/usr/bin/env python3
"""
Cleanup ROUND F 2026-09-16 -- pdhd/work + sbnd_xin only.  Doc sbnd_xin/111 sec 16.  Run as `plan_20260916e.py pdhd sbnd`.

OWNER, 2026-09-16 (after round E): "pdhd and sbnd still hold this same round's doc 111/113 test arms
(d111h*/d113h*: 683 dirs in pdhd, and d111s*/d113s* in sbnd). If you want those gone too", yes,
please go ahead removing those."  Fork of plan_20260916d.py (round E); the keep test is round D's.
What THIS round changes (pdhd + sbnd configs; the pdvd config is round E's and is not run):
  1. The d111/d113 prefix hold is lifted in pdhd and sbnd too (d112 still held).  keep_arms gain
     the doc-113 session's answer (message 2026-09-16, session now named pdvd-doc-97-refresh):
       pdhd d113hbase + d113hnone -- the pending hand scan of the 21 PDHD Michels lost in `none`;
       pdhd d111hst -- docs 112/113 Repro and the d112_* scripts' --arm (logs in ~/tmp/d111);
       sbnd work-mcp{1k,2k}-d113snew + work-mcp{1k,2k}-d111ssnew -- both sides of doc 113's G3 gate
         (figs/113_gate_sbnd.txt, d111_sbnd_gate.py --old d111ssnew --new d113snew).
     Not needed: d111h{A0,A1,K,S,off,off5,sr6,sr10,sr10tr,sr15,stsmk,tr}, d113h{base2,foot,nopaint,
     step1}, sbnd work-*-d111snew / work-*-d111sold.  The last two are doc 111's own SBND gate
     sides; its verdicts are committed (figs/111_gate_off_sbnd.txt, figs/111s_gate_sbnd.txt), the
     same standing as docs 109-110's gate arms released in round D.  No new prefix planned.
  2. LIVENESS against the pushed head ee91071b (remote_head_20260916e.txt).

--- round E's header follows ---
...
what THIS round changes:
  ...
  7. LIVENESS IS DERIVED AGAINST THE PUSHED REMOTE HEAD, not local main.  Local main
     ...
     LIVE_REF (default: the sha in remote_head_20260916e.txt) is compared instead.
...
TREES = {
 "sbnd": dict(
   ...
   # round F: d111/d113 released from the prefix hold (header item 1); d112 still held.
   open_prefix=("work-d103","work-d104","work-d149","work-d150","d112"),
   keep_arms=[
     # round F (header item 1): both sides of doc pdvd/113's G3 SBND gate.
     "work-mcp1k-d113snew","work-mcp2k-d113snew","work-mcp1k-d111ssnew","work-mcp2k-d111ssnew",
 ...
 "pdhd": dict(
   ...
   flip_evidence=[],
   # round D: see pdvd.  round F: d111/d113 released from the hold (header item 1); d112 still held.
   open_prefix=("d112",),
   ...
   keep_arms=["stmw","allpd","ql_labels","stm_scan_labels","stm_michel_labels",
              ...
              "d101hnew",
              # round F (header item 1): the pending PDHD Michel scan's two arms; the docs 112/113 Repro arm.
              "d113hbase","d113hnone","d111hst"],
   uncited_ok={},
   tier2={}, tier3={}, tier4={},
 ),
}
