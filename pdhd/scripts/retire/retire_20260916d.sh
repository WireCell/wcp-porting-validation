#!/bin/bash
# Cleanup ROUND D 2026-09-16 (doc sbnd_xin/111) -- deletion driver.  DRY RUN unless CONFIRM=yes.
# Fork of retire_20260913.sh with the stamp changed and nothing else; round C's header follows.
#
# Cleanup ROUND C 2026-09-13 (doc 106) -- deletion driver.
# Fork of retire_20260912b.sh.  One change: the record gate below now requires a
# manifest for EVERY target, not just the record directory (round B measured that
# "a directory exists" cannot tell a complete freeze from a partial one).
#
# --- round B's header follows ---
# Cleanup round 2026-09-12 (doc 105) -- deletion driver.  DRY RUN unless CONFIRM=yes.
#
#   ./retire_20260916d.sh 1 [tree ...]
#   CONFIRM=yes ./retire_20260916d.sh 1 pdvd
#
# Fork of retire_20260910.sh; the keep test is doc 104's, unchanged (see
# plan_20260916d.py's header).  THE ORDER IS NOT OPTIONAL:
#     1. python3 archive_records_20260916d.py 1     freeze the record layer (M13)
...
# counts against what plan_20260916d.py printed before typing CONFIRM=yes.
set -u
D="$(cd "$(dirname "$0")" && pwd)"
STAMP=20260916d
...
for t in $TREES; do
  ...
  REC=${REC_OVERRIDE:-$REC}     # the negative control points this at an incomplete copy
  if [ "$CONFIRM" = yes ] && [ ! -d "$REC" ]; then
    echo "   REFUSING: $REC missing -- run 'python3 archive_records_${STAMP}.py $TIER' first."
    exit 6
  fi
  if [ "$CONFIRM" = yes ]; then
    nom=$(while read -r p; do [ -s "$REC/$(basename "$p").manifest.tsv" ] || echo "$p"; done < "$TF" | wc -l)
    if [ "$nom" != 0 ]; then
      echo "   REFUSING: $nom of $n targets have no manifest in $REC -- the freeze is incomplete."
      exit 14
    fi
    echo "   record gate: $n/$n targets have a manifest in $REC"
  fi
...
echo "count per tree (prebroken_<tree>_20260916d.txt); this number only means"
