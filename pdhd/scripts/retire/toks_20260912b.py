...
"""Emit every token an arm dir can legitimately be cited BY.
  ...
  usage:  python3 toks_20260912b.py     # writes toks_20260912b.txt (flat) + toks_20260912b.json (per dir)
...
# 2026-09-12: STAMPED output names.  toks.json / toks.txt are committed records of
# the 09-06..09-10 rounds, and an unstamped name makes every round overwrite them.
json.dump(out, open("toks_20260912b.json", "w"), indent=0)
allt = set()
for v in out.values():
    allt |= set(v["toks"]) | {v["dir"], v["arm"]} | ({v["round"]} if v["round"] else set())
allt = {x for x in allt if x and x not in GENERIC}
open("toks_20260912b.txt", "w").write("\n".join(sorted(allt)) + "\n")
