...
"""Freeze the ORPHANED DRIVER LOGS of cleanup ROUND I (doc sbnd_xin/121) before removal.
  ...
  usage:  python3 archive_orphanlogs_20260925.py files_orphanlogs_pdvd_20260925.txt [...]
...
plan_orphanlogs_20260925.py's header: `.log` is not in archive_records' HEAVY list, so a
...
Output: $RETIRE_OUT (default sbnd_xin/archive/records/cleanup-20260925/orphanlogs).
...
OUT  = os.environ.get("RETIRE_OUT",
                      f"{R}/sbnd/sbnd_xin/archive/records/cleanup-20260925/orphanlogs")
...
def main(lists):
    if not lists:
        sys.exit("usage: archive_orphanlogs_20260925.py <files_orphanlogs_*.txt> ...")
    try:
