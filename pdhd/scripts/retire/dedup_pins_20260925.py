...
"""Hardlink-deduplicate the ~/tmp binary pins.  DRY RUN unless CONFIRM=yes.
  ...
  usage:  python3 dedup_pins_20260925.py            # report only
          CONFIRM=yes python3 dedup_pins_20260925.py
...
T = "/home/xqian/tmp"
# ROUND K fork (2026-09-25, doc sbnd_xin/125): the live holds are round K's -- the pdvd docs 117-119
# peer (pdvd-run-39305-tpc-beam-flash, busy; ~/tmp/d119inst holds SEVEN lib trees A..live and its
# install waiter is running, ~/tmp/d119wt/build a toolkit build), the FM campaign (wcfm-*) and every
# live Claude session scratchpad (claude-*).  A plain run of the 09-12 original would have discovered
# d119inst/*/lib as pin roots (>= 5 .so) and hardlinked + chmod -w'd them under a writing peer.
LIVE_TMP = ("h26", "h27", "h28", "p97", "mg10", "d117", "d118", "d119", "wcfm", "claude-")
# A live round may still be writing its pin or copy INTO it in place; hardlinking
# and chmod -w under it is exactly the hazard this file warns about.  Skipped.

def discover_roots(top=T, maxdepth=3):
    """Find every pin-shaped directory, rather than globbing the top level.

    2026-09-10: the 09-06 glob (`{T}/*libpin*` + `{T}/*libsnap*`) saw 28 roots
    and missed 21 more -- every pin a round wrote INSIDE its own dir
    (~/tmp/p81/libpin_p81, ~/tmp/d64/libpin, ...).  Those are the newest pins
    and the ones that have never been deduplicated.  So: match the name at any
    depth, and also take any directory holding >= 5 shared objects, which
    catches a pin whose name nobody thought of."""
    roots, base = set(), top.rstrip("/").count("/")
    for cur, subs, files in os.walk(top):
        if cur.count("/") - base >= maxdepth:
            subs[:] = []
        subs[:] = [s for s in subs if not os.path.islink(os.path.join(cur, s))
                   and not (cur == top and s.startswith(LIVE_TMP))]
        name = os.path.basename(cur)
        nso  = sum(1 for f in files if f.endswith(".so") or ".so." in f)
        if cur != top and (any(k in name for k in ("libpin", "libsnap", "pinlib"))
                           or nso >= 5):
            roots.add(cur)
            subs[:] = []          # do not also add its children
    return sorted(roots)
