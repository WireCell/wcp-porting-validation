#!/usr/bin/env python3
"""
Cleanup ROUND C 2026-09-13 -- planner for sbnd_xin + pdvd + pdhd.  Doc 106.

OWNER INSTRUCTION, verbatim (2026-09-13):
  "I wonder if there are rooms to further improve? Note, we can retire the
   intermediate results now. Four directories ~/tmp, sbnd_xin, pdvd, pdhd,
   please. We can clean up a bit the work* directories."
Same keep test as doc 104/105 (substrate | latest production | hand-scan source |
OPEN), applied now that the h26-h28/p97 peer round is committed and pushed (doc
pdhd/28 5f53a58b, doc 26 sec 3.4 1a3c7d87; the peer confirmed 2026-09-13 it
writes nothing).  Fork of plan_20260912b.py; what THIS round changes:

  1. PDHD PRODUCTION MOVED.  doc pdhd/28 line 12: "PDHD's is now h28prod (pin
     libpin_h28, Clus md5 30113227)"; gate F: h28prod == h28wl bit-identical, so
     production = [h28prod, h28wl] -- one claim, two arms, as p96vprod/p96vscope.
     h28off (knob off on the same pin, == h26q2dprod on every branch, row, tree,
     zip, calib and census: docs/scan/d28/gate_h28off.txt) is KEPT as the flip's
     OFF baseline (doc 98: retiring the OFF side kills the check; the peer's ask).
     h26q2dprod / h26conf / h26q2d are superseded production and its gate pair.
     This is set BEFORE the h28 prefix is dropped -- otherwise production falls
     out of every keep category at once.
  2. THE LIVE PREFIXES h26/h27/h28/p97 ARE GONE; h25 is held by prefix (the h25
     lever arms are scan sources, and a blind re-judge session is still open).
  3. DOC 81'S OPEN DECISION IS CLOSED.  PROTECTED held p81v*/p81h* as "an OPEN
     decision".  Doc pdvd/95 answered sec 9 item 6 and flipped PDVD; doc pdhd/26
     lines 114-115: "Doc pdvd/81 sec 8a held PDHD OFF because its Michels lean more
     on cross-shared cells; the owner took the flip with that known" (8ae3bfba).
     pdhd/wct-pr-perevt.jsonnet now has michel_q2d: true.  The p81 arms go.
  4. SUBSTRATE RE-DERIVED BY A LEXICAL FIRST-HOP CENSUS (2026-09-13), not carried
     over.  Zero inbound links and not a runner's SRC: pdvd d48nu7, d143pnew (its
     only inbound is d08cfgA's 2 links, released with it and d48flipcfg, whose
     links chain through d08cfgA), d28dlfp, d34base, d31r6e2e, d11vtrace (+ the 2
     d42fit dirs it held); pdhd d11prod, d11sepoffA, d11seponA, stmwc, d02prod.
     KEPT although zero-inbound: d16vnu / d16hnu -- the PR runners' SRC tag
     (d95_arms.sh:54, d96_arms.sh:64, ~/tmp/h28/run_arm28.sh:9); production cannot
     be re-run without them.  sbnd work-dbg25a-ql: its only borrower
     (work-dbg25a-d97prodchk, 20 links) is released, so it goes too.
  5. sbnd: superseded flip-check / production-check evidence goes -- d97prodchk
     (prod-2026-09-04), d99r3prod* (prod-09-05), d145prod (pr/145 shipped arm,
     production is d102m since), d147-* (flip-equivalence of a closed pr/147),
     87* (doc 87 output-knob evidence).  KEPT: d102m/d102mpr production,
     vtx105-base + pr130r1-probe* + pr134-f086 (scan/display manifests resolve
     into them), the sentinel layer (s144*, sent97), d146sv25 (pr/146 sec 12 OPEN,
     kine_sat_cont_keep_deg still null in production), probe178410a, tfix388-r9.
Interlocks and the confirm-time re-plan are round B's, unchanged.
...
TREES = {
 "sbnd": dict(
   ...
   uncited_ok={"87grp-def": G87, "87grp-min": G87, "87grp-nopct": G87},
   tier2={}, tier3={}, tier4={},
 ),
 ...
 "pdvd": dict(
   ...
   substrate=["d51vclus","keep","d27fresh","d41prov","d39r2prov","d16vnu","(bare)"],
 ...
 "pdhd": dict(
   ...
   substrate=["d51hclus","d09","d09ctl","d09ctl2","stm0","(bare)","d16hnu","wcc"],
   ...
   keep_arms=["stmw","allpd","ql_labels","stm_scan_labels","stm_michel_labels",
              ...
              "h28off","h28cfg","h28cfg0","h28cfgT","h28cfgoff"],
   uncited_ok={},
   tier2={}, tier3={}, tier4={},
 ),
}
