#!/usr/bin/env python3
"""ROUND K PASS 2 (2026-09-25, stamp 20260925b, doc sbnd_xin/125 sec 9.3): a fork of plan_20260925.py
whose ONLY change is that pr150s0 leaves `production` (see the sbnd TREES entry).  Every other
line, interlock and hold is pass 1's; the stamp is new so pass 1's committed tier/keep records are
never overwritten.


Cleanup ROUND K 2026-09-25 -- planner for sbnd_xin + pdvd + pdhd.  Doc sbnd_xin/125.

OWNER INSTRUCTION, verbatim (2026-09-25):
  "Hi, the /home/xqian disk is quite full, I think it is time to retire some directories in
   ~/tmp, ./pdvd ./pdhd ./sbnd_xin. We have done several rounds of this clean up. We want to
   keep the latest production, and can retire the intemediate results and work* directories.
   Can you investigate this and perform the cleanup. Can you summarize in a dedicated md file,
   commit and push."
OWNER RULINGS 2026-09-25 (asked with the measurements): (1) d123hits{,pr} -- "Release all of
it" (r3nue too); (2) the stale-protected old SBND production set -- "All but d115pr"; (3) the
file-level lever -- "Both" (plan_files_20260925b.py); (4) PDVD -- the ToT Q/L scan sources,
q29flip/q29stm and the doc 111-113 peer holds all go.

Fork of plan_20260921b.py.  What THIS round changes (all in TREES below):
  1. SBND production re-derived value-first: d123lgop/lgoppr + d123lgflip*; substrate d123base.
  2. pr150s0 kept as the SENTINEL REFERENCE, not as production.
  3. The d119/d120 prefix hold narrows to the live pdvd/119 arms; RELEASE_EXACT (new, gated by
     INTERLOCK 17) releases the closed sbnd/119 arms that sit under the live pdvd/119 prefix.
  4. PDVD production = q35flip (doc qlmatch/35 sec 11); d117/d118/d119 held live.
  5. LIVENESS pins remote_head_20260925b.txt = 07ae168f, verified by ls-remote over https.

--- round J's header follows ---
...
plan_20260919.py; what THIS round changes:
  ...
  6. LIVENESS pins remote_head_20260925b.txt = f04d8937, VERIFIED equal to the remote by
...
TREES = {
 "sbnd": dict(
   ...
   # ROUND K (2026-09-25).  SBND production moved TWICE today (doc 123 sec 17: flash_source=hits,
   # ref/prod-2026-09-25; sec 19: the QLXTPC scenario-1 light gate + ceiling, ref/prod-2026-09-25b).
   # SUBSTRATE: `d123base` (x7).  Its g*/ holds the ONLY SP frames + imaging on our disk -- d115 and
   # d102m hold 0 frames-dnn.tar.bz2 since round J -- and every d123 Q/L arm links into
   # work-<s>-d123base/g*/ by ABSOLUTE path (hits_arm.sh step 3; 315/630/770/315 links for
   # mcp1k/mcp2k/r3cv/r3off lgop).  mc_base.sh built the round-3 bases from reco1 via
   # products/d115/<s>/files.lst, not from d115.
   substrate=["d123base"],
   # PRODUCTION, value-first.  The shipped values are sec 17's flash_source=hits and sec 19's
   # xtpc_sc1_light_gate=true + xtpc_sc1_overpred_max=2.9.  `d123lgop` carries exactly those (by
   # TLA) on the four samples it was measured on, `d123lgoppr` is its stage B; `d123lgflip` is the
   # flipped runner with NO TLA on the production libs, proven == lgop (sec 19.3: 48/48 A+B, MC
   # 18/18) and the only production-runner output for nueCC-48 / NCpi0 (sec 19.4); `d123lgflipoff`
   # is its SBND_XTPC_SC1_GATE=0 escape (== the pre-flip hits arm 48/48), `d123lgflippr` its stage B.
   # ROUND K PASS 2 (stamp 20260925b, doc 125 sec 9.3): `pr150s0` RELEASES.  The suite is re-baselined
   # on production (scripts/analysis/d125/sentinels_prod.py: 13 PASS / 0 FAIL / 10 OPEN / 7 INERT); its
   # 8 d123 FAILs are exactly the doc-118 tfull cost pr/150 recorded before the flip, and the owner
   # ruled them OPEN (2026-09-25, "OPEN as doc 118 cost, release").  The pre-flip record lives on in
   # the 164 MB witness work-sent150-* (PROTECTED; 21/0/2/7, verdict-for-verdict == pr150s0).
   production=["d123lgop","d123lgoppr","d123lgflip","d123lgflipoff","d123lgflippr"],
   scan_arms=[],
   flip_evidence=["d123flip","d123flipoff","d123flippr"],
   # ROUND K: the generic d119/d120 prefix hold is DROPPED -- doc sbnd_xin/119 and 120 are closed and
   # pushed, so the r3* d119 arms (incl. round J's carried `d119flip-torn`) release.  But FOUR
   # `work-nuecc48-d119{gA,gB,sgA,sgB}` dirs belong to the LIVE pdvd doc 119 round (its knob-off
   # gates on SBND, pdvd/docs/nf_sp_img_clus/119_*.md) and link into work-nuecc48-d123flip/g0, and
   # `work-mcp10-m66d119{A,B}sb` were created today by abtest/sbndgate_run.sh -- held by prefix.
   open_prefix=("work-nuecc48-d119","work-mcp10-m66d119"),
   keep_arms=[
     ...
     # pr150csp3bw: the B arm of pr/150's blind vertex scan (120 KEY items); re-rendering needs its
     # calib dumps (pr150_vtx_scan.py:34,56).  A scan source.
     "pr150csp3bw",
     # d115pr: the published truth baseline of docs 115-117 -- the OWNER'S CALL 2026-09-21, re-affirmed
     # 2026-09-25 ("All but d115pr").
     "d115pr",
     # ROUND K: pr150tcsp3bw, pr150g16new, d113g16off LEAVE (work-tags.md:3172 "Releasable"; no consumer).
     # ROUND K: the two EMPTY (4 KB) placeholders abtest/sbndgate_run.sh:11 left for the 09-18 merge gate
     # map to no doc (INTERLOCK 14 refused them on the first run); kept rather than excused by a ruling
     # nobody gave -- they free nothing.
     "work-mcp10-m66d34bsb","work-mcp10-m66d34nsb",
   ],
   uncited_ok={},
   release_exact=[],
   tier2={}, tier3={}, tier4={},
 ...
 "pdvd": dict(
   ...
   # ROUND K: PRODUCTION MOVED 2026-09-24 (doc pdvd/qlmatch/35 sec 11, wcp b989f63b): ToT light +
   # lasso_weight_unrailed + ks_sat_tol 0.3075 are the runner defaults.  `q35flip` = the flipped runner
   # with NO override, == the graded ToT cell `q35tk` 120/120 (35:496-501); it ran on
   # ~/tmp/p35/libpin_prod.  Its light is `039252_light*_tot`, read through .tlas -- light dirs are
   # outside this grammar and are not in scope this round (they fail closed).
   # `d116vflip` leaves production but STAYS: it is the record key-arm of d35_stm_grade.py:10 and the
   # release builder's source (stm_michel_release/build_release.py:64,76).  `d116vr2` = scan source.
   production=["q35flip","d116vflip"],
   scan_arms=scan_arms("pdvd"),
   # the ToT flip's gate controls: q35ctl (pre-flip production), q35tk (graded cell) -- both also the
   # smx35/smx35c STM scan sources -- and q31ctl/q34tk1 (C1-C3, C5; 35:118,199).
   flip_evidence=["q35tk","q35ctl","q31ctl","q34tk1"],
   # ROUND K: docs pdvd/117 (round 6 lists next steps), 118 and 119 are LIVE (peer session
   # pdvd-run-39305-tpc-beam-flash, busy; doc 119 written 10:36 today) -- held by prefix.  The two
   # closed-sbnd/119 arms d119vnu/d119vr3 are released by EXACT name below, not by lifting the prefix.
   open_prefix=("d117","d118","d119"),
   ...
   # ROUND K, owner 2026-09-25: q29flip/q29stm (superseded Q/L production), the ToT Q/L blind-scan
   # sources (q32ti, q33*, q34tk2/3, q34ck1-3) and the doc 111-113 peer holds (d111vst, d113vbase,
   # d113vnone) are released -- their PROTECTED lines move to RETIRED.
   uncited_ok={},
   release_exact=["d119vnu","d119vr3"],
   tier2={}, tier3={}, tier4={},
 ),
 ...
 "pdhd": dict(
   ...
   # production unchanged since 2026-09-18 (doc pdvd/116 sec 10).
   production=["d116hflip","d116hr2","d102hcs","d109hstm"],
   scan_arms=scan_arms("pdhd"),
   flip_evidence=[],
   # ROUND K: only the LIVE pdvd/119 knob-off gates on PDHD (d119gA/gB/sgA/sgB) are held.
   open_prefix=("d119g","d119sg"),
   keep_arms=["stmw","allpd","ql_labels","stm_scan_labels","stm_michel_labels",
              "d08_scan_labels","d05_scan_labels",
              "d101hnew"],
   uncited_ok={},
   release_exact=["d119hnu","d119hr3","d119nu"],
   tier2={}, tier3={}, tier4={},
 ),
}
...
def plan_tree(tree, cfg):
    ...
    EXACT = set(cfg.get("release_exact", []))
    def is_open(d, a):
        # ROUND K: an EXACT-name release overrides the open-prefix hold (and only that hold):
        # the closed sbnd/119 arms d119vnu/vr3/hnu/hr3 sit under the LIVE pdvd/119 prefix.
        if a in EXACT:
            return False
    ...
    # ---- INTERLOCK 17 (ROUND K): every EXACT-name release resolves, is in the release, and is
    # not production/substrate/scan/PROTECTED -- an override that names nothing, or that would
    # reach a kept name, is a config slip.
    ex_bad = []
    for a in sorted(EXACT):
        n = ndirs(a); inr = sum(1 for d in tier1 if universe[d] == a)
        if n == 0 or inr != n or a in keep_names or prot_hit(a, a):
            ex_bad.append(f"{a}(dirs {n}, released {inr})")
    check(tree, 17, not ex_bad,
          f"exact-name releases resolve and are wholly released ({sorted(EXACT) or 'none'}"
          f"{'; BAD: ' + str(ex_bad) if ex_bad else ''})")
