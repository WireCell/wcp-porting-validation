...
#
#   ./sweep_tmp_20260925.sh
#   CONFIRM=yes ./sweep_tmp_20260925.sh
#
# ROUND K (2026-09-25, doc sbnd_xin/125): the held prefixes are d117 d118 d119 wcfm (the live pdvd
# docs 117-119 peer and the FM campaign) and the permanent pins follow production (d123-libpin-r6,
# p35/libpin_prod).  The worktree step still runs `git -C wcp worktree remove`; toolkit worktrees
# are all under a held prefix this round.
#
# ROUND I (2026-09-21, doc sbnd_xin/121) changes exactly one thing in this file: the held
# prefixes.  They were `d111 d112 d113 d103 d108` -- the pdvd doc-113 PEER's scratch, held
# by round D because that session was live.  That peer is finished, docs pdvd/113-116 are
# all committed and pushed, and ListAgents shows no live peer, so the list narrows to this
# round's own open rounds `d119 d120` (docs 119 and 120 landed 2026-09-21), matching
# plan_20260925.py's sbnd open_prefix.  This releases ~15 GiB of ~/tmp that a plain sed
# fork would have carried on holding -- but each unit still has to clear the census's own
# rules (age, pins, citation, PROTECTED) on its own merits.
#
# Fork of sweep_tmp_20260913.sh.  Round D changes: the permanent pins follow production
# (libpin_d102, d102m-libsnap, pdhdstm_libpin); the census's fifth class (closed-round scratch) is in the
# tier; and the registered git worktrees the census lists (tmp_worktrees_20260925.txt)
