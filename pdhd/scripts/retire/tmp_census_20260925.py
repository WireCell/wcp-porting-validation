...
"""Cleanup ROUND D 2026-09-16 (doc sbnd_xin/111): the ~/tmp census.  Retires nothing.
  ...
  usage:  python3 tmp_census_20260925.py            # report + tmp_tier_20260925.txt
          python3 tmp_census_20260925.py --list     # print the tier only

Run AFTER plan_20260925.py: "alive" is what survives the work release, read from
its tier1_*_20260925.txt files, not what is on disk now.
...
# rounds, matching plan_20260925.py's sbnd open_prefix and sweep_tmp_20260925.sh.
# ROUND K: typed by hand, because live_tokens() sees nothing -- docs pdvd/117-119 are committed
# and pushed, yet pdvd/117 lists next steps and the peer pdvd-run-39305-tpc-beam-flash is busy
...
# ROUND K: the round-3 sample tags (round I fixed the planner's SAMP and not this copy).
...
def registered_worktrees():
    # ROUND K: BOTH repos -- ~/tmp/d119wt is a TOOLKIT worktree; asking only wcp would let a
    # unit containing it be rm -rf'd, leaving a stale registration in the toolkit repo.
    out = set()
    for repo in (R, TK):
        r = subprocess.run(["git", "-C", repo, "worktree", "list", "--porcelain"], capture_output=True, text=True).stdout
        out |= {l.split(" ", 1)[1] for l in r.splitlines() if l.startswith("worktree ")} - {repo}
    return out
