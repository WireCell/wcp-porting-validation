#!/bin/bash
# ~/tmp sweep, cleanup ROUND C 2026-09-13 (doc 106).  DRY RUN unless CONFIRM=yes.
#
#   ./sweep_tmp_20260913.sh
...
PERM="$T/h28/libpin_h28 $T/p96/libpin_p96 $T/pdhdstm_libpin"
...
while read -r p; do
  ...
  case "$p/" in "$T"/h25*|"$T"/h29*|*"/$THIS/"*) echo "REFUSING: live unit $p"; exit 6;; esac
  for k in $PERM; do
...
PY
  ...
  while read -r p; do
    c=$(dirname "$p")
    case "$c" in "$T"/*_libpin) rmdir "$c" 2>/dev/null && echo "   removed empty container $c";; esac
  done < "$TF"
else
