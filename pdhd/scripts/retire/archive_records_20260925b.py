...
HEAVY = [re.compile(p) for p in (
    ...
    # ROUND I, doc 111 sec 12 item 6 carried forward at last: round G (2026-09-16)
    # compressed 8569 calib/log/csv files in 26 cold pdvd arms to .zst, and none of the
    # patterns above match "calib-evt123-group02.json.zst" -- they all anchor on the
    # ORIGINAL extension.  Without this line a compressed heavy file reads as record
    # layer and is carried bodily into the .tar.zst.
    r'.*\.zst$')]
...
# (the tar is zstd already, so it costs nothing); a compressed .wct-*.json follows ONEPER.
RECZST = re.compile(r'^(stdout\.log|wct_[a-z]+(_evt\d+)?\.log|[a-z]+\.stdout|\.wct-.*\.json)\.zst$')
...
ONEPER = re.compile(r'^\.wct-.*\.json(\.zst)?$')

def is_heavy(n): return not RECZST.match(n) and any(p.match(n) for p in HEAVY)
