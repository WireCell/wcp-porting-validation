#!/usr/bin/env python3
"""Recompute the normalization-study metrics on the REPROCESSED dumps (lambda=100,
vuv_eff=0.023). Crosser anchors: at_x_boundary, ndf>=30, brightest valid meas>300,
dedup by main cluster keeping lowest ks. Uses pred_pe straight from the dumps."""
...
for ev in range(10):
    for grp in ('group02','group13'):
        ...
        if not g: continue
        d=json.load(open(g[0])); art=g[0].split('/')[-1].split('-')[1]
        ...
        for b in best.values():
            ...
            isc=sum(meas)/sum(pred)
            rows.append(dict(art=art,grp=grp,main=b['main_cluster'],ndf=b['ndf'],ks=b['ks_dis'],
                             measTot=sum(meas),N90m=n90(meas),N90p=n90(pred),
                             dark=dark*100,ds=ds,isc=isc))
# clean anchors: bright (measTot>3000) and reasonable match (ks<0.4)
...
for r in sorted(clean,key=lambda r:r['art']):
...
print('clean anchors: %d'%len(clean))
print('MEDIAN  N90m=%.0f  N90p=%.0f  dark%%=%.0f  direct-scale=%.2f  integral-scale=%.2f'%(
    med('N90m'),med('N90p'),med('dark'),med('ds'),med('isc')))
print('N90 ratio pred/meas median = %.2f'%statistics.median([r['N90p']/r['N90m'] for r in clean]))
