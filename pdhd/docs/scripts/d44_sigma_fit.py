#!/usr/bin/env python3
"""PDHD fork of doc pdvd/44's d44_sigma_fit.py -- derive the fit's EFFECTIVE
transverse smearing per plane, empirically.

Forked BY DUPLICATION from pdvd/docs/nf_sp_img_clus/scripts/d44_sigma_fit.py;
the PDVD script is untouched.  Two changes, both in DET:
  * a "pdhd" entry (pitch, x_anode, v, track-fitting JSON);
  * PDHD's plane thresholds are (3200, 6400) = the CUMULATIVE per-plane channel
    counts (3200 U, 3200 V, 3840 W).  These are NOT raw LArSoft channel numbers:
    PdvdMagnifyTrackingVisitor -- the visitor the PDHD PR job binds -- writes
    ...
    (PdvdMagnifyTrackingVisitor.h ChanScheme), exactly as PDVD's (3808, 7616)
    are ITS per-plane counts.  Reading them as raw channel ids silently
    mislabels every plane.
...
DET = {
    ...
    # PDHD.  The bounds are NOT raw channel ids: PdvdMagnifyTrackingVisitor (the
    # visitor pdhd/pr.jsonnet binds) writes pu/pv/pw and T_proj_data.channel in a
    # COMPACT PER-PLANE RANK -- global = base[plane] + rank of the wire's channel
    # among that plane's channels over the whole detector
    # (PdvdMagnifyTrackingVisitor.h ChanScheme).  PDHD has 3200 U, 3200 V and
    # 3840 W channels, so base = (0, 3200, 6400) and the two thresholds are
    # 3200 / 6400 -- exactly as PDVD's (3808, 7616) are ITS per-plane counts.
    # Reading them as raw LArSoft channel numbers (U 0-799, V 800-1599,
    # W 1600-2559 per 2560-channel APA block) mislabels every plane.
    # x_anode is the collection-plane x TrackFitting.cxx actually reads
    # (iface->planes()[2]->wires().front()->center().x()): 353.202 cm on the
    # anode-0/2 imaging face, 353.002 cm on anode-1/3's -- 2 mm apart, i.e. 1.3 us
    # of drift out of 2240, so one scalar is used.  v = the CALIBRATED PDHD drift
    # speed (params.jsonnet lar.drift_speed).
    # apa_chans: channels each APA contributes to each plane, in APA order --
    # 800 U, 800 V, 960 W (4 APAs x that = the 3200/3200/3840 above).  doc 47
    # sec 10 uses it to split by APA, because PDHD deconvolves APA0 against a
    # DIFFERENT field response file than APA1-3 (params.jsonnet files.fields:
    # np04hd-garfield-6paths-mcmc-bestfit, an MCMC refit to PDHD data, vs the
    # generic dune-garfield-1d565) -- a within-detector control on the field
    # response model.  NB APA0 also carries the per-wire-region `fltresp`
    # filters, plane2layer [0,2,1] and the _APA1 Wiener set, so the split is
    # only interpretable against the simulation's own S6b-vs-S1 difference.
    "pdhd": dict(pitch=(4.6693, 4.6693, 4.7920), bounds=(3200, 6400),
                 x_anode=353.10, v=1.576, apa_chans=(800, 800, 960),
                 slice_us=2.0, json=TOOLKIT_CFG + "/pdhd/pdhd_track_fitting.json"),
}
