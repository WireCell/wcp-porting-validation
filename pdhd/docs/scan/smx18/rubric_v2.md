...
3. **(v2) A flat profile means `THRU` only where the track could have left.**
   Look at `f_meas`. The track stopped if the **measured charge ends in all
   three planes** at the fit end, and:
   * no dead band or coverage hole swallows the end,
   * the end is not at a face, the cathode or the APA seam (use the numbers),
   * nothing continues past it within the ±150-channel window.
...
* `--notes` prefixes that are counted across the round: `OVERSHOOT:`,
  ...
  (the end is at an anode face), `REVERSED:` (the chain's "entry" end looks like
  the real stop candidate; the verdict is still for the fit end, and the far end
  cannot be judged from these frames).
