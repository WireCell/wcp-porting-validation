# Your task: a verdict-blind scan of assigned PDHD items (doc pdhd/25 §6, rubric v5)
...
scanned before. You are an independent look, and you must not see any earlier
answer. A second scanner judges the same items separately.

## Read this first, in full, before your first item

`/home/xqian/tmp/h25r/RUBRIC.md` (rubric **v5**)

It defines:
* the verdict alphabet and the **precedence order**, rules 1–7, including rule 7
  (cosmic muons travel down: topology decides at an upper end, as rule 1 does
  everywhere);
...
1. **Read only** the rubric, this file, your item list, and
   ...
   * Never open any file with `key`, `labels`, `verdicts`, `queue`, `rulings`,
     `sheet`, `adj`, `controls`, `preregistered`, `movers`, `census`, `twin`
     or `misses` in its name, and never open `ops/` or `lbl_*`.
   * Never open any prep payload (`smprep-*.json`), ROOT file, calib json or
     zip, or anything under `pdhd/work/`, `pdhd/docs/`, `pdhd/stm_michel_scan/prep-*`
     or `pdvd/docs/`.
   * **Never open anything under `/home/xqian/tmp/h18/` … `/home/xqian/tmp/h24/`
     or under `/home/xqian/tmp/h25/`** (note: `h25`, not `h25r`). Those are
     earlier scans and the reconstruction runs. Read frames only through `<SHOTS>`.
   * Under `/home/xqian/tmp/h25r/`, open only `RUBRIC.md`, this file, `shots/`,
...
3. **Do not list or read any other scanner's out dir, scratch dir or item list.**

## Paths

* `<SHOTS>` = `/home/xqian/tmp/h25r/shots`
...
look, so expect many of them to be hard. That is not a reason to lean either
way. Do not aim for any particular mix of verdicts. Mark confidence honestly.
Say in the evidence paragraph what made a call hard, and name the competing
reading.
