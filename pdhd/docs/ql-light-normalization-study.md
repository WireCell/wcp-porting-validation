...
Cumulative light vs PMT rank for the 7 clean crossers — measured (black) rises much
faster than the original prediction (red, λ=2000). *(The figures here were
regenerated as before/after — the green λ=100 curves are the tuned result described
...
*Per crosser (rows), columns = measured | pred λ=2000 (before) | pred λ=100 (after),*
2-D PMT map (y vs z), colour = normalized PE (shape only). The middle column is
visibly the most diffuse (many comparable secondary PMTs); the right column
(tuned) localises the predicted light toward the measured pattern (left).
...
### Before/after figures (real calib-mode pred_pe, all 7 clean anchors)

The figures compare **measured** vs the **real QLMatching `pred_pe`** dumped at
λ=2000 (before) and λ=100 (after) — generated by reprocessing the anchor events
with `run_clus_evt.sh -calib` at each model, not from the offline re-predictor.

![per-PMT PE before/after](../pics/ql_norm_perpmt.png)

*Per-PMT PE (channels sorted by measured, symlog), measured / before / after, for
all 7 clean crossers.* After (green) lifts the bright PMTs onto the measured level
(normalization) and drops the long over-predicted tail (spread). evt198 and evt270
overlap measured almost perfectly; evt150 (the 29k-point multi-track blob) keeps a
visible tail — the residual a single global λ cannot remove.

See also `ql_norm_concentration.png` (cumulative concentration) and
`ql_norm_crosser_patterns.png` (2-D y–z maps) above, both regenerated before/after.

Analysis scripts: `pdhd/ql_light_calib/` (`repredict.py` validated re-predictor,
`fit.py` λ sweep, `after_metrics.py` reprocessed-dump metrics, `plot_norm.py`
before/after figures from the calib dumps).
