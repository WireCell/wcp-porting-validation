...
#   (up to nproc jobs, override with PDHD_MAX_JOBS=N), then combines all
...
if [ "$EVT" != "all" ]; then
    ...
    if [ -n "$ANODE" ]; then
        TAG_SUFFIX="_a${ANODE}"
        ANODE_PAIRS="${ANODE}:${CLUS_INPUT}/clusters-apa-apa${ANODE}-ms-active.tar.gz"
    else
        TAG_SUFFIX=""
        ANODE_PAIRS=""
        for i in 0 1 2 3; do
            ANODE_PAIRS="$ANODE_PAIRS ${i}:${CLUS_INPUT}/clusters-apa-apa${i}-ms-active.tar.gz"
        done
    fi
...
# in parallel (up to PDHD_MAX_JOBS).  After all events, zip and upload once.
#
...
#
# Speed/t0/x0 geometry: even APAs (0, 2) drift -x; odd APAs (1, 3) drift +x.
# These values must stay in sync with wct-img-2-bee.py:anode_args().
bee_anode_args() {
    local idx=$1
    if [ $((idx % 2)) -eq 0 ]; then
        echo '--speed "-1.565*mm/us" --t0 "0*us" --x0 "-353.2*cm"'
    else
        echo '--speed "1.565*mm/us" --t0 "0*us" --x0 "353.2*cm"'
    fi
}
...
for _e in "${_all_events[@]}"; do
    _workdir="$PDHD_DIR/work/${RUN_PADDED}_${_e}"
    _clus_input=""
    if ls "$_workdir/clusters-apa-apa"*"-ms-active.tar.gz" >/dev/null 2>&1; then
        _clus_input="$_workdir"
    else
        for _rname in "run${RUN}" "run${RUN_PADDED}" "run${RUN_STRIPPED}"; do
            _rdir="$PDHD_DIR/input_data/$_rname"
            [ -d "$_rdir" ] || continue
            for _ename in "evt${_e}" "evt_${_e}"; do
                _cand="$_rdir/$_ename"
                if [ -d "$_cand" ] && \
                   ls "$_cand/clusters-apa-apa"*"-ms-active.tar.gz" >/dev/null 2>&1; then
                    _clus_input="$_cand"
                    break 2
                fi
            done
            if ls "$_rdir/clusters-apa-apa"*"-ms-active.tar.gz" >/dev/null 2>&1; then
                _clus_input="$_rdir"
                break
            fi
        done
    ...
    _apa0_clus=$(ls "$_clus_input/clusters-apa-apa"*"-ms-active.tar.gz" 2>/dev/null | head -1)
    _event_no=$(tar tzf "$_apa0_clus" | head -1 | sed -E 's/.*cluster_([0-9]+)_.*/\1/')
    if ! echo "$_event_no" | grep -qE '^[0-9]+$'; then
        echo "[skip] run=$RUN evt=$_e: cannot parse event number from $_apa0_clus" >&2
        BATCH_FAIL=$((BATCH_FAIL + 1))
        BATCH_FAIL_LIST+=("$_e")
        continue
    fi
