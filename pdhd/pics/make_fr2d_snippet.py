#!/usr/bin/env python3
"""2D time-domain field-response inset (V plane) for the PD-HD sim-chain diagram.

Counterpart of pdvd/pics/make_fr2d_snippet.py (PD-VD original) — only the
field-response file differs: PD-HD simulation uses the dune-garfield-1d565
response (the APA1-3 production FR).

Loads the PD-HD field-response file and renders the induced-current response
...
Output: pdhd/pics/sim_chain_src/field_response_2d_time_V.png
"""
import os
import numpy as np
import matplotlib
...
def main():
    ...
    ax.set_ylabel("wire offset from track", fontsize=11)
    ax.set_title("field response  —  %s plane (induction)" % PNAME,
                 fontsize=11)
    ax.tick_params(labelsize=10)
    cb = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.03)
    cb.set_label("induced current  [a.u.]", fontsize=10)
