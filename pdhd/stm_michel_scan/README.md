...
| `../docs/scan/pdhd_stm_michel_smx21_verdicts.json` | the doc pdhd/19 §9 record, the current PDHD one: smx20 plus the owner's eight `own19` judgements (`owner_review`) and the v5 double scan of the 26 unqueued `FLAT_STOP` items (`review_v5`: 20 confirmed, 2 adopted, 4 split). A split keeps the previous call and carries `owner_queue`. Round files in `../docs/scan/smx21/`, including the plan written before the results |
