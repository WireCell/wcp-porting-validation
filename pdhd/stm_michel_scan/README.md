...
| `scan_harness.py` | drives **this app** headlessly (bokeh serve on a scratch port + playwright chromium) so an agent can scan by looking: `shots` writes 7 framed PNGs per item, `context` dumps the object table and the chain's scalars, `apply` presses the real widgets — radio, every object row + tag button, the pin, the verdict, SAVE — and re-reads `labels.json` to prove each row landed. Never touches the port a human is on. Used for doc [pdvd/55](../../pdvd/docs/nf_sp_img_clus/55_stm-michel-handscan-pdvd-tranche1.md) |
