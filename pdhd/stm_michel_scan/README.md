...
| `campaign/` | doc [pdhd/18](../docs/18_stm-michel-handscan-pdhd.md): the multi-agent scan round, committed this time — `AGENT_TASK.md` (the scanner's brief), `mkv.py` (the only write path: alphabets, every row tagged, kind from tags, verdict against tags, rubric sha stamped), `nextwave.py` (waves recomputed from disk; `--double` for a seeded re-scan), `shoot.sh`, `mkzoom.py` (the 3× dQ/dx frame), `resolve.py` (`--supersede`, `--partial`), `mkspec.py` / `apply_parallel.sh` (the apply, pipelined by batch), `fixup_spec.py` (rows that did not land), `merge.py` (into a NEW tag, sha-guarded), `mkrecord.py`, `mkqueue.py` (tiered owner queue), `cmp_owner.py` (the blind calibration). Doc [pdhd/19](../docs/19_stm-michel-display-fix-and-review.md), the review round: `cmp_review.py` (review vs the round it reviews, by the private tier map and by confidence; writes the adjudication list), `mkadj.py` (adjudication packets, the two scans as A/B in seeded random order, map kept outside the round), `mkreview_record.py` (the post-review record: reviewed items rebuilt with a `review` block, the rest copied verbatim), `merge.py --base TAG` (carry the unreviewed rows of an existing tag into the new one). Doc pdhd/19 §8, the owner's rulings: `mkowner_record.py` (adds the owner's chat rulings as `owner_review`, and judges a two-scanner v5 pass per item as confirmed / adopted / split; `--skip-v5` gives the rulings' census effect alone; `--stopper-split` judges the pass on stopper-or-not only, so a kind or `THRU`/`UNCLEAR` difference inside an agreed class is resolved by rule instead of going to the owner, and an `owner_queue` flag is cleared once the owner rules) |
| `../docs/scan/pdhd_stm_michel_smx22_verdicts.json` | the doc pdhd/20 record, **the current PDHD one**: smx21 plus the owner's twelve `own20` judgements, made on the FIXED dQ/dx panel (`--dead-points`, which draws the points the chain discards as hollow grey instead of as measurements). Three of their eight earlier calls changed and all four smx21 splits were settled, so no item carries `owner_queue` any more. 16 `owner_review` blocks = these 12 plus 4 carried from earlier rounds. Labels stay at tag `smx21` — this round adopts no item, so no label row changes. Round files in `../docs/scan/smx22/` |
| `../docs/scan/pdhd_stm_michel_smx21_verdicts.json` | the doc pdhd/19 §9 record: smx20 plus the owner's eight `own19` judgements (`owner_review`) and the v5 double scan of the 26 unqueued `FLAT_STOP` items (`review_v5`: 20 confirmed, 2 adopted, 4 split). A split keeps the previous call and carries `owner_queue`. Round files in `../docs/scan/smx21/`, including the plan written before the results |
| `../docs/scan/pdhd_stm_michel_smx20_verdicts.json` | the doc pdhd/19 §8 record: the smx19 record plus the owner's five chat rulings (`owner_review`, which `d18_census.py` takes before `owner_smx1`) and the v5 double scan of five upper-end topology items (`review_v5`). Rubric v5. Round files in `../docs/scan/smx20/` |
| `../docs/scan/pdhd_stm_michel_smx19_verdicts.json` | the doc pdhd/19 record: 317 records. The 94 items of doc 18's owner queue (tiers A–E) were re-scanned blind on the fixed display (rubric v4) and adjudicated where the class changed; each carries a `review` block keeping the smx18 call and the blind call. The other 223 are copied from smx18 verbatim. The owner's `smx1` label still decides the 30 owner items. Label tag `smx19` (`merge.py --base smx18`); round files in `../docs/scan/smx19/` |
...
| `../../pdvd/docs/scan/pdvd_stm_michel_smx1a_verdicts.json` | the doc pdvd/55 scan record: 60 items, each with verdict, Michel kind, confidence, pin, every object tag and the sentence of evidence behind the call. Replays into a tag with `scan_harness.py apply` |
