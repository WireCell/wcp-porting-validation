#!/bin/bash
# Shared helper library for wcfm/run_*.sh scripts.
# Source after setting WCFM_DIR:
#   . "$WCFM_DIR/_runlib.sh"

# List runs found under input_data/.  Called when the invoking script
# receives no positional arguments.
list_runs() {
    local base="$WCFM_DIR/input_data"
    [ -d "$base" ] || { echo "no input_data/ under $WCFM_DIR" >&2; return 1; }
...
#   batch_summary; exit $?
#
# Concurrency cap: ${WCFM_MAX_JOBS:-6} (CLAUDE.md sec 2: never oversubscribe the shared box).

batch_init() {
    BATCH_OK=0
    BATCH_FAIL=0
    BATCH_FAIL_LIST=()
    BATCH_MAX=${WCFM_MAX_JOBS:-6}
