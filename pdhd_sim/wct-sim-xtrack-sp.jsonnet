// wct-sim-xtrack-sp.jsonnet  (ProtoDUNE-HD)
//
...
// pgrapher/experiment/pdhd/sp.jsonnet) -> FrameFileSink, ONE anode per run.  Every
...
//
// Forked BY DUPLICATION from wct-sim-check-track.jsonnet (untouched).  SP is built from
...
//                      assumes: pdhd_track_fitting.json DL 4.1207 DT 8.2017 cm2/s,
//                      params.jsonnet 1.576 mm/us; lifetime 1e4 ms so charge/tick is flat)
...
//   l1sp_mode          '' = bypass L1SPFilterPD (PRIMARY here: bare OmnibusSigProc output);
//                      'process' = PDHD data production
//   seed               Random seeds[0]
...
//   wire-cell -V elecGain=14 --tla-code "tracks=$(cat tracks.json)" --tla-code anode_index=1 \
...
function(
    ...
    anode_index = 1,
    output_prefix = '/home/xqian/tmp/xtrack/pdhd/S1',
    noise = true,
    fluctuate = true,
    diffusion = true,
    lar_DL = 4.1207,             // cm2/s
    lar_DT = 8.2017,             // cm2/s
    lar_drift = 1.576,           // mm/us
    lar_lifetime = 1.0e4,        // ms
    splat = false,
    nsigma = 3,
    wire_filter = 'production',  // | 'passthrough'
    dump_rawdecon = false,
    l1sp_mode = '',              // '' bypass | 'process' (PDHD production)
    seed = 0,
    save_raw = true,
...
local splat_node = g.pnode({
    ...
    data: {
        anode: wc.tn(anode),
        field_response: wc.tn(tools.fields[ident]),
...
}, nin=1, nout=1, uses=[anode, tools.fields[ident]]);
...
local all_chans = std.range(ident * 2560, (ident + 1) * 2560 - 1);
local chndb = {
